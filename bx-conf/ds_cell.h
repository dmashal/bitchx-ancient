...
typedef struct cell {
    ...
    int	cur_pos;		/* current cursor column when using
					 * edit */
    int	cur_row;		/* current cursor row when using edit */
    int	desc_start;		/* Used when editing large fields. */

    unsigned int termkey;		/* this is cleared on enter, setting
					 * it to any value other than zero
					 * causes the function to return.
					 * Usually set to the key we terminate
					 * with. */
    unsigned int special;		/* Holds value for the current item
					 * function number */


    unsigned int key;		/* Current Keystroke */
    unsigned int other_getkey;	/* set this to use other get_char
					 * routine */
    /* if supplied */
    ...
    int	menu_bar;		/* Set this if a menu at the bottom of
