/*
 * alist.h -- resizeable arrays Copyright 1997 EPIC Software Labs
 */
...
static __inline u_32int_t
cs_alist_hash(const char *s, u_32int_t * mask)
{
	u_32int_t x;
...
static __inline u_32int_t
ci_alist_hash(const char *s, u_32int_t * mask)
{
	u_32int_t x;
...
/*
 * Everything that is to be filed with this system should have an identifying
 * name as the first item in the struct.
 */
typedef struct {
	char           *name;
	u_32int_t hash;
...
/*
 * This is the actual list, that contains structs that are of the form
 * described above.  It contains the current size and the maximum size of the
 * array.
 */
typedef struct {
	Array_item    **list;
