...
/*
 * type definition to distinguish different server versions
 */
#define Server2_7       0
#define Server2_8       1
#define Server2_9       2
...
typedef struct {
	...
	int orignick_pending;                   /* Is a ORIGNICK command
						 * pending? */
	int server_change_pending;              /* We are waiting for a
						 * server change */
	...
	int sent;               /* set if something has been sent, used for
				 * redirect */
	int lag;                /* indication of lag from server CDE */
	time_t lag_time;                /* time ping sent to server CDE */
	time_t last_msg;                /* last mesg recieved from the server
					 * CDE */
	...
	struct sockaddr_foobar local_addr;      /* ip address of this
						 * connection */
	struct sockaddr_foobar uh_addr; /* ip address of this connection */
	struct sockaddr_foobar local_sockname;  /* sockname of this
						 * connection */
	...
	void (*parse_server)(char *);                   /* pointer to parser for
							 * this server */
	...
	char           *recv_nick;
	/*
	 * sent_nick: the nickname of the last person to whom you sent a
	 * privmsg
	 */
	char           *sent_nick;
	char           *sent_body;
