...
/*
 * DEFAULT_DOUBLE STATUS  can be defined as 0 or 1 0 makes the status line be
 * 1 line only. 1 makes it 2 lines this will be the default for all created
 * windows. DEFAULT_STATUS_LINES can be defined as 0 or 1 0 is off, and 1
 * makes the window split the default for ALL windows. a window split is the
 * topic bar.
 */
...
/*
 * #define DEFAULT_FORMAT_NONICK_FROM_FSET "$G %W$1%K:%n $3- %K[%Cfrom
 * %c$2%K]"
 */
...
/*
 * #define DEFAULT_FORMAT_USERS_FOOTER_FSET "%K[ %nEnd of channel %W$0 %K-
 * %W$1 %nusers counted %K]"
 */
...
#define DEFAULT_FORMAT_USERS_HEADER_FSET NULL   /* "%K[ %WC%nhannel  %K][
						 * %WN%wickname %K][%n
						 * %Wu%wser@host
						 * %K][%n %Wl%wevel
						 * %K]" */
