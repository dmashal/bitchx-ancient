...
/* These are in expr.c */
/*
 * This function is a general purpose interface to alias expansion. The
 * second argument is the text to be expanded. The third argument are the
 * command line expandoes $0, $1, etc. The fourth argument is a flag whether
 * $0, $1, etc are used The fifth argument, if set, controls whether only the
 * first "command" will be expanded.  If set, this argument will be set to
 * the "rest" of the commands (after the first semicolon, or the null).  If
 * NULL, then the entire text will be expanded.
 */
extern char    *BX_expand_alias(const char *, const char *, int *, char **);

/*
 * This is the interface to the "expression parser" The first argument is the
 * expression to be parsed The second argument is the command line expandoes
 * ($0, $1, etc) The third argument will be set if the command line expandoes
 * are used.
 */
extern char    *BX_parse_inline(char *, const char *, int *);

/*
 * This function is used to call a user-defined function. Noone should be
 * calling this directly except for call_function.
 */
...
/*
 * This function is sued to save all the current aliases to a global file.
 * This is used by /SAVE and /ABORT.
 */
extern void     save_aliases(FILE *, int);
extern void     save_assigns(FILE *, int);

/*
 * This function is in functions.c This function allows you to execute a
 * primitive "BUILT IN" expando. These are the $A, $B, $C, etc expandoes. The
 * argument is the character of the expando (eg, 'A', 'B', etc)
 *
...
/*
 * This function is used by parse_command to directly execute an alias. Noone
 * should be calling this function directly. (call parse_line.)
 */
...
/*
 * This is in functions.c This is only used by next_unit and expand_alias to
 * call built in functions. Noone should call this function directly.
 */
