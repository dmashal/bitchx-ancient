...
char   *BX_upper(char *);
char   *BX_lower(char *);
char   *BX_sindex(char *, char *);
char   *BX_rsindex(char *, char *, char *, int);
...
#if 0
#define my_stricmp(x, y) strcasecmp(x, y)       /* unable to use these for
						 * reasons of case sensitivity
						 * and finish */
...
char   *BX_strformat(char *, const char *, ssize_t, char);
char   *chop_word(char *);
...
char   *BX_MatchingBracket(char *, char, char);
...
#define SOS -32767
#define EOS 32767
char   *BX_strsearch(char *, char *, char *, int);
char   *BX_move_to_abs_word(const char *, char **, int);
