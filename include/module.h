/*
 * Module/dll handling code written by Colten Edwards. Copyright 1997
 */
...
/*
 * if we change the table below, we change this module number to the current
 * date.
 */
...
#define HOOK_PROC       0x0010  /* new numeric proc return int 1 if we don't
				 * want client to also handle this. or 0  if
				 * we want client to handle */
...
/*
 * the order of this enum is extremely important. ie a module compiled on a
 * differant order would fail miserably. So when adding to the list make sure
