...
/*
 * This puts a character to the current target, whatever it is. All output
 * everywhere should go through this. This does not mangle its output, so its
 * suitable for outputting escape sequences.
 */
...
struct term_struct {
	int TI_bw;              /* bw  / bw        cub1 wraps from column 0
				 * to last column */
	int TI_am;              /* am  / am        terminal has automatic
				 * margins */
	int TI_xsb;             /* xb  / xsb       beehive (f1=escape,
				 * f2=ctrl C) */
	int TI_xhp;             /* xs  / xhp       standout not erased by
				 * overwriting (hp) */
	int TI_xenl;            /* xn  / xenl      newline ignored after 80
				 * cols (concept) */
	int TI_eo;              /* eo  / eo        can erase overstrikes with
				 * a blank */
	int TI_gn;              /* gn  / gn        generic line type */
	int TI_hc;              /* hc  / hc        hardcopy terminal */
	int TI_km;              /* km  / km        Has a meta key (shift,
				 * sets parity bit) */
	int TI_hs;              /* hs  / hs        has extra status line */
	int TI_in;              /* in  / in        insert mode distinguishes
				 * nulls */
	int TI_da;              /* da  / da        display may be retained
				 * above the screen */
	int TI_db;              /* db  / db        display may be retained
				 * below the screen */
	int TI_mir;             /* mi  / mir       safe to move while in
				 * insert mode */
	int TI_msgr;            /* ms  / msgr      safe to move while in
				 * standout mode */
	int TI_os;              /* os  / os        terminal can overstrike */
	int TI_eslok;                   /* es  / eslok     escape can be used
					 * on the status line */
	int TI_xt;              /* xt  / xt        tabs destructive, magic so
				 * char (t1061) */
	int TI_hz;              /* hz  / hz        can't print ~'s
				 * (hazeltine) */
	int TI_ul;              /* ul  / ul        underline character
				 * overstrikes */
	int TI_xon;             /* xo  / xon       terminal uses xon/xoff
				 * handshaking */
	int TI_nxon;            /* nx  / nxon      padding won't work,
				 * xon/xoff required */
	int TI_mc5i;            /* 5i  / mc5i      printer won't echo on
				 * screen */
	int TI_chts;            /* HC  / chts      cursor is hard to see */
	int TI_nrrmc;                   /* NR  / nrrmc     smcup does not
					 * reverse rmcup */
	int TI_npc;             /* NP  / npc       pad character does not
				 * exist */
	int TI_ndscr;                   /* ND  / ndscr     scrolling region
					 * is non-destructive */
	int TI_ccc;             /* cc  / ccc       terminal can re-define
				 * existing colors */
	int TI_bce;             /* ut  / bce       screen erased with
				 * background color */
	int TI_hls;             /* hl  / hls       terminal uses only HLS
				 * color notation (Tektronix) */
	int TI_xhpa;            /* YA  / xhpa      only positive motion for
				 * hpa/mhpa caps */
	int TI_crxm;            /* YB  / crxm      using cr turns off micro
				 * mode */
	int TI_daisy;                   /* YC  / daisy     printer needs
					 * operator to change character set */
	int TI_xvpa;            /* YD  / xvpa      only positive motion for
				 * vpa/mvpa caps */
	int TI_sam;             /* YE  / sam       printing in last column
				 * causes cr */
	int TI_cpix;            /* YF  / cpix      changing character pitch
				 * changes resolution */
	int TI_lpix;            /* YG  / lpix      changing line pitch
				 * changes resolution */
	int TI_cols;            /* co  / cols      number of columns in a
				 * line */
	int TI_it;              /* it  / it        tabs initially every #
				 * spaces */
	int TI_lines;                   /* li  / lines     number of lines on
					 * screen or page */
	int TI_lm;              /* lm  / lm        lines of memory if > line.
				 * 0 means varies */
	int TI_xmc;             /* sg  / xmc       number of blank characters
				 * left by smso or rmso */
	int TI_pb;              /* pb  / pb        lowest baud rate where
				 * padding needed */
	int TI_vt;              /* vt  / vt        virtual terminal number
				 * (CB/unix) */
	int TI_wsl;             /* ws  / wsl       number of columns in
				 * status line */
	int TI_nlab;            /* Nl  / nlab      number of labels on screen */
	int TI_lh;              /* lh  / lh        rows in each label */
	int TI_lw;              /* lw  / lw        columns in each label */
	int TI_ma;              /* ma  / ma        maximum combined
				* attributes terminal can handle */
	int TI_wnum;            /* MW  / wnum      maximum number of
				 * defineable windows */
	int TI_colors;                  /* Co  / colors    maximum number of
					 * colors on screen */
	int TI_pairs;                   /* pa  / pairs     maximum number of
					 * color-pairs on the screen */
	int TI_ncv;             /* NC  / ncv       video attributes that
				 * can't be used with colors */
	int TI_bufsz;                   /* Ya  / bufsz     numbers of bytes
					 * buffered before printing */
	int TI_spinv;                   /* Yb  / spinv     spacing of pins
					 * vertically in pins per inch */
	int TI_spinh;                   /* Yc  / spinh     spacing of dots
					* horizontally in dots per inch */
	int TI_maddr;                   /* Yd  / maddr     maximum value in
					 * micro_..._address */
	int TI_mjump;                   /* Ye  / mjump     maximum value in
					 * parm_..._micro */
	int TI_mcs;             /* Yf  / mcs       character step size when
				 * in micro mode */
	int TI_mls;             /* Yg  / mls       line step size when in
				 * micro mode */
	int TI_npins;                   /* Yh  / npins     numbers of pins in
					 * print-head */
	int TI_orc;             /* Yi  / orc       horizontal resolution in
				 * units per line */
	int TI_orl;             /* Yj  / orl       vertical resolution in
				 * units per line */
	int TI_orhi;            /* Yk  / orhi      horizontal resolution in
				 * units per inch */
	int TI_orvi;            /* Yl  / orvi      vertical resolution in
				 * units per inch */
	int TI_cps;             /* Ym  / cps       print rate in characters
				 * per second */
	int TI_widcs;                   /* Yn  / widcs     character step
					 * size when in double wide mode */
	int TI_btns;            /* BT  / btns      number of buttons on mouse */
	int TI_bitwin;                  /* Yo  / bitwin    number of passes
					 * for each bit-image row */
	int TI_bitype;                  /* Yp  / bitype    type of bit-image
					 * device */
	char           *TI_cbt; /* bt  / cbt       back tab (P) */
	char           *TI_bel; /* bl  / bel       audible signal (bell) (P) */
	char           *TI_cr;  /* cr  / cr        carriage return (P*) (P*) */
	char           *TI_csr; /* cs  / csr       change region to line #1
				 * to line #2 (P) */
	char           *TI_tbc; /* ct  / tbc       clear all tab stops (P) */
	char           *TI_clear;       /* cl  / clear     clear screen and
					 * home cursor (P*) */
	char           *TI_el;  /* ce  / el        clear to end of line (P) */
	char           *TI_ed;  /* cd  / ed        clear to end of screen
				 * (P*) */
	char           *TI_hpa; /* ch  / hpa       horizontal position #1,
				 * absolute (P) */
	char           *TI_cmdch;       /* CC  / cmdch     terminal settable
					 * cmd character in prototype !? */
	char           *TI_cup; /* cm  / cup       move to row #1 columns #2 */
	char           *TI_cud1; /* do  / cud1      down one line */
	char           *TI_home; /* ho  / home      home cursor (if no cup) */
	char           *TI_civis;       /* vi  / civis     make cursor
					 * invisible */
	char           *TI_cub1; /* le  / cub1      move left one space */
	char           *TI_mrcup;       /* CM  / mrcup     memory relative
					 * cursor addressing */
	char           *TI_cnorm;       /* ve  / cnorm     make cursor appear
					 * normal (undo civis/cvvis) */
	char           *TI_cuf1; /* nd  / cuf1      non-destructive space
				 * (move right one space) */
	char           *TI_ll;  /* ll  / ll        last line, first column
				 * (if no cup) */
	char           *TI_cuu1; /* up  / cuu1      up one line */
	char           *TI_cvvis;       /* vs  / cvvis     make cursor very
					 * visible */
	...
	char           *TI_rmacs;       /* ae  / rmacs     end alternate
					 * character set (P) */
	char           *TI_sgr0; /* me  / sgr0      turn off all attributes */
	char           *TI_rmcup;       /* te  / rmcup     strings to end
					 * programs using cup */
	...
	char           *TI_flash;       /* vb  / flash     visible bell (may
					 * not move cursor) */
	char           *TI_ff;  /* ff  / ff        hardcopy terminal page
				 * eject (P*) */
	...
	char           *TI_if;  /* if  / if        name of initialization
				 * file */
	char           *TI_ich1; /* ic  / ich1      insert character (P) */
	char           *TI_il1; /* al  / il1       insert line (P*) */
	char           *TI_ip;  /* ip  / ip        insert padding after
				 * inserted character */
	...
	char           *TI_kdch1;       /* kD  / kdch1     delete-character
					 * key */
	char           *TI_kdl1; /* kL  / kdl1      delete-line key */
	char           *TI_kcud1;       /* kd  / kcud1     down-arrow key */
	char           *TI_krmir;       /* kM  / krmir     sent by rmir or
					 * smir in insert mode */
	...
	char           *TI_kich1;       /* kI  / kich1     insert-character
					 * key */
	...
	char           *TI_rmkx; /* ke  / rmkx      leave 'keyboard_transmit'
				 * mode */
	char           *TI_smkx; /* ks  / smkx      enter 'keyboard_transmit'
				 * mode */
	char           *TI_lf0; /* l0  / lf0       label on function key f0
				 * if not f0 */
	char           *TI_lf1; /* l1  / lf1       label on function key f1
				 * if not f1 */
	char           *TI_lf10; /* la  / lf10      label on function key f10
				 * if not f10 */
	char           *TI_lf2; /* l2  / lf2       label on function key f2
				 * if not f2 */
	char           *TI_lf3; /* l3  / lf3       label on function key f3
				 * if not f3 */
	char           *TI_lf4; /* l4  / lf4       label on function key f4
				 * if not f4 */
	char           *TI_lf5; /* l5  / lf5       label on function key f5
				 * if not f5 */
	char           *TI_lf6; /* l6  / lf6       label on function key f6
				 * if not f6 */
	char           *TI_lf7; /* l7  / lf7       label on function key f7
				 * if not f7 */
	char           *TI_lf8; /* l8  / lf8       label on function key f8
				 * if not f8 */
	char           *TI_lf9; /* l9  / lf9       label on function key f9
				 * if not f9 */
	char           *TI_rmm; /* mo  / rmm       turn off meta mode */
	char           *TI_smm; /* mm  / smm       turn on meta mode (8th-bit
				 * on) */
	char           *TI_nel; /* nw  / nel       newline (behave like cr
				 * followed by lf) */
	char           *TI_pad; /* pc  / pad       padding char (instead of
				 * null) */
	...
	char           *TI_indn; /* SF  / indn      scroll forward #1 lines
				 * (P) */
	char           *TI_il;  /* AL  / il        insert #1 lines (P*) */
	char           *TI_cub; /* LE  / cub       move #1 characters to the
				 * left (P) */
	char           *TI_cuf; /* RI  / cuf       move #1 characters to the
				 * right (P*) */
	char           *TI_rin; /* SR  / rin       scroll back #1 lines (P) */
	char           *TI_cuu; /* UP  / cuu       up #1 lines (P*) */
	char           *TI_pfkey;       /* pk  / pfkey     program function
					 * key #1 to type string #2 */
	char           *TI_pfloc;       /* pl  / pfloc     program function
					 * key #1 to execute string #2 */
	char           *TI_pfx; /* px  / pfx       program function key #1 to
				 * transmit string #2 */
	char           *TI_mc0; /* ps  / mc0       print contents of screen */
	char           *TI_mc4; /* pf  / mc4       turn off printer */
	char           *TI_mc5; /* po  / mc5       turn on printer */
	char           *TI_rep; /* rp  / rep       repeat char #1 #2 times
				 * (P*) */
	...
	char           *TI_rc;  /* rc  / rc        restore cursor to position
				 * of last save_cursor */
	char           *TI_vpa; /* cv  / vpa       vertical position #1
				 * absolute (P) */
	char           *TI_sc;  /* sc  / sc        save current cursor
				 * position (P) */
	char           *TI_ind; /* sf  / ind       scroll text up (P) */
	char           *TI_ri;  /* sr  / ri        scroll text down (P) */
	char           *TI_sgr; /* sa  / sgr       define video attributes
				 * #1-#9 (PG9) */
	char           *TI_hts; /* st  / hts       set a tab in every row,
				 * current columns */
	char           *TI_wind; /* wi  / wind      current window is lines
				 * #1-#2 cols #3-#4 */
	char           *TI_ht;  /* ta  / ht        tab to next 8-space
				 * hardware tab stop */
	char           *TI_tsl; /* ts  / tsl       move to status line */
	char           *TI_uc;  /* uc  / uc        underline char and move
				 * past it */
	char           *TI_hu;  /* hu  / hu        half a line up */
	char           *TI_iprog;       /* iP  / iprog     path name of
	...
	char           *TI_mc5p; /* pO  / mc5p      turn on printer for #1
				 * bytes */
	char           *TI_rmp; /* rP  / rmp       like ip but when in insert
				 * mode */
	char           *TI_acsc; /* ac  / acsc      graphics charset pairs,
				 * based on vt100 */
	char           *TI_pln; /* pn  / pln       program label #1 to show
				 * string #2 */
	char           *TI_kcbt; /* kB  / kcbt      back-tab key */
	char           *TI_smxon;       /* SX  / smxon     turn on xon/xoff
					 * handshaking */
	char           *TI_rmxon;       /* RX  / rmxon     turn off xon/xoff
					 * handshaking */
	...
	char           *TI_enacs;       /* eA  / enacs     enable alternate
					 * char set */
	...
	char           *TI_kIC; /* #3  / kIC       shifted insert-character
				 * key */
	...
	char           *TI_rfi; /* RF  / rfi       send next input char (for
				 * ptys) */
	...
	char           *TI_mgc; /* MC  / mgc       clear right and left soft
				 * margins */
	char           *TI_smgl; /* ML  / smgl      set left soft margin at
				 * current column */
	char           *TI_smgr; /* MR  / smgr      set right soft margin at
				 * current column */
	char           *TI_fln; /* Lf  / fln       label format */
	char           *TI_sclk; /* SC  / sclk      set clock, #1 hrs #2 mins
				 * #3 secs */
	char           *TI_dclk; /* DK  / dclk      display clock at (#1,#2) */
	char           *TI_rmclk;       /* RC  / rmclk     remove clock */
	char           *TI_cwin; /* CW  / cwin      define a window #1 from
				 * #2,#3 to #4,#5 */
	char           *TI_wingo;       /* WG  / wingo     go to window #1 */
	char           *TI_hup; /* HU  / hup       hang-up phone */
	char           *TI_dial; /* DI  / dial      dial number #1 */
	char           *TI_qdial;       /* QD  / qdial     dial number #1
					 * without checking */
	char           *TI_tone; /* TO  / tone      select touch tone dialing */
	char           *TI_pulse;       /* PU  / pulse     select pulse
					 * dialing */
	char           *TI_hook; /* fh  / hook      flash switch hook */
	char           *TI_pause;       /* PA  / pause     pause for 2-3
					 * seconds */
	...
	char           *TI_op;  /* op  / op        Set default pair to its
				 * original value */
	char           *TI_oc;  /* oc  / oc        Set all color pairs to the
				 * original ones */
	char           *TI_initc;       /* Ic  / initc     initialize color
					 * #1 to (#2,#3,#4) */
	char           *TI_initp;       /* Ip  / initp     Initialize color
					 * pair #1 to fg=(#2,#3,#4),
					 * bg=(#5,#6,#7) */
	char           *TI_scp; /* sp  / scp       Set current color pair to
				 * #1 */
	char           *TI_setf; /* Sf  / setf      Set foreground color #1 */
	char           *TI_setb; /* Sb  / setb      Set background color #1 */
	char           *TI_cpi; /* ZA  / cpi       Change number of
				 * characters per inch */
	char           *TI_lpi; /* ZB  / lpi       Change number of lines per
				 * inch */
	char           *TI_chr; /* ZC  / chr       Change horizontal
				 * resolution */
	char           *TI_cvr; /* ZD  / cvr       Change vertical resolution */
	char           *TI_defc; /* ZE  / defc      Define a character */
	char           *TI_swidm;       /* ZF  / swidm     Enter double-wide
					 * mode */
	char           *TI_sdrfq;       /* ZG  / sdrfq     Enter
					 * draft-quality mode */
	char           *TI_sitm; /* ZH  / sitm      Enter italic mode */
	char           *TI_slm; /* ZI  / slm       Start leftward carriage
				 * motion */
	char           *TI_smicm;       /* ZJ  / smicm     Start micro-motion
					 * mode */
	char           *TI_snlq; /* ZK  / snlq      Enter NLQ mode */
	char           *TI_snrmq;       /* ZL  / snrmq     Enter
					* normal-quality mode */
	char           *TI_sshm; /* ZM  / sshm      Enter shadow-print mode */
	char           *TI_ssubm;       /* ZN  / ssubm     Enter subscript
					 * mode */
	char           *TI_ssupm;       /* ZO  / ssupm     Enter superscript
					 * mode */
	char           *TI_sum; /* ZP  / sum       Start upward carriage
				 * motion */
	char           *TI_rwidm;       /* ZQ  / rwidm     End double-wide
					 * mode */
	char           *TI_ritm; /* ZR  / ritm      End italic mode */
	char           *TI_rlm; /* ZS  / rlm       End left-motion mode */
	char           *TI_rmicm;       /* ZT  / rmicm     End micro-motion
					 * mode */
	char           *TI_rshm; /* ZU  / rshm      End shadow-print mode */
	char           *TI_rsubm;       /* ZV  / rsubm     End subscript mode */
	char           *TI_rsupm;       /* ZW  / rsupm     End superscript
					 * mode */
	char           *TI_rum; /* ZX  / rum       End reverse character
				 * motion */
	char           *TI_mhpa; /* ZY  / mhpa      Like column_address in
				 * micro mode */
	char           *TI_mcud1;       /* ZZ  / mcud1     Like cursor_down
					 * in micro mode */
	char           *TI_mcub1;       /* Za  / mcub1     Like cursor_left
					 * in micro mode */
	char           *TI_mcuf1;       /* Zb  / mcuf1     Like cursor_right
					 * in micro mode */
	char           *TI_mvpa; /* Zc  / mvpa      Like row_address in micro
				 * mode */
	char           *TI_mcuu1;       /* Zd  / mcuu1     Like cursor_up in
					 * micro mode */
	char           *TI_porder;      /* Ze  / porder    Match software
					 * bits to print-head pins */
	char           *TI_mcud; /* Zf  / mcud      Like parm_down_cursor in
				 * micro mode */
	char           *TI_mcub; /* Zg  / mcub      Like parm_left_cursor in
				 * micro mode */
	char           *TI_mcuf; /* Zh  / mcuf      Like parm_right_cursor in
				 * micro mode */
	char           *TI_mcuu; /* Zi  / mcuu      Like parm_up_cursor in
				 * micro mode */
	char           *TI_scs; /* Zj  / scs       Select character set */
	char           *TI_smgb; /* Zk  / smgb      Set bottom margin at
				 * current line */
	char           *TI_smgbp;       /* Zl  / smgbp     Set bottom margin
					 * at line #1 or #2 lines from bottom */
	char           *TI_smglp;       /* Zm  / smglp     Set left (right)
					 * margin at column #1 (#2) */
	char           *TI_smgrp;       /* Zn  / smgrp     Set right margin
					 * at column #1 */
	char           *TI_smgt; /* Zo  / smgt      Set top margin at current
				 * line */
	char           *TI_smgtp;       /* Zp  / smgtp     Set top (bottom)
					 * margin at row #1 (#2) */
	char           *TI_sbim; /* Zq  / sbim      Start printing bit image
				 * graphics */
	char           *TI_scsd; /* Zr  / scsd      Start character set
				 * definition */
	char           *TI_rbim; /* Zs  / rbim      Stop printing bit image
				 * graphics */
	char           *TI_rcsd; /* Zt  / rcsd      End definition of
				 * character set */
	char           *TI_subcs;       /* Zu  / subcs     List of
					 * subscriptable characters */
	char           *TI_supcs;       /* Zv  / supcs     List of
					 * superscriptable characters */
	char           *TI_docr; /* Zw  / docr      Printing any of these
				 * characters causes CR */
	char           *TI_zerom;       /* Zx  / zerom     No motion for
					 * subsequent character */
	char           *TI_csnm; /* Zy  / csnm      List of character set
				 * names */
	char           *TI_kmous;       /* Km  / kmous     Mouse event has
					 * occurred */
	char           *TI_minfo;       /* Mi  / minfo     Mouse status
					 * information */
	char           *TI_reqmp;       /* RQ  / reqmp     Request mouse
					 * position */
	char           *TI_getm; /* Gm  / getm      Curses should get button
				 * events */
	char           *TI_setaf;       /* AF  / setaf     Set foreground
					 * color using ANSI escape */
	char           *TI_setab;       /* AB  / setab     Set background
					 * color using ANSI escape */
	char           *TI_pfxl; /* xl  / pfxl      Program function key #1 to
				 * type string #2 and show string #3 */
	char           *TI_devt; /* dv  / devt      Indicate language/codeset
				 * support */
	char           *TI_csin; /* ci  / csin      Init sequence for multiple
				 * codesets */
	char           *TI_s0ds; /* s0  / s0ds      Shift to code set 0 (EUC
				 * set 0, ASCII) */
	char           *TI_s1ds; /* s1  / s1ds      Shift to code set 1 */
	char           *TI_s2ds; /* s2  / s2ds      Shift to code set 2 */
	char           *TI_s3ds; /* s3  / s3ds      Shift to code set 3 */
	char           *TI_smglr;       /* ML  / smglr     Set both left and
					 * right margins to #1, #2 */
	char           *TI_smgtb;       /* MT  / smgtb     Sets both top and
					 * bottom margins to #1, #2 */
	char           *TI_birep;       /* Xy  / birep     Repeat bit image
					 * cell #1 #2 times */
	char           *TI_binel;       /* Zz  / binel     Move to next row
					 * of the bit image */
	char           *TI_bicr; /* Yv  / bicr      Move to beginning of same
				 * row */
	char           *TI_colornm;     /* Yw  / colornm   Give name for
					 * color #1 */
	char           *TI_defbi;       /* Yx  / defbi     Define
					 * rectangualar bit image region */
	char           *TI_endbi;       /* Yy  / endbi     End a bit-image
					 * region */
	char           *TI_setcolor;    /* Yz  / setcolor  Change to ribbon
					 * color #1 */
	char           *TI_slines;      /* YZ  / slines    Set page length to
					 * #1 lines */
	char           *TI_dispc;       /* S1  / dispc     Display PC
					 * character */
	char           *TI_smpch;       /* S2  / smpch     Enter PC character
					 * display mode */
	char           *TI_rmpch;       /* S3  / rmpch     Exit PC character
					 * display mode */
	char           *TI_smsc; /* S4  / smsc      Enter PC scancode mode */
	char           *TI_rmsc; /* S5  / rmsc      Exit PC scancode mode */
	char           *TI_pctrm;       /* S6  / pctrm     PC terminal
					 * options */
	char           *TI_scesc;       /* S7  / scesc     Escape for
					 * scancode emulation */
	char           *TI_scesa;       /* S8  / scesa     Alternate escape
					 * for scancode emulation */
	char           *TI_ehhlm;       /* Xh  / ehhlm     Enter horizontal
					 * highlight mode */
	char           *TI_elhlm;       /* Xl  / elhlm     Enter left
					 * highlight mode */
	char           *TI_elohlm;      /* Xo  / elohlm    Enter low
					 * highlight mode */
	char           *TI_erhlm;       /* Xr  / erhlm     Enter right
					 * highlight mode */
	char           *TI_ethlm;       /* Xt  / ethlm     Enter top
					 * highlight mode */
	char           *TI_evhlm;       /* Xv  / evhlm     Enter vertical
					 * highlight mode */
	char           *TI_sgr1; /* sA  / sgr1      Define second set of video
				 * attributes #1-#6 */
	char           *TI_slength;     /* sL  / slength   YI Set page length
					 * to #1 hundredth of an inch */
	char           *TI_OTi2; /* i2  / OTi2      secondary initialization
				 * string */
	char           *TI_OTrs; /* rs  / OTrs      terminal reset string */
	int TI_OTug;            /* ug  / OTug      number of blanks left by
				 * ul */
	int TI_OTbs;            /* bs  / OTbs      uses ^H to move left */
	int TI_OTns;            /* ns  / OTns      crt cannot scroll */
	int TI_OTnc;            /* nc  / OTnc      no way to go to start of
				 * line */
	...
	char           *TI_OTko; /* ko  / OTko      list of self-mapped
				 * keycaps */
	char           *TI_OTma; /* ma  / OTma      map arrow keys rogue(1)
				 * motion keys */
	int TI_OTpt;            /* pt  / OTpt      has 8-char tabs invoked
				 * with ^I */
