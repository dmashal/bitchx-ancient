/*
 * 'new' config.h: A configuration file designed to make best use of the
 * abilities of ircII, and trying to make things more intuitively
 * understandable.
 *
 * Done by Carl v. Loesch <lynx@dm.unirm1.it> Based on the 'classic' config.h by
 * Michael Sandrof. Copyright(c) 1991 - See the COPYRIGHT file, or do a HELP
 * IRCII COPYRIGHT
 *
...
/*
 * some caution is required here. the \ is a continuation char and is
 * required on any servers you add into this list. also the very last server
 * should not have a continuation char.
 *
...
/*
 * On some channels mass modes can be confusing and in some case spectacular
 * so in the interest of keeping sanity, Jordy added this mode compressor to
 * the client. It reduces the duplicate modes that might occur on a channel..
 * it's explained in names.c much better.
 */
...
/*
 * Uncomment the following to make ircII read a list of irc servers from the
 * ircII.servers file in the ircII library. This file should be whitespace
 * separated hostname:portnum*password (with the portnum and password being
 ...
 * tcllib. This offers some benefit at the expense of a slightly larger
 * binary.
 */
#undef TCL_PLUS

/*
 * we define the default network type for server groups. Do not just undefine
 * this.
 */
...
/*
 * Define this if you want the $glob() function to be in your client. There
 * is a case for having this functino and a case against having this
 * function:
 *
 * Pro: makes it easier to write scripts like xdcc, since they can easily get at
 * the filenames in your xdcc directory ConS8 with $unlink(), $rmdir(), etc,
 * it makes it that much easier for a backdoor to do damage to your account.
 *
...
/*
 * crisk graciously allowed me to include his hebrew modification to ircII in
 * the client. defining this variable to 1 allows that happen. It also adds a
 * HEBREW_TOGGLE variable which can turn this feature on/off
 */
#undef WANT_HEBREW

/*
 * if you use cidentd the filename is called .authlie instead of .noident. as
 * well some modifications to the format of the file were made. So we require
 * some pre-knowledge of what to expect. WinNT identd servers will also
 * require this.
 */
...
/*
 * I moved this here because it seemed to be the most appropriate place for
 * it.  Define this if you want support for ``/window create'' and its
 * related features.  If you dont want it, youll save some code, and you wont
 * need 'wserv', and if you do want it, you can have it in all of its broken
 * glory (no, i dont have plans to fix it =) window create doesn't make any
 * sense on Windows 95/NT.
 */
...
/*
 * Define this if you want an mIRC compatable /dcc resume capability. Note
 * that this BREAKS THE IRC PROTOCOL, and if you use this feature, the
 * behavior is NON COMPLIANT.  If this warning doesnt bother you, and you
 * really want this feature, then go ahead and #define this.
 */
...
/*
 * If you define UNAME_HACK, the uname information displayed in the CTCP
 * VERSION info will appear as "*IX" irregardless of any other settings.
 * Useful for paranoid users who dont want others to know that theyre running
 * a buggy SunOS machine. >;-)
 */
#undef UNAME_HACK

/*
 * If you define ONLY_STD_CHARS, only "normal" characters will displayed.
 * This is recommended when you want to start BitchX in an xterm without the
 * usage of the special "vga"-font.
 */
#undef ONLY_STD_CHARS

/*
 * Normally BitchX uses only the IBMPC (cp437) charset. Define LATIN1, if you
 * want to see the standard Latin1 characters (i.e. Ä Ö Ü ä ö ü ß <-> "A "O
 * "U "a "o "u \qs ).
 *
 * You will still be able to see ansi graphics, but there will be some smaller
 * problems (i.e. after a PageUp).
 *
 * If you use xterm there is no easy way to use both fonts at the same time.
 * You have to decide if you use the standard (latin1) fonts or vga.pcf
 * (cp437).
 *
...
/*
 * If you use LINUX and non ISO8859-1 fonts with custom screen mapping, and
 * if your see some pseudographics instead of your national characters,
 ...
 *
 * Note: On the console ansi graphics can be displayed just fine, even when
 * you've defined LATIN1. The problem is that ansi graphics will look ugly if
 * you use the scroll up feature (PageUp/PageDown)
 */
#undef ASCII_LOGO
 ...
 * white and white). This way the default format-strings are readable on a
 * display with white background and black foreground.
 */
...
/*
 * WinNT and EMX probably need an ident server, so define this unless you
 * have an external one, or don't want ident support (bad idea).
 */
#if defined(WINNT) || defined(__EMX__)
#define WANT_IDENTD
#endif
...
#define DEFAULT_MAX_URLS 50     /* this defines the MAX number of urls saved */
#undef BITCHX_LITE
#undef EMACS_KEYBINDS           /* change this is you have problems with your
				 * keyboard */
#define EXEC_COMMAND
#undef PUBLIC_ACCESS            /* this define removes /load /exec commands */
#define DEFAULT_OPERVIEW_HIDE 0 /* defines the operview window. if hidden or
				 * not */
...
#define DEFAULT_KICK_REASON "cool story bro"
#define DEFAULT_PROTECT_CHANNELS "*"
#define DEFAULT_SHITLIST_REASON "Excessive shitheads must go"
...
/*
 * #define CLOAKED  "emacs" *//* define this to the program you want to show
 * up in "ps" and "top" to hide irc from evil sys-admins.
 */
...
/*
 * on certain systems we can define NON_BLOCKING to 1 connects are then done
 * alot differantly. We can perform actual work in the background, while
 * connecting. This also protects us from certain "bombs" that are available.
 * If you have trouble with this undef the NON_BLOCKING_CONNECTS. DCC
 * sends/gets are much improved with this.
 */
...
#undef PARANOID                 /* #define this if your paranoid about dcc
				 * hijacking */
#undef WANT_CHAN_NICK_SERV      /* do we want to include some chan/nick/oper
				 * server commands */
