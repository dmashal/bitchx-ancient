 ...
 *
 * Written by Scott Reynolds, based on code by Michael Sandrof Heavily modified
 * by Colten Edwards for BitchX
 *
 * Copyright(c) 1997
 *
 */
...
typedef struct {
	...
	char           *server_func;    /* what gets sent to the server (if
					 * anything) */
	void (*func)(char *, char *, char *, char *);                   /* function that is the
									 * command */
	unsigned flags;
	char           *help;
}               IrcCommand;
...
typedef struct _IrcCommandDll {
	...
	char           *server_func;    /* what gets sent to the server (if
					 * anything) */
	void (*func)(struct _IrcCommandDll *, char *, char *, char *, char *);                  /* function that is the
												 * command */
	unsigned flags;
	char           *result;
	char           *help;
...
typedef struct nick_stru {
	...
	my_uint need_userhost;          /* on join we send a userhost for
					 * this nick */
...
typedef struct WinSetStru {
	...
	/*
	 * These are the various formats from a window make_status() creates
	 * these
	 */
	char           *mode_format;
	char           *umode_format;
...
#define MAX_FUNCTIONS           36
	...
	char           *status_line[3]; /* The status lines string current
					 * display */
	char           *status_format[4];       /* holds formated status info
						 * from build_status */
	char           *format_status[4];       /* holds raw format for
						 * window from /set */
...
typedef struct WindowStru {
	char           *name;
	unsigned int refnum;    /* the unique reference number, assigned by
				 * IRCII */
	...
	int line_cnt;                   /* counter of number of lines
					 * displayed in window */
	int absolute_size;
	int noscroll;                   /* true, window scrolls... false
					 * window wraps */
	int scratch_line;               /* True if a scratch window */
	int old_size;                   /* if new_size != display_size,
					 * resize_display */
	int visible;            /* true, window ise, window is drawn... false
				 * window is hidden */
	int update;             /* window needs updating flag */
	int repaint_start;
	int repaint_end;
	unsigned miscflags;             /* Miscellaneous flags. */
	int beep_always;                /* should this window beep when
					 * hidden */
	unsigned long notify_level;
	unsigned long window_level;     /* The LEVEL of the window,
					 * determines what messages go to it */
	int skip;
	int columns;
	char           *prompt; /* A prompt string, usually set by EXEC'd
				 * process */
	...
	Display        *top_of_scrollback,      /* Start of the scrollback
						 * buffer */
	*top_of_display,                /* Where the viewport starts */
	*ceiling_of_display,                    /* the furthest top of
						 * display */
	*display_ip,                    /* Where next line goes in rite() */
	*scrollback_point, *screen_hold;                /* Where t_o_d was at
							 * start of sb */
	int display_buffer_size;                /* How big the scrollback
						 * buffer is */
	...
	int holding_something;                  /* True if we ARE holding
						 * something */
	int held_displayed;             /* lines displayed since last hold */
	int lines_displayed;                    /* Lines held since last
						 * unhold */
	int lines_held;                 /* Lines currently being held */
	int last_lines_held;                    /* Last time we updated
						 * "lines held" */
	...
	char           *bind_channel;
	char           *query_nick;     /* User being QUERY'ied in this
					 * window */
	char           *query_host;
	char           *query_cmd;
...
typedef struct ScreenStru {
	...
	HVPS hvps, hvpsnick;
#endif
	HWND hwndFrame, hwndClient, hwndMenu, hwndLeft, hwndRight, hwndnickscroll, hwndscroll;
	int VIO_font_width, VIO_font_height, spos, mpos, codepage;
	char aviokbdbuffer[256];
#elif defined(GTK)
	GtkWidget      *window, *viewport, *menubar, *scroller, *clist, *scrolledwindow, *box;
	GdkFont        *font;
	char           *fontname;
...
typedef struct channel_stru {
	...
	char           *chanpass;       /* if TS4 then this has the channel
					 * pass */
	char connected;                 /* true if this channel is actually
					 * connected */
	...
	int stats_unbans;               /* total unbans I have seen in
					 * channel */

	int stats_sops;                 /* total server ops I have seen in
					 * channel */
	int stats_sdops;                /* total server dops I have seen in
					 * channel */
	int stats_shops;
	int stats_sdehops;
	int stats_sebans;
	int stats_sunebans;
	int stats_sbans;                /* total server bans I have seen in
					 * channel */
	int stats_sunbans;              /* total server unbans I have seen in
					 * channel */

	int stats_topics;               /* total topics I have seen in
					 * channel */
	...
	int stats_signoffs;             /* total signoffs I have seen in
					 * channel */
...
typedef struct {
	...
	void (*func)(Window *, char *, int);                    /* function to do every
...
typedef struct command_struct {
	...
	IrcVariable    *fsets;
	/*
	 * CSetArray		*csets; WSetArray		*wsets;
	 */
	HashEntry      *parse;
}               CommandStruct;
...
typedef struct _dcc_internal {
	...
	struct transfer_struct transfer_orders; /* structure for resending
						 * files */
	...
	int dccnum;     /* dcc number we are at */
