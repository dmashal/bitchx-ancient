...
/*
 * this is a method first used in eggdrop modules.. A global table of
 * functions is passed into the init function of module, which is then
 * assigned to the value global. This table is then indexed, to access the
 * various functions. What this means to us, is that we no longer require
 * -rdynamic on the LDFLAGS line, which reduces the size of the client. This
 * also makes this less compiler/environment dependant, allowing modules to
 * work on more platforms. A Function_ptr *global is required in the module.
 * The second arg to the init function is used to initialize this table. The
 * table itself is initialized in modules.c. This file should only be
 * included once in the module and also should be the last file included. It
 * should never be included in the source itself. As long as we add new
 * functions to the END of the list in module.h then currently compiled
 * modules will continue to function fine. If we change the order of the list
 * however, then BAD things will occur. Copyright Colten Edwards July 1998.
 */
...
/*
 * If we are building wserv or scr-bx we can't use the global table so we
 * forward to the actual functions instead of throught the global table.
 */
...
/*
 * need to undefine these particular defines. Otherwise we can't include them
 * in the table.
 */
#undef new_malloc
#undef new_free
 ...
 * result.  The old method wasn't portable to some 64 bit platforms.  Next
 * step - change instances of global[XYZ] to &xyzfunc in the code.
 *
...
#define chop (*(char * (*)(char *, size_t))global[CHOP])
...
#define strformat (*(char *(*)(char *, const char *, ssize_t, char ))global[STRFORMAT])
...
#define add_to_screen (*(void (*)(const unsigned char *))global[ADD_TO_SCREEN])
...
/*
 * Previous broken definitions - yet it still seemed to work? #define
 * cursor_not_in_display(x) ((void) (global[CURSOR_IN_DISPLAY]((Screen *)x)))
 * #define cursor_in_display(x) ((void) (global[CURSOR_IN_DISPLAY]((Screen
 * *)x)))
 */
...
#define normalize_string (*(u_char *(*)(const u_char *))global[NORMALIZE_STRING])
