...
void
CBStartPaste(HWND HWin, BOOL AddCR,
...
LRESULT CALLBACK
AfxWndProc(HWND handle, UINT mess, WPARAM parm1, LPARAM parm2)
{
	PAINTSTRUCT ps;
	HDC PaintDC;
	int Xs, Ys, Xe, Ye;
...
//	Xs:	horizontal position in window
coordinate(pixels)
// Ys:vertical
// Output
// Xs, Ys:screen coordinate
// Right:TRUE if the(Xs, Ys)
is on the right half of
//   a character cell.
{
	if (Xs != NULL)
...
void SetLogFont()
{
	...
	VTlf.lfStrikeOut = 0;
	/*
	 * VTlf.lfWidth = ts.VTFontSize.x; VTlf.lfHeight = ts.VTFontSize.y;
...
ChangeFont()
{
	...
	/*
	 * FontWidth = Metrics.tmAveCharWidth + ts.FontDW; FontHeight =
	 * Metrics.tmHeight + ts.FontDH;
	 */
...
void
DispChangeWinSize(int Nx, int Ny)
{
	LONG W, H, dW, dH;
	RECT R;
...
void
ResizeWindow(int x, int y, int w, int h, int cw, int ch)
{
	int dw, dh, NewX, NewY;
	POINT Point;
...
void
PaintWindow(HDC PaintDC, RECT PaintRect, BOOL fBkGnd,
...
DispClearWin()
{
	...
	if (WinHeight > NumOfLines)
		 ...
		 * SetScrollRange(HVTWin,SB_VERT,0,1,FALSE); } else
...
void
DispSetupDC(BYTE Attr, BYTE Attr2, BOOL Reverse)
// Setup device context
// Attr, Attr2:character attribute 1 & 2
// Reverse:true if text is selected(reversed)
by mouse
{
	...
	/*
	 * else if ((Attr & AttrBlink) != 0) BackColor =
	 * RGB(127,127,127); TextColor = ts.VTBlinkColor[0];
	 */
	...
	/*
	 * else if ((Attr & AttrBold) != 0) BackColor =
	 * ts.VTBoldColor[1];
	 */
	else
		BackColor = RGB(0, 0, 0);
...
void
DispStr(PCHAR Buff, int Count, int Y, int *X)
//Display a string
...
//*X:horizontal position shifted by the width of the string
{
	...
	/*
	 * ExtTextOut(VTDC,*X+ts.FontDX,Y+ts.FontDY, ETO_CLIPPED |
	 * ETO_OPAQUE, &RText,Buff,Count,&Dx[0]);
	 */
	*X = RText.right;
}

void
DispEraseCurToEnd(int YEnd)
{
	RECT R;
...
void
DispEraseCharsInLine(int XStart, int Count)
{
	RECT R;
...
BOOL
DispDeleteLines(int Count, int YEnd)
//return value:
...
BOOL
DispInsertLines(int Count, int YEnd)
//return value:
//TRUE - screen is successfully updated
// FALSE - screen is not updated
{
...
void
DispScrollToCursor(int CurX, int CurY)
{
	if (CurX < NewOrgX)
		NewOrgX = CurX;
...
void
DispScrollNLines(int Top, int Bottom, int Direction)
...
DispUpdateScroll()
{
	...
	if (dScroll != 0) {
		...
		/*
		 * if ((SRegionTop==0) && (dScroll>0)) { // update scroll
		 * bar if BuffEnd is changed if ((BuffEnd==WinHeight) &&
		 * (ts.EnableScrollBuff>0))
		 * SetScrollRange(HVTWin,SB_VERT,0,1,TRUE); else
...
// reset window
{
	...
	/*
	 * (if (! LoadTTDLG()) return; SetLogFont(); Ok =
	 * ChooseFontDlg(HVTWin,&VTlf,&ts); FreeTTDLG();
	 */
	if (!Ok)
		return;

	/*
	 * strcpy(ts.VTFont,VTlf.lfFaceName); ts.VTFontSize.x =
	 * VTlf.lfWidth; ts.VTFontSize.y = VTlf.lfHeight; ts.VTFontCharSet =
	 * VTlf.lfCharSet;
	 */
...
ChangeBuffer(int Nx, int Ny)
{
	...
	if (Nx > BuffXMax)
		Nx = BuffXMax;
	/*
	 * if (ts.ScrollBuffMax > BuffYMax) ts.ScrollBuffMax = BuffYMax; if
	 * (Ny > ts.ScrollBuffMax) Ny = ts.ScrollBuffMax;
	 */
...
void
ResetTerminal()
{                                       /* reset variables but don't update
					 * screen */
	DispReset();
	BuffReset();
...
void
BuffScroll(int Count, int Bottom)
{
	int i, n;
	LONG SrcPtr, DestPtr;
	int BuffEndOld;
...
void
BuffInsertLines(int Count, int YEnd)
...
void
BuffEraseCharsInLine(int XStart, int Count)
...
void
BuffDeleteLines(int Count, int YEnd)
...
void
BuffDrawLine(BYTE Attr, BYTE Attr2, int Direction, int C)
{
	//IO - 8256 terminal
	LONG Ptr;
	int i, X, Y;
...
int
LeftHalfOfDBCS(LONG Line, int CharPtr)
...
//  dx:    moving distance in character
unit(-: left, +: right)
...
void
BuffPutChar(BYTE b, BYTE Attr, BYTE Attr2, BOOL Insert)
...
//	XStart:	x	position of the upper - left
corner(screen cordinate)
// YStart:y position
...
// check if a
character is the word delimiter
BOOL IsDelimiter(LONG Line, int CharPtr){
	return 0;
}

void
GetMinMax(int i1, int i2, int i3,
          int *min, int *max)
{
	if (i1 < i2) {
		*min = i1;
...
void
BuffDblClk(int Xw, int Yw)
//Select a word at(Xw, Yw)
by mouse double click
...
void
BuffStartSelect(int Xw, int Yw, BOOL Box)
...
// Box:Box selection if TRUE
{
	...
	TmpPtr = GetLinePtr(SelectStart.y);
	//check if the
	cursor is on the right half of a character
	if (((SelectStart.x > 0) &&
...
void
BuffChangeSelect(int Xw, int Yw, int NClick)
		...
		if ((SelectEnd.y > SelectStart.y) ||
			...
			if (IsDelimiter(TmpPtr, X)) {
				...
				while (((i != 0) &&
					...
					i = MoveCharPtr(TmpPtr, (int *)&SelectEnd.x, 1);
				//move right
			} else {
				while ((i != 0) &&
...
void
BuffChangeWinSize(int Nx, int Ny)
//Change window size
...
void
BuffChangeTerminalSize(int Nx, int Ny)
{
	int i, Nb, W, H;
	BOOL St;
	...
	if ((Nx != NumOfColumns) || (Ny != NumOfLines)) {
		...
		NumOfLines = Ny;
		/*
		 * ts.TerminalWidth = Nx; ts.TerminalHeight = Ny-StatusLine;
		 */
...
winbx_init(void)
{
	 ...
	 * MAKEINTRESOURCE(IDI_VT), IMAGE_ICON,16,16,0));
	 */
...
void
gui_new_window(Screen * new, Window * win)
{
}
...
void
gui_settitle(char *titletext, Screen * screen)
{
...
void
gui_resize(Screen * this_screen)
{
	if (this_screen->co < 20)
		this_screen->old_co = this_screen->co = 20;
	if (this_screen->li < 10)
		this_screen->old_li = this_screen->li = 10;
	if (this_screen->co > 199)
		this_screen->old_co = this_screen->co = 199;
	if (this_screen->li > 99)
		this_screen->old_li = this_screen->li = 99;
	/*
	 * cx = this_screen->co * this_screen->VIO_font_width; cy =
	 * this_screen->li * this_screen->VIO_font_height;
	 */
