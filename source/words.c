 ...
 */
extern char *BX_strsearch(char *start, char *mark, char *chars, int how)
{
	if (!mark)
		mark = start;
...
extern char *
BX_move_to_abs_word(const char *start, char **mark, int word)
{
	char *pointer = (char *)start;
	int counter = word;

	if(pointer == NULL)
		ircpanic("BX_move_to_abs_word::pointer is NULL");

	/*
	 * This fixes a bug that counted leading spaces as a word, when
	 * theyre really not a word.... (found by Genesis K.)
	 *
	 * The stock client strips leading spaces on both the cases $0 and $-0.
	 * I personally think this is not the best choice, but im not going
	 * to stick my foot in this one... im just going to go with what the
	 * stock client does...
	 */
...
extern char *
BX_move_word_rel(const char *start, char **mark, int word)
{
	char *pointer = *mark;
	int counter = word;
	...
	/*
	 * XXXX - this is utterly pointless at best, and totaly wrong at
	 * worst.
	 */
...
extern char *
BX_extract2(const char *start, int firstword, int lastword)
{
	/*
	 * If firstword or lastword is negative, then we take those values
...
#ifndef NO_CHEATING
		/*
		 * Really. the only case where firstword == EOS is when the
		 * user wants $~, in which case we really dont need to do
		 * all the following crud.  Of course, if there ever comes a
		 * time that the user would want to start from the EOS
		 * (when??) we couldnt make this assumption.
		 */
		return m_strdup(mark);
#endif
	}
	/*
	 * SOS is used when the user does $-n, all leading spaces are
	 * retained
	 */
	...
	/*
	 * IF the user did something like this: $n-  $n-m then include any
	 * leading spaces on the 'n'th word. this is the "old" behavior that
	 * we are attempting to emulate here.
	 */
#ifndef NO_CHEATING
	...
	/*
	 * When we find the last word, we need to move to the END of the
	 * word, so that word 3 to 3, would include all of word 3, so we
	 * sindex to the space after the word
	 */
	if (lastword == EOS)
	...
	/*
	 * If the end is before the string, then there is nothing to extract
	 * (this is perfectly legal, btw)
	 */
	if (mark2 < mark)
 ...
 */
extern char *
BX_extract(char *start, int firstword, int lastword)
{
	/*
	 * firstword and lastword must be zero.  If they are not, then they
	 * are assumed to be invalid  However, please note that taking word
	 * set (-1,3) is valid and contains the words 0, 1, 2, 3.  But word
	 * set (-1, -1) is an empty_string.
	 */
	char *mark;
	char *mark2;
	char *booya = NULL;
	...
	/*
	 * before we do anything, we strip off leading and trailing spaces.
	 *
	 * ITS OK TO TAKE OUT SPACES HERE, AS THE USER SHOULDNT EXPECT THAT THE
	 * WORD FUNCTIONS WOULD RETAIN ANY SPACES. (That is to say that
	 * since the word/list functions dont pay attention to the
	 * whitespace anyhow, noone should have any problem with those ops
	 * removing bothersome whitespace when needed.)
	 */
	...
	/*
	 * Ok.. now if we get to here, then lastword is positive, so we
	 * sanity check firstword.
	 */
	if (firstword < 0)
		firstword = 0;
