...
char   *
convert_flags_to_str(unsigned long flags)
{
	unsigned int i;
	unsigned long p;
...
char   *
convert_flags(unsigned long flags)
{
	unsigned int i;
	unsigned long p;
...
void
prepare_addshit(UserhostItem * stuff, char *nick, char *args)
{
	char *uh;
...
void
add_userhost_to_userlist(char *nick, char *uhost, char *channels, char *passwd, unsigned int flags)
{
	UserList *uptr;
...
int
remove_userhost_from_userlist(char *host, char *channel)
{
	UserList *uptr = NULL;
	int count = 0;
...
void
prepare_adduser(UserhostItem * stuff, char *nick, char *args)
{
	int thetype = 0;
	unsigned long flags = 0;
	int ppp = 0;
	...
	char *channels = NULL, *passwd = NULL, *p = NULL, *uh, *e_host,
	*host;
	...
	if (thetype == USERLIST_ADD) {
		...
		if (!(remove_userhost_from_userlist(host, channels)))
			bitchsay("Unable to locate user on the userlist");
		else
	...
	*buffer = 0;
	...
	if ((nick = next_arg(args, &args)) && *nick) {
		...
		if (passwd && is_number(passwd) && args && *args) {
			...
			if (aop == 1 || aop == 2)
				strlcpy(buffer, "OPS,", sizeof buffer);
			else if (aop == 3 || aop == 4)
				strlcpy(buffer, "VOICE,", sizeof buffer);
			passwd = next_arg(args, &args);
			if (level >= 25)
				strlcat(buffer, "INVITE,", sizeof buffer);
			if (level >= 49)
				strlcat(buffer, "UNBAN,", sizeof buffer);
			if (level >= 90)
				strlcat(buffer, "FLOOD,", sizeof buffer);
			switch (prot) {
			case 1:
				strlcat(buffer, "REOP,", sizeof buffer);
				break;
			case 2:
				strlcat(buffer, "DEOP,", sizeof buffer);
				break;
			default:
				break;
			}
...
BUILT_IN_COMMAND(add_shit)
{
	...
	if (args && *args) {
		...
		if (args && *args)
			...
			malloc_strcpy(&reason, "Shitlisted");
		...
		if (!bang && !atsign && (cn = check_nickname(m_strdup(nick)))) {
			...
			/*
			 * *!*@*test* *!*test@* *!*  bang !atsign *@*  !bang
			 * atsign *test* !bang !atsign
			 */
			char *uh;
...
void
sync_nicklist(UserList * added, int type)
{
	ChannelList *chan;
	NickList *nick;
	int i;
...
void
sync_shitlist(ShitList * added, int type)
{
	ChannelList *chan;
	NickList *nick;
	int i;
...
void
add_to_a_list(char *thestring, int thetype, char *nick, char *channels, char *reason, int shitlevel)
{
	...
	case SHITLIST_ADD:
	{
		if (!(sremove = nickinshit(nick, thestring))) {
			...
			bitchsay("%s!%s already on shitlist", nick, thestring);
		break;
	}
	case SHITLIST_REMOVE:
	{
		...
		while ((sremove = (ShitList *) removewild_from_list((List **) &shitlist_list, s_str))) {
			...
			bitchsay("Removing %s!%s from Shitlist", nick, thestring);
		}
		if (!scount)
			bitchsay("Couldn't locate %s!%s on the Shitlist", nick, thestring);
		new_free(&s_str);
		break;
	}
	}
...
BUILT_IN_COMMAND(showshitlist)
{
	...
	if (!tmp) {
		bitchsay("No entries in shitlist");
		return;
	}
	while (tmp) {
 ...
 */
UserList *
lookup_userlevelc(char *nick, char *userhost, char *channel, char *passwd)
{
...
ShitList *
nickinshit(char *niq, char *uh)
{
	char theuh[BIG_BUFFER_SIZE + 1];
	ShitList *thisptr = shitlist_list;
	char *u;
	...
	while (thisptr) {
		if (!strcmp(thisptr->filter, theuh) || (        /* wild_match(theuh,
								   thisptr->filter) || */wild_match(thisptr->filter, theuh)))
			return (thisptr);
...
int
find_user_level(char *from, char *host, char *channel)
{
	UserList *tmp;
...
int
find_shit_level(char *from, char *host, char *channel)
{
	/* XXX */
	ShitList *shitlist_list = NULL;
	...
	if (!shitlist_list)
		return 0;
	if (!(shitlist_list = (ShitList *) nickinshit(from, host)))
		return 0;
	if (check_channel_match(shitlist_list->channels, channel))
		return shitlist_list->level;
	return 0;
}


int
real_check_auto(void *arg, char *sub)
{
...
delay_opz(void *arg, char *sub)
{
	...
	char *p = (char *)arg;          /* original args unmodified  so we can
					 * free them */
...
delay_kick(void *arg, char *sub)
{
	...
	if (protected) {
		...
		if ((serv_num = next_arg(args, &args)))
			this_server = my_atol(serv_num);
		if (prepare_command(&server, channel, PC_SILENT))
...
void
run_user_flag(char *name, char *what, NickList * user, NickList * kicker)
{
	char *tmp = NULL;
...
NickList *
check_auto(char *channel, NickList * nicklist, ChannelList * chan)
{
	...
	if (get_cset_int_var(chan_ptr->csets, SHITLIST_CSET) && shitptr != NULL && userptr == NULL) {
		...
		case 4:         /* perm ban on offender */
			if (nicklist->sent_deop < 4 || (nicklist->sent_deop < 4 && shitptr->level == 4)) {
				...
				send_to_server("KICK %s %s :%s", channel, nicklist->nick, (shitptr->reason && *shitptr->reason) ? shitptr->reason : theshit);
			}
		default:
			break;
		}
	...
	if (get_cset_int_var(chan_ptr->csets, KICK_IF_BANNED_CSET) && check_channel_match(get_string_var(PROTECT_CHANNELS_VAR), chan_ptr->channel)) {
		...
		if (nicklist->ip && nicklist->host) {
			...
			strlcpy(user, nicklist->host, sizeof user);
			if ((u = strchr(user, '@')))
				*u = 0;
 ...
 */
int
check_prot(char *from, char *person, ChannelList * chan, BanList * thisban, NickList * n)
{
	NickList *tmp = NULL;
	NickList *kicker;
	char *tmp_ban = NULL;
...
/*				do_reop = 1; */
			...
			if (user->flags & PROT_KICK) {
				if (kicker && (!kicker->userlist || (kicker->userlist && !(kicker->userlist->flags & PROT_KICK))))
					send_to_server("KICK %s %s :\002b\002X Protected User", chan->channel, kicker->nick);
			}
...
int
check_channel_match(char *tmp, char *channel)
{
	char *p, *chan = NULL;
	int wmatch = 0;
	size_t size;
...
void
check_hack(char *nick, ChannelList * channel, NickList * ThisNick, char *from)
{
#ifdef WANT_USERLIST
	UserList *userptr = NULL;
	ShitList *shitptr = NULL;
...
int
change_pass(char *from, char *msg)
{
	UserList *tmp;
