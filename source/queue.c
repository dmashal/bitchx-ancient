...
BUILT_IN_COMMAND(queuecmd)
{
	...
	/*
	 * If the queue list is empty, make an entry Some OS's barf on
	 * writing to static strings... I wish ultrix wasnt so darn
	 * forgiving...
	 */
	char this_sucks[4];
	...
	/*
	 * if the next queue is empty, then we need to see if we should make
	 * it or output an error
	 */
	if ((tmp->next) == NULL) {
		if (commands)
