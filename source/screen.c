...
BX_skip_incoming_mirc(const unsigned char *text)
{
	...
	text2 = LOCAL_COPY(text);
	return text2;
}

static void
delchar(unsigned char **text, size_t cnum)
{
	size_t i;
 ...
 */
void
BX_add_to_screen(const unsigned char *buffer)
{
 ...
 */
void
BX_add_to_window(Window * window, const unsigned char *str)
{
...
u_char **
BX_split_up_line(const unsigned char *str, int max_cols)
{
	int nl = 0;
...
unsigned char **
BX_prepare_display(const unsigned char *orig_str,
                   int max_cols,
                   int *lused,
                   int flags)
{
	...
	int pos = 0,                    /* Current position in "buffer" */
	    ...
	    firstwb = 0, beep_cnt = 0,  /* Number of beeps              */
	    beep_max,                   /* Maximum number of beeps      */
	    tab_cnt = 0,                /* TAB counter                  */
	    tab_max,                    /* Maximum number of tabs       */
	    nds_count = 0, nds_max, line = 0,   /* Current pos in "output"      */
	...
	u_char buffer[BIG_BUFFER_SIZE + 1], *cont_ptr = NULL, *cont = empty_string,
	       c, *words = NULL, *str = NULL;
	...
	for (ptr = str; *ptr && (pos < BIG_BUFFER_SIZE - 8); ptr++) {
		...
		case '\011':            /* TAB */
		{
			...
			if ((tab_max > 0) && (tab_cnt > tab_max)) {
				...
				/*
				 * Only go as far as the edge of the
				 * screen
				 */
				len = 8 - (col % 8);
				for (i = 0; i < len; i++) {
	...
	}                               /* End of (ptr = str; *ptr && (pos <
					 * BIG_BUFFER_SIZE - 8); ptr++) */
 ...
 */
static int
add_to_scratch_window_display_list(Window * window, const unsigned char *str)
{
 ...
 */
int
BX_output_line(const unsigned char *str)
{
 ...
 */
int
BX_output_with_count(const unsigned char *str, int clreol, int output)
{
 ...
 */
void
BX_scroll_window(Window * window)
{
	if (dumb_mode)
		return;
 ...
 */
void
BX_cursor_not_in_display(Screen * s)
{
	if (!s)
		s = output_screen;
	if (s->cursor_window)
 ...
 */
int
BX_is_cursor_in_display(Screen * screen)
{
 ...
 */
void
BX_repaint_window(Window * w, int start_line, int end_line)
{
...
#ifndef GUI
extern Window *
BX_create_additional_screen(void)
{
	Window *win;
	Screen *new;
 ...
 */
const u_char *
BX_skip_ctl_c_seq(const u_char *start, int *lhs, int *rhs, int proper)
{
	...
	*lhs = *rhs = -1;
	...
	for (;; ) {
		...
		/*
		 * 1X -> 1 <stop> X if X >= '6' 1X -> 1X <stop>  if
		 * X < '6'
		 */
		case '1':
			after++;
...
u_char *
BX_normalize_string(const u_char *str)
{
	u_char *output;
	u_char chr, chr1;
	int pos, maxpos;
	...
	/*
	 * The output string has a few extra chars on the end just in case
	 * you need to tack something else onto it.
	 */
