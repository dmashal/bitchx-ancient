...
ExecuteTimers(void)
{
	...
	/*
	 * We do NOT want to parse timers while waiting cause it gets icky
	 * recursive
	 */
	...
	while (PendingTimers && BX_time_diff(now1, PendingTimers->time) < 0) {
		...
		/*
		 * If a callback function was registered, then we use it.
		 * If no callback function was registered, then we use
		 * ''parse_line''.
		 */
...
int
timer_callback_exists(void *ref)
{
	TimerList *t;
...
char   *
function_timer(char *n, char *args)
{
	char *ref;
	char *out = NULL;
	TimerList *tmp;
