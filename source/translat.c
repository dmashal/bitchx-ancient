...
/* Globals */
unsigned char transToClient[256];               /* Server to client
						 * translation. */
unsigned char transFromClient[256];             /* Client to server
						 * translation. */
...
{
	last_input_screen->digraph_hit = 1;     /* Just stuff away first
						 * character. */
}
 ...
 */
void
set_translation(Window * win, char *tablename, int unused)
{
	FILE *table;
	...
	/*
	 * Any problems in the translation tables between hosts are almost
	 * certain to be caused here. many scanf implementations do not work
	 * as defined. In particular, scanf should ignore white space
	 * including new lines (many stop at the new line character, hence
	 * the fgets and sscanf workaround), many fail to read 0xab as a
	 * hexadecimal number (failing on the x) despite the 0x being
	 * defined as optionally existing on input, and others zero out all
	 * the output variables if there is trailing non white space in the
	 * format string which doesn't appear on the input. Overall, the
	 * standard I/O libraries have a tendancy not to be very standard.
	 */
