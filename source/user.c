...
static inline void
add_userlist_to_tier_1(UserList * ptr, unsigned long hvalue)
{
...
static inline void
add_userlist_to_tier_2(UserList * ptr, unsigned long hvalue)
{
...
static inline void
add_userlist_to_tier_3(UserList * uptr)
{
	add_to_list((List **) &user_list, (List *) uptr);
}

void
add_userlist(UserList * uptr)
{
	char *ptr;
	long hvalue;
 ...
 */
static inline void
move_link_to_top(UserList * tmp, UserList * prev, HashEntry * location)
{
	if (prev) {
		UserList *old_list;
 ...
 */
static inline void
remove_link_from_list(UserList * tmp, UserList * prev, HashEntry * location)
{
	if (prev) {
...
static inline UserList *
find_userlist_tier3(char *host, char *channel, int remove)
{
	UserList *tmp;
...
static inline UserList *
find_userlist_tier1(char *userhost, char *channel, unsigned long hvalue, int remove)
{
	HashEntry *location;
	UserList *tmp, *prev = NULL;
...
static inline UserList *
find_userlist_tier2(char *userhost, unsigned long hvalue, int remove)
{
	HashEntry *location;
	UserList *tmp, *prev = NULL;
...
UserList *
find_userlist(char *userhost, char *channel, int remove)
{
	char *ptr;
	long hvalue;
...
void
destroy_sorted_userlist(UserList ** uptr)
{
	UserList *t;
...
#if 0
UserHist *
next_userlist(UserList * uptr, int *size)
{
	long hvalue = -1;
...
static inline int
check_best_passwd(char *passwd, char *test)
{
	if (passwd && test)
...
UserList *
find_bestmatch(char *nick, char *userhost, char *channel, char *passwd)
{
	UserList *best = NULL;
	...
	if ((hvalue = hash_name(strrchr(check, '@'), USERHOST_HASHSIZE)) != -1) {
		for (tmp = (UserList *) UserListByHost_Table[hvalue].list; tmp; tmp = tmp->next) {
			...
			if (user_match > best_user_match && chan_match > best_chan_match) {
				...
				best = tmp;
				if (check_best_passwd(tmp->password, passwd))
					best_passwd = tmp;
...
UserList *
next_userlist(UserList * uptr, int *size, void **location)
{
	int hvalue = -1;
