...
HWND MDIClient = 0, MDIFrame = 0;       /* MDI handles */
QMSG qmsg = {0};                        /* message-queue structure */
HDC hdc = 0;                            /* handle to the device context */
HVPS hvps = 0, mainHvps = 0,            /* handle to the AVIO presentation
					 * space */
     hvpsnick = 0;
static HEV sem = 0;
VIOMODEINFO viomode;
...
ULONG
EXPENTRY (*DLL_mciPlayFile)(HWND, PSZ, ULONG, PSZ, HWND) = NULL;
...
void
gui_settitle(char *titletext, Screen * os2win)
{
		...
		case DID_OK:
			...
			while (changescreen) {
				if (changescreen->alive) {
					...
					/*
					 * No point in making bigger windows
					 * than the screen
					 */
					if (cx > maxX)
					...
					/*
					 * Recalculate in case we modified
					 * the values
					 */
					cx = (co - 1) * changescreen->VIO_font_width;
					cy = li * changescreen->VIO_font_height;
...
void
font_dialog_stub(Screen * screen)
{
	HAB hnbab;
	HMQ hnbmq;
...
HWND
newsubmenu(MenuStruct * menutoadd, HWND location, int pulldown)
{
	HWND tmphandle;
	MenuList *tmp;
	MENUITEM miSubMenu;
	...
	/*
	 * Go through all menuitems adding items and submenus (and subitems)
	 * as neccessary
	 */
...
HWND
menucreatestub(char *addthismenu, HWND location, int pulldown)
{
...
void
detach_shared(Screen * this_screen)
{
	int itemid = (int)WinSendMsg(this_screen->hwndMenu, MM_ITEMIDFROMPOSITION, (MPARAM) 0, (MPARAM) NULL);

	/*
	 * Set any owners of shared menus to HWND_OBJECT so the won't get
	 * destroyed - thanks mikh
	 */
	...
	while (itemid != -1) {
		/*
		 * This may cause memory leaks... unless they are all shared
		 * (menu.bx is)
		 */
...
void
window_menu_stub(Screen * menuscreen, char *addthismenu)
{
...
int
test_callback(HWND window, void *data)
{
...
void
pm_aboutbox(char *about_text)
{
	HWND mainwindow, text, stext, logo, okbutton, buttonbox, lbbox;
...
MRESULT EXPENTRY
GenericWndProc(HWND hwnd, ULONG msg, MPARAM mp1, MPARAM mp2)
{
	...
	case WM_BUTTON1MOTIONSTART:
		if (this_screen) {
			...
			/*
			 * Allocate a buffer to store the entire screen so
			 * it won't change during selection - NuKe
			 */
			...
			if (WinTrackRect(hwnd, NULLHANDLE, &ti)) {
				...
				mark = new_malloc((ty - sy) * (tx - sx + 2) + 1);
				i = 0;
				for (y = sy; y < ty; y++) {
					len = tx - sx;
				...
				/*
				 * Ok, we got the stuff - now place it in
				 * the clipboard
				 */
				...
				/*
				 * Ok, clipboard wants giveable unnamed
				 * shared memory
				 */
	...
	case WM_SETFOCUS:
		if (SHORT1FROMMP(mp2) == TRUE) {
			/*
			 * If we have a shared menu attached to the window
			 * make it owned by the current window so it opens
			 * properly
			 */
			...
			if (this_screen && this_screen->current_window) {
				...
				/*
				 * Tell the main thread to set this window
				 * as current
				 */
	...
	case WM_BUTTON3DBLCLK:
	{
		...
		/*
		 * I was hoping there was an easier way but....I
		 * gave up
		 */
	...
	break;
	case WM_VSCROLL:
		if (SHORT1FROMMP(mp1) == 1000) {        /* Channel is being
							 * scrolled */
		...
		} else if (SHORT1FROMMP(mp1) == 1001) { /* Nicklist is scrolling */
			switch (SHORT2FROMMP(mp2)) {
			case SB_LINEUP:
				this_screen->spos--;
				break;
...
MRESULT EXPENTRY
MDIWndProc(HWND hwnd, ULONG msg, MPARAM mp1, MPARAM mp2)
{
	switch (msg) {
	case WM_PAINT:
	{
...
void
load_default_font(Screen * font_screen)
{
...
MRESULT EXPENTRY
NotebookPage1DlgProc(HWND hWnd, ULONG msg, MPARAM mp1, MPARAM mp2)
{
	char szBuffer[1024];
	int i;
	char *ptr;
...
MRESULT EXPENTRY
NotebookPage2DlgProc(HWND hWnd, ULONG msg, MPARAM mp1, MPARAM mp2)
{
	char szBuffer[1024];
	int i;
	char *ptr;
...
MRESULT EXPENTRY
NotebookPage3DlgProc(HWND hWnd, ULONG msg, MPARAM mp1, MPARAM mp2)
{
...
MRESULT EXPENTRY
NotebookPage4DlgProc(HWND hWnd, ULONG msg, MPARAM mp1, MPARAM mp2)
{
	char szBuffer[1024];            /* String Buffer */
	char *ptr;
...
MRESULT EXPENTRY
NotebookPage5DlgProc(HWND hWnd, ULONG msg, MPARAM mp1, MPARAM mp2)
{
	switch (msg) {
	case WM_CHAR:
		break;
...
MRESULT EXPENTRY
NotebookDlgProc(HWND hWnd, ULONG msg, MPARAM mp1, MPARAM mp2)
{
...
void
pm_clrscr(Screen * tmp)
{
	VioScrollUp(0, 0, -1, -1, -1, (PBYTE) &default_pair, tmp->hvps);
}

void
gui_font_set(char *fontsize, Screen * screen)
{
	char *height, *width;
	int t;
...
gui_font_init(void)
{
	...
	/*
	 * if(!get_int_var(SCROLLERBARS_VAR)) WinSetParent();
	 */
}
...
gui_scroll(int top, int bot, int n)
{
	aflush();
	if (n > 0)
		VioScrollUp(top, 0, bot, output_screen->co, n, (PBYTE) &pair, output_screen->hvps);
	else if (n < 0) {
		n = -n;
		VioScrollDn(top, 0, bot, output_screen->co, n, (PBYTE) &pair, output_screen->hvps);
	}
	return;
}
...
void
pm_new_window(Screen * new, Window * win)
{
	int cx, cy;
	HDC hdc;
...
MRESULT EXPENTRY
FileFilterProc(HWND hwnd, ULONG message, MPARAM mp1, MPARAM mp2)
{
...
void
pm_file_dialog(char *data[7])
{
	/*
	 * 0 type 1 path 2 title 3 ok 4 apply 5 szButton 6 code
	 */
	HAB hfdab;
	HMQ hfdmq;
...
gui_paste(char *args)
{
	...
	if (rc) {                       /* Text data in clipboard */
		...
		oclip = clip = malloc(strlen(pszClipText) + 1);
		line = 0;
		...
		if (!smartpaste) {
			...
			/*
			 * Rosmo's Incredibly Dull SmartPaste: Fits as much
			 * as possible into a 80-nick_length buffer,
			 * stripping spaces from beginning and end and then
			 * pasting
			 */
