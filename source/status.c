...
char *away_format = NULL, *hold_lines_format = NULL, *channel_format = NULL, *notify_format = NULL, *cpu_saver_format = NULL, *kill_format = NULL, *lag_format = NULL, *mail_format = NULL, *nick_format = NULL, *query_format = NULL, *server_format = NULL, *clock_format = NULL, *dcccount_format = NULL, *cdcc_format = NULL, *msgcount_format = NULL, *umode_format = NULL, *mode_format = NULL, *topic_format = NULL, *user_format = NULL;
...
static char *
convert_format(Window * win, char *format, int k)
{
	char buffer[BIG_BUFFER_SIZE + 1];
...
void
fix_status_buffer(Window * win, char *buffer, int in_status)
{
	unsigned char rhs_buffer[3 * BIG_BUFFER_SIZE + 1];
	unsigned char lhs_buffer[3 * BIG_BUFFER_SIZE + 1];
	unsigned char lhs_fillchar[6], rhs_fillchar[6], *fillchar = lhs_fillchar, *lhp = lhs_buffer, *rhp = rhs_buffer, *cp, *start_rhs = 0, *str = NULL,
	*ptr = NULL;
	int in_rhs = 0, pr_lhs = 0, pr_rhs = 0, *prc = &pr_lhs;
...
#endif
	strlcpy(buffer, lhs_buffer, sizeof buffer);
	strlcat(buffer, rhs_buffer, sizeof buffer);
	new_free(&str);
}

char   *
stat_convert_format(Window * win, char *form)
{
...
void
BX_build_status(Window * win, char *format, int unused)
{
	int i, k;
	char *f = NULL;
	int ofs = from_server;
...
make_status(Window * win)
{
	...
	/*
	 * The second status line is only displayed in the bottom window and
	 * should always be displayed, no matter what SHOW_STATUS_ALL is set
	 * to - krys
	 */
	for (status_line = 0; status_line < 1 + win->double_status + win->status_lines; status_line++) {
		u_char lhs_fillchar[6], rhs_fillchar[6], *fillchar = lhs_fillchar, *lhp = lhs_buffer, *rhp = rhs_buffer, *cp, *start_rhs = 0, *str;
		int in_rhs = 0, pr_lhs = 0, pr_rhs = 0, line = status_line, *prc = &pr_lhs, i;
		...
		/*
		 * Patched 26-Mar-93 by Aiken make_window now
		 * right-justifies everything after a %> it's also more
		 * efficient.
		 */
		...
		*cp = 0;
		...
		/* This can't be fixed easily with sizeof */
		...
		*buf2 = '\0';
		...
		while ((traverse_all_windows(&window))) {
			if (window->miscflags & WINDOW_NOTIFIED) {
				...
				strlcat(buf2, ltoa(window->refnum), sizeof buf2);
			}
		}
	}
	if (doneone && window->wset->notify_format) {
		snprintf(my_buffer, sizeof my_buffer, window->wset->notify_format, buf2);
		return (my_buffer);
	}
	RETURN_EMPTY;
}
...
status_channel(Window * window)
{
	...
	if (window->current_channel     /* && chan_is_connected(s,
					   window->server) */) {
		int num;
...
static char *
status_oper_kills(Window * window)
{
        static char my_buffer[MY_BUFFER + 1];
...
static char *
status_cdcccount(Window * window)
{
#ifdef WANT_CDCC
...
static char *
status_cpu_saver_mode(Window * window)
{
        static char my_buffer[MY_BUFFER / 2 + 1];
...
status_users(Window * window)
{
        ...
        if (window->server != -1 && window->wset->status_users_format) {
                if ((chan = prepare_command(&serv, NULL, PC_SILENT))) {
                        ...
                        char buff[40], buff1[40], buff2[40], buff3[40],
                             buff4[40];
...
static char *
status_notifyusers(Window * window)
{
        int serv = window->server;
...
static char *
status_newserver(Window * window)
{
...
static char *
status_windowspec(Window * window)
{
