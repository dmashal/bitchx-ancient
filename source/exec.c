 ...
 *
 * Copyright 1990 Michael Sandrof Copyright 1997 EPIC Software Labs See the
 * COPYRIGHT file, or do a HELP IRCII COPYRIGHT
 */

#define _GNU_SOURCE                     /* Needed for strsignal from string.h
					 * on GLIBC systems */
...
typedef struct {
	...
	int termsig;    /* The signal that terminated the
			 * process */
...
/*
 * exec: the /EXEC command.  Does the whole shebang for ircII sub procceses I
 * melded in about half a dozen external functions, since they were only ever
 * used here.  Perhaps at some point theyll be broken out again, but not
 * until i regain control of this module.
 */
...
BUILT_IN_COMMAND(execcmd)
{
	...
	int direct = 0;
	Process *proc;
	char *stdoutc = NULL, *stderrc = NULL, *stderrpc = NULL, *stdoutpc = NULL, *endc = NULL;
	...
	while ((*args == '-') && (flag = next_arg(args, &args))) {
		...
		/*
		 * /EXEC -WINDOW forces all output for an /exec to go to the
		 * window that is current at this time
		 */
		...
		/*
		 * /EXEC -MSG <target> redirects the output of an /exec to
		 * the given target.
		 */
		...
		/*
		 * /EXEC -CLOSE forcibly closes all the fd's to a process, in
		 * the hope that it will take the hint.
		 */
		...
		/*
		 * /EXEC -NOTICE <target> redirects the output of an /exec to
		 * a specified target
		 */
		...
		/*
		 * /EXEC -start <command> starts a command with no
		 * association to the main BitchX process.
		 */
	...
	/*
	 * /EXEC -INQUIET quietly sends a line of text to a
	 * process
	 */
...
if (*args == '%') {
	...
	/*
	 * Check to see if the user is changing the default
	 * target
	 */
	...
	/*
	 * Check to see if the user changed the NAME of
	 * %proc.
	 */
	if (logical_flag) {
...
/*
 * The user is trying to fire up a new /exec, so pass the
 * buck.
 */
else {
	...
	case 0:
	{
		 ...
		 * Sever all ties we had with the
		 * parent ircII process
		 */
#if !defined(WINNT) && !defined(__EMX__)
		setsid();
#endif
		if (setgid(getgid()) == -1)
			ircpanic("FATAL: setgid failed with errno %d", errno);
		if (setuid(getuid()) == -1)
			ircpanic("FATAL: setuid failed with errno %d", errno);
	 ...
	 */
	/*
	 * Figure out what shell (if any)
	 * we're using
	 */
	shell = get_string_var(SHELL_VAR);

	/*
	 * If we're not using a shell, doovie
	 * up the exec args array and pass it
	 * off to execvp
	 */
	if (direct || !shell) {
		char **args;
	...
	/*
	 * If we're using a shell, let them
	 * have all the fun
	 */
	else {
...
#ifndef __EMXPM__
	/*
	 * Something really died if we got
	 * here
	 */
...
/*
* PARENT: add the new process to the process
* table list
*/
default:
#else
...
#endif
{
	...
	/*
	 * If there are no empty proc
	 * entries, make a new one.
	 */
...
/*
 * do_processes: This is called from the main io() loop to
 * handle any pending /exec'd events.  All this does is call
 * handle_filedesc() on the two reading descriptors.  If an
 * EOF is asserted on either, then they are closed.  If EOF
 * has been asserted on both, then  we mark the process as
 * being "dumb".  Once it is reaped (exited), it is expunged.
 */
void
...
/*
 * This is the back end to do_processes, saves some repeated
 * code
 */
static void
handle_filedesc(Process * proc, int *fd, int hook_nonl, int hook_nl)
{
...
/*
 * get_child_exit: This looks for dead child processes of the
 * client. There are two main sources of dead children:
 * Either an /exec'd process has exited, or the client has
 * attempted to fork() off a helper process (such as wserv or
 * gzip) and that process has choked on itself.
 *
 * When SIGCHLD is recieved, the global variable
 * 'dead_children_processes' is incremented.  When this
 * function is called, we go through and call waitpid() on
 * all of the outstanding zombies, conditionally stopping
 * when we reach a specific wanted sub-process.
 *
 * If you want to stop reaping children when a specific
 * subprocess is reached, specify the process in 'wanted'. If
 * all youre doing is cleaning up after zombies and /exec's,
 * then 'wanted' should be -1.
 */

extern SIGNAL_HANDLER(child_reap);

int
get_child_exit(pid_t wanted)
{
	Process *proc;
	pid_t pid;
	int status, i;
	...
	/*
	 * Iterate until we've reaped all of the dead
	 * processes or we've found the one asked for.
	 */
	if (dead_children_processes)
		while ((pid = waitpid(wanted, &status, WNOHANG)) > 0) {
...
#endif
			/*
			 * First thing we do is look to see
			 * if the process we're working on is
			 * the one that was asked for.  If it
			 * is, then we get its exit status
			...
			/*
			 * If it wasnt the process asked for,
			 * then we've probably stumbled
			 * across a dead /exec'd process.
			 * Look for the corresponding child
			 * process, and mark it as being
			 * dead.
			 */
			else {
				for (i = 0; i < process_list_size; i++) {
	...
	/*
	 * Now we may have reaped some /exec'd processes that
	 * were previously dumb and have now exited.  So we
	 * call cleanup_dead_processes() to find and delete
	 * any such processes.
	 */
	cleanup_dead_processes();

	/*
	 * If 'wanted' is not -1, then we didnt find that
	 * process, and if 'wanted' is -1, then you should
	 * ignore the retval anyhow. ;-)
	 */
	return -1;
}
...
/*
 * clean_up_processes: In effect, we want to tell all of our
 * sub processes that we're going away.  We cant be 100% sure
 * that theyre all dead by the time this function returns,
 * but we can be 100% sure that they will be killed off next
 * time they come up to run.  This is the only thing that can
 * be guaranteed, and is in fact all we really need to know.
 */
void
...
/*
 * text_to_process: sends the given text to the given
 * process.  If the given process index is not valid, an
 * error is reported and 1 is returned. Otherwise 0 is
 * returned. Added show, to remove some bad recursion, phone,
 * april 1993
 */
int
text_to_process(int proc_index, const char *text, int show)
{
	...
	strlcpy(my_buffer, text, sizeof my_buffer);
	strlcat(my_buffer, "\n", sizeof my_buffer);
...
/*
 * When a server goes away, re-assign all of the /exec's that
 * are current bound to that server.
 */
void
...
/*
 * This adds a new /wait %proc -cmd   entry to a running
 * process.
 */
void
add_process_wait(int proc_index, const char *cmd)
{
...
/*
 * A process must go through two stages to be completely
 * obliterated from the client.  Either stage may happen
 * first, but until both are completed we keep the process
 * around.
 *
 * 1) We must recieve an EOF on both stdin and stderr, or we
 * must have closed stdin and stderr already (handled by
 * do_processes) 2) The process must have died (handled by
 * get_child_exit)
 *
 * The reason why both must happen is becuase the process can
 * die (and we would get an async signal) before we read all
 * of its output on the pipe, and if we simply deleted the
 * process when it dies, we could lose some of its output.
 * The reason why we cant delete a process that has asserted
 * EOF on its output is because it could still be running
 * (duh! ;-) So we wait for both to happen.
 */

/*
 * This function is called by the three places that can
 * effect a change on the state of a running process: 1)
 * get_child_exit, which can mark a process as exited 2)
 * do_processes, which can mark a child as being done with
 * I/O 3) execcmd, which cacn mark a child as being done with
 * I/O
 *
 * Any processes that are found to have both exited and having
 * completed their I/O will be summarily destroyed.
 */
static void
cleanup_dead_processes(void)
{
	...
	for (i = 0; i < process_list_size; i++) {
		 ...
		 * both exited and finished its io, UNLESS it
		 * has been disowned.
		 */
		...
		/*
		 * First thing we do is run any /wait %proc
		 * -cmd commands
		 */
		next = dead->waitcmds;
		dead->waitcmds = NULL;
...
/*
 * ignore_process: When we no longer want to communicate with
 * the process any longer, we call here.  It continues
 * execution until it is done, but we are oblivious to any
 * output it sends.  Now, it will get an EOF condition on its
 * output fd, so it will probably either take the hint, or
 * its output will go the bit bucket (which we want to
 * happen)
 */
static void
ignore_process(int index)
{
	Process *proc;
...
/*
 * kill_process: sends the given signal to the specified
 * process.  It does not delete the process from the process
 * table or anything like that, it only is for sending a
 * signal to a sub process (most likely in an attempt to kill
 * it.)  The actual reaping of the children will take place
 * async on the next parsing run.
 */
void
...
#define killpg(pg, sig) kill(-(pg), (sig))
	...
	/*
	 * The exec'd process shouldn't be in our process
	 * group
	 */
	if (pgid == getpid()) {
...
/*
 * This kills (sends a signal, *NOT* ``make it stop
 * running'') all of the currently running subprocesses with
 * the given signal.  Presumably this is because you want
 * them to die.
 *
 * Remember that UNIX signals are asynchornous.  At best, you
 * should assume that they have an advisory effect.  You can
 * tell a process that it should die, but you cannot tell it
 * *when* it will die -- that is up to the system. That means
 * that it is pointless to assume the condition of any of the
 * kill()ed processes after the kill().  They may indeed be
 * dead, or they may be ``sleeping but runnable'', or they
 * might even be waiting for a hardware condition (such as a
 * swap in).  You do not know when the process will actually
 * die.  It could be 15 ns, it could be 15 minutes, it could
 * be 15 years.  Its also useful to note that we, as the
 * parent process, will not recieve the SIGCHLD signal until
 * after the child dies.  That means it is pointless to try
 * to reap any children processes here.  The main io() loop
 * handles reaping children (by calling get_child_exit()).
 */
static void
kill_all_processes(int signo)
{
	int i;
	int tmp;
...
/* * * * * * logical stuff * * * * * * */
/*
 * valid_process_index: checks to see if index refers to a
 * valid running process and returns true if this is the
 * case.  Returns false otherwise
 */
static int
valid_process_index(int process)
{
...
/*
 * logical_to_index: converts a logical process name to it's
 * approriate index in the process list, or -1 if not found
 */
int
logical_to_index(const char *logical)
{
	Process *proc;
	int i;
...
/*
 * get_process_index: parses out a process index or logical
 * name from the given string
 */
int
...
/*
 * is_valid_process: tells me if the spec is a process that
 * is either running or still has not closed its pipes, or
 * both.
 */
int
is_valid_process(const char *arg)
{
	if (!arg || *arg != '%')
		return -1;
...
/*
 * set_process_bits: This will set the bits in a fd_set map
 * for each of the process descriptors.
 */
void
