...
void
BX_close_server(int cs_index, char *message)
{
...
int
close_all_servers(char *message)
{
	int i;
 ...
 */
void
close_unattached_server(int server)
{
...
set_server_bits(fd_set *rd, fd_set *wr)
{
	...
	for (i = 0; i < number_of_servers; i++) {
		if (server_list[i].reconnect > 0) {
			/*
			 * CONNECT_DELAY is in seconds but we must return in
			 * milliseconds.
			 */
...
int
timed_server(void *args, char *sub)
{
	char *p = (char *)args;
	static int retry = 0;
	int serv = -1;
...
int
find_old_server(int old_server)
{
	int i;
...
int
advance_server(int i)
{
	int server = i;

	/*
	 * We were waiting for this server to connect and it didn't, so we
	 * will either try again or move to the next server.
	 */
	...
	if (server_list[i].retries >= get_int_var(MAX_SERVER_RECONNECT_VAR)) {
		...
		if (server != i) {
			/*
			 * We have a new server to try, so lets move the
			 * variables over from the last one and tell it to
			 * try to connect.
			 */
...
void
reconnect_server(int *servernum, int *times, time_t *last_timeout)
{
	int orig;
	...
	for (i = 0; i < number_of_servers; i++) {
		...
		if (((des = server_list[i].read) > -1) && FD_ISSET(des, rd)) {
...
#ifdef NON_BLOCKING_CONNECTS
					/*
					 * If we get here before getting
					 * above we have problems.
					 */
			...
			case -1:        /* EOF condition */
			{
				...
				/*
				 * Try to make sure output goes to
				 * the correct window
				 */
 ...
 */
extern int
BX_find_in_server_list(char *server, int port)
{
 ...
 */
int
BX_parse_server_index(char *str)
{
	int i;
 ...
 */
int
BX_find_server_refnum(char *server, char **rest)
{
	int refnum;
	int port = irc_port;
 ...
 */
void
BX_add_to_server_list(char *server, int port, char *password, char *nick, char *snetwork, int ssl, int overwrite)
{
...
remove_from_server_list(int i)
{
	...
	/*
	 * this should save a coredump.  If number_of_servers drops down to
	 * zero, then trying to do a realloc ends up being a free, and
	 * accessing that is a no-no.
	 */
 ...
 */
void
BX_parse_server_info(char *name, char **port, char **password, char **nick, char **snetwork)
{
	char *ptr, delim;
...
int
BX_build_server_list(char *servers)
{
	char *host, *rest, *password = NULL, *port = NULL, *nick = NULL, *snetwork = NULL;
...
int
read_and_parse_server(char **filename, char *buffer)
{
	FILE *fp;
	int i = 0;
 ...
 */
int
BX_read_server_file(char *servers_file)
{
	int some = 0;
	char *file_path = NULL;
	char buffer[BIG_BUFFER_SIZE + 1];
 ...
 */
void
write_server_list(char *filename)
{
	FILE *serverfile;
	char *sgroup = NULL;
	int i;
 ...
 */
void
write_server_file(char *servers_file)
{
	char *file_path = NULL;
	char *expanded;
...
connect_to_server_direct(char *server_name, int port)
{
	...
	snprintf(lockfile, sizeof lockfile, "%s/.identwd/%s.%i.LOCK", pw->pw_dir,
 ...
 */
int
finalize_server_connect(int refnum, int c_server, int my_from_server)
{
	if (serv_open_func)
...
int
BX_connect_to_server_by_refnum(int refnum, int c_server)
{
	char *sname;
	int sport;
	int conn;
...
int
next_server_internal(int server, int depth, int original)
{
	int been_here = 0;
	...
	if (is_server_open(server)) {
		/*
		 * The depth allows us to make sure we don't recurse forever
		 * if there are no servers in the list that meet the
		 * requirements.
		 */
 ...
 */
void
BX_get_connected(int server, int old_server)
{
	...
	/*
	 * We shall defer this to get executed in the main loop to maintain
	 * other server or socket connections during connect attempts.
	 */
	set_server_reconnect(server, 1);
 ...
 */
void
try_connect(int server, int old_server)
{
	if (server_list) {
		if (server >= number_of_servers)
			server = 0;
...
char   *
BX_get_possible_umodes(int gu_index)
{
	if (gu_index == -1)
 ...
 */
void
BX_set_server_away(int ssa_index, char *message, int silent)
{
...
char   *
BX_get_server_away(int gsa_index)
{
	if (gsa_index == -1)
		return NULL;
	if (gsa_index == -2) {
...
void
set_server_awaytime(int server, time_t t)
{
...
time_t
get_server_awaytime(int server)
{
	if (server <= -1 || server > number_of_servers)
		return 0;
...
void
BX_set_server_flag(int ssf_index, int flag, int value)
{
	if (ssf_index == -1)
...
int
BX_get_server_flag(int gsf_index, int value)
{
	if (gsf_index == -1)
 ...
 */
void
BX_set_server_version(int ssv_index, int version)
{
	if (ssv_index == -1)
 ...
 */
int
BX_get_server_version(int gsv_index)
{
	if (gsv_index == -1)
...
char   *
BX_get_server_name(int gsn_index)
{
	if (gsn_index == -1)
...
char   *
BX_get_server_network(int gsn_index)
{
	if (gsn_index == -1)
		gsn_index = primary_server;
	if (gsn_index <= -1 || gsn_index >= number_of_servers)
 ...
 */
char   *
BX_set_server_password(int ssp_index, char *password)
{
...
int
BX_server_list_size(void)
{
	return number_of_servers;
}

int
get_server_watch(int gsn_index)
{
	if (gsn_index == -1)
		gsn_index = primary_server;
	if (gsn_index <= -1 || gsn_index >= number_of_servers)
...
void
set_server_watch(int gsn_index, int watch)
{
	if (gsn_index == -1)
		gsn_index = primary_server;
	if (gsn_index == -1 || gsn_index >= number_of_servers)
...
char   *
BX_get_server_itsname(int gsi_index)
{
	if (gsi_index == -1)
...
void
BX_set_server_itsname(int ssi_index, char *name)
{
	if (ssi_index == -1)
 ...
 */
int
BX_is_server_open(int iso_index)
{
 ...
 */
int
BX_is_server_connected(int isc_index)
{
...
void
clear_sent_to_server(int servnum)
{
	server_list[servnum].sent = 0;
}

int
sent_to_server(int servnum)
{
...
int
BX_get_server_port(int gsp_index)
{
	if (gsp_index == -1)
 ...
 */
char   *
BX_get_server_nickname(int gsn_index)
{
 ...
 */
void
BX_set_server2_8(int ss2_index, int value)
{
...
int
BX_get_server2_8(int gs2_index)
{
	if (gs2_index == -1)
 ...
 */
int
BX_get_server_operator(int gso_index)
{
 ...
 */
void
BX_set_server_operator(int sso_index, int flag)
{
 ...
 */
void
BX_set_server_nickname(int ssn_index, char *nick)
{
...
void
BX_set_server_redirect(int s, const char *who)
{
	malloc_strcpy(&server_list[s].redirect, who);
}

char   *
BX_get_server_redirect(int s)
{
	return server_list[s].redirect;
}

int
BX_check_server_redirect(char *who)
{
...
void
BX_set_server_cookie(int ssm_index, char *cookie)
{
...
char   *
BX_get_server_cookie(int ssm_index)
{
	char *s = NULL;
...
void
BX_set_server_motd(int ssm_index, int flag)
{
	if (ssm_index != -1 && ssm_index < number_of_servers)
...
int
BX_get_server_lag(int gso_index)
{
	if ((gso_index < 0 || gso_index >= number_of_servers))
		return 0;
...
void
BX_set_server_lag(int gso_index, int secs)
{
...
time_t
get_server_lagtime(int gso_index)
{
	if ((gso_index < 0 || gso_index >= number_of_servers))
		return 0;
...
void
set_server_lagtime(int gso_index, time_t secs)
{
	if ((gso_index != -1 && gso_index < number_of_servers))
...
int
BX_get_server_motd(int gsm_index)
{
...
void
BX_server_is_connected(int sic_index, int value)
{
...
void
set_server_version_string(int servnum, const char *ver)
{
	malloc_strcpy(&server_list[servnum].version_string, ver);
}

char   *
get_server_version_string(int servnum)
{
	return server_list[servnum].version_string;
}

unsigned long
get_server_ircop_flags(int servnum)
{
...
void
set_server_ircop_flags(int servnum, unsigned long flag)
{
	if (servnum < 0 || servnum >= number_of_servers)
		return;
...
int
get_server_in_timed(int servnum)
{
	if (servnum < 0 || servnum >= number_of_servers)
		return 0;
...
void
set_server_in_timed(int servnum, int val)
{
	if (servnum < 0 || servnum >= number_of_servers)
		return;
...
time_t
get_server_lastmsg(int servnum)
{
	if (servnum < 0 || servnum >= number_of_servers)
		return 0;
...
char   *
get_server_userhost(int gsu_index)
{
 ...
 */
void
got_my_userhost(UserhostItem * item, char *nick, char *stuff)
{
...
static int
write_to_server(int server, int des, char *buffer)
{
	int err = 0;
...
void
add_to_server_queue(int server, int des, char *buffer)
{
	QueueSend *tmp, *tmp1;
...
static void
vsend_to_server(int type, const char *format, va_list args)
{
	...
	if (server > -1 && ((des = server_list[server].write) != -1) && format) {
		...
		/* XXX - This might be a break. - Ancient */
		strlcat(buffer, "\r\n", sizeof buffer);
...
void
BX_send_to_server(const char *format,...)
{
	va_list args;
...
/* send_to_server: sends the given info the the server */
void
BX_my_send_to_server(int refnum, const char *format,...)
{
	int old_from_server = from_server;
	va_list args;
...
void
BX_queue_send_to_server(int refnum, const char *format,...)
{
	int old_from_server = from_server;
	va_list args;
...
extern char *
BX_create_server_list(char *input)
{
	int i;
	int do_read = 0;
	char *value = NULL;
...
void
BX_server_disconnect(int i, char *args)
{
	char *message;
...
void
set_server_orignick(int server, char *nick)
{
	if (server <= -1 || server >= number_of_servers)
		return;
	if (nick)
...
char   *
get_server_orignick(int server)
{
	if (server <= -1 || server >= number_of_servers)
		return NULL;
...
void
nick_command_is_pending(int servnum, int value)
{
	if (servnum != -1)
...
void
orignick_is_pending(int servnum, int value)
{
	if (servnum != -1)
...
int
is_orignick_pending(int servnum)
{
	if (servnum != -1)
 ...
 */
void
BX_fudge_nickname(int servnum, int resend_only)
{
	...
	/*
	 * Process of fudging a nickname: If the nickname length is less
	 * then 9, add an underscore.
	 */
...
char   *
BX_get_pending_nickname(int servnum)
{
...
void
set_server_recv_nick(int server, const char *nick)
{
	if (server <= -1 || server >= number_of_servers)
		return;
	if (nick)
...
char   *
get_server_recv_nick(int server)
{
	if (server <= -1 || server >= number_of_servers)
		return NULL;
...
void
set_server_sent_nick(int server, const char *nick)
{
	if (server <= -1 || server >= number_of_servers)
		return;
	if (nick)
...
char   *
get_server_sent_nick(int server)
{
	if (server <= -1 || server >= number_of_servers)
		return NULL;
...
void
set_server_sent_body(int server, const char *msg_body)
{
	if (server <= -1 || server >= number_of_servers)
		return;
	if (msg_body)
...
char   *
get_server_sent_body(int server)
{
	if (server <= -1 || server >= number_of_servers)
		return NULL;
...
Sping  *
get_server_sping(int server, char *sname)
{
	if (server <= -1)
		return NULL;
	if (sname)
...
void
set_server_sping(int server, Sping * tmp)
{
	if (server <= -1) {
		new_free(&tmp->sname);
		new_free(&tmp);
...
void
clear_server_sping(int server, char *name)
{
	Sping *tmp = NULL, *next;
...
ChannelList *
BX_get_server_channels(int server)
{
	if (server <= -1 || server > number_of_servers)
		return NULL;
...
void
BX_set_server_last_ctcp_time(int server, time_t t)
{
	if (server <= -1 || server > number_of_servers)
		return;
...
time_t
BX_get_server_last_ctcp_time(int server)
{
	if (server <= -1 || server > number_of_servers)
		return 0;
...
void
BX_set_server_trace_flag(int server, int flag)
{
	if (server <= -1 || server > number_of_servers)
		return;
...
int
BX_get_server_trace_flag(int server)
{
	if (server <= -1 || server > number_of_servers)
		return 0;
...
int
BX_get_server_read(int server)
{
	return server_list[server].read;
}

void
BX_set_server_linklook(int server, int val)
{
	if (server <= -1 || server > number_of_servers)
		return;
...
int
BX_get_server_linklook(int server)
{
	if (server <= -1 || server > number_of_servers)
		return 0;
...
void
BX_set_server_stat_flag(int server, int val)
{
	if (server <= -1 || server > number_of_servers)
		return;
...
int
BX_get_server_stat_flag(int server)
{
	if (server <= -1 || server > number_of_servers)
		return 0;
...
time_t
BX_get_server_linklook_time(int server)
{
	return server_list[server].link_look_time;
}

void
BX_set_server_linklook_time(int server, time_t t)
{
	server_list[server].link_look_time = t;
}

void
BX_set_server_trace_kill(int server, int val)
{
	if (server <= -1 || server > number_of_servers)
		return;
...
int
BX_get_server_trace_kill(int server)
{
	if (server <= -1 || server > number_of_servers)
		return 0;
...
void
BX_add_server_channels(int server, ChannelList * chan)
{
	if (server <= -1 || server > number_of_servers)
		return;
	if (chan)
...
void
BX_set_server_channels(int server, ChannelList * chan)
{
	if (server <= -1 || server > number_of_servers)
		return;
	if (chan)
...
void
set_server_try_once(int s, int val)
{
...
void
set_server_reconnecting(int s, int val)
{
	if (s > -1 && s < number_of_servers)
...
void
set_server_reconnect(int s, int val)
{
	if (s > -1 && s < number_of_servers) {
...
void
set_server_old_server(int s, int val)
{
	if (s > -1 && s < number_of_servers)
...
void
set_server_req_server(int s, int val)
{
	if (s > -1 && s < number_of_servers)
...
void
set_server_retries(int s, int val)
{
	if (s > -1 && s < number_of_servers)
...
void
set_server_change_refnum(int s, int val)
{
	if (s > -1 && s < number_of_servers)
...
int
get_server_change_pending(int s)
{
	if (s > -1 && s < number_of_servers)
...
irc_server *
add_server(irc_server ** serv1, char *channel, char *arg, int hops, time_t t)
{
	irc_server *serv2;
...
void
remove_split_server(int type, char *server)
{
	irc_server *temp;
	irc_server **s;
...
void
clean_split_server_list(int type, time_t len)
{
...
int
is_server_valid(char *name, int server)
{
	if (server > -1 && server < number_of_servers &&
...
WhoEntry *
who_queue_top(int server)
{
	return server_list[server].who_queue;
}

void
set_who_queue_top(int server, WhoEntry * item)
{
	server_list[server].who_queue = item;
}

IsonEntry *
ison_queue_top(int server)
{
	return server_list[server].ison_queue;
}

void
set_ison_queue_top(int server, IsonEntry * ison)
{
	server_list[server].ison_queue = ison;
}

UserhostEntry *
userhost_queue_top(int server)
{
	return server_list[server].userhost_queue;
}

void
set_userhost_queue_top(int server, UserhostEntry * item)
{
	server_list[server].userhost_queue = item;
}

Server *
BX_get_server_list(void)
{
	return server_list;
}

int
get_server_local_port(int gsp_index)
{
	if (gsp_index == -1)
		gsp_index = primary_server;
	else if (gsp_index >= number_of_servers)
...
get_server_local_addr(int servnum)
{
	return server_list[servnum].local_addr;
}

struct sockaddr_foobar
get_server_uh_addr(int servnum)
{
...
void
BX_send_msg_to_channels(ChannelList * channel, int server, char *msg)
{
	int serv_version;
	char *p = NULL;
	...
	/*
	 * Because of hybrid and it's removal of , targets we need to detect
	 * this and get around it..
	 */
	serv_version = get_server_version(server);
	if (serv_version == Server2_8hybrid6) {
		/*
		 * this might be a cause for some flooding however. so we
		 * use the server queue. Which will help alleviate some
		 * flooding from the client.
		 */
...
void
BX_send_msg_to_nicks(ChannelList * chan, int server, char *msg)
{
	int serv_version;
	char *p = NULL;
	NickList *n = NULL;
