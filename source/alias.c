...
void
prepare_alias_call(void *al, char **stuff)
{
 ...
 */
void
add_var_alias(char *name, char *stuff)
{
	char *ptr;
	Alias *tmp = NULL;
	int af;
...
void
add_local_alias(char *name, char *stuff)
{
	char *ptr;
	Alias *tmp = NULL;
	AliasSet *list = NULL;
...
void
add_cmd_alias(char *name, ArgList * arglist, char *stuff)
{
	Alias *tmp = NULL;
	int cnt, af, loc;
...
void
add_var_stub_alias(char *name, char *stuff)
{
	Alias *tmp = NULL;
	char *ptr;
	int af;
...
void
add_cmd_stub_alias(char *name, char *stuff)
{
	Alias *tmp = NULL;
	int cnt, af;
...
static void
resize_cache(AliasSet * set, int newsize)
{
	int c, d;
...
static Alias *
find_cmd_alias(char *name, int *cnt)
{
	Alias *item = NULL;
	int loc;
	int i;
...
unstub_alias(Alias * item)
{
	...
	/*
	 * If already_looking is 1, then we are un-stubbing this alias
	 * because we are loading a file that presumably it must be in. So
	 * we dont load it again (duh).
	 */
	if (already_looking)
		return;
 ...
 */
static Alias *
find_local_alias(char *name, AliasSet ** list)
{
	Alias *alias = NULL;
	int c = 0;
	char *ptr;
...
static void
delete_all_var_alias(char *name)
{
	Alias *item;
	int i;
	int count = 0;
...
void
delete_var_alias(char *name, int owd)
{
	Alias *item;
	int i;
...
static void
delete_cmd_alias(char *name, int owd)
{
	Alias *item;
	int i;
...
static char *
get_variable_with_args(const char *str, const char *args, int *args_flag)
{
	Alias *alias = NULL;
	char *ret = NULL;
	char *name = NULL;
...
char   *
get_cmd_alias(char *name, int *howmany, char **complete_name, void **args)
{
	Alias *item;
 ...
 */
char  **
glob_cmd_alias(char *name, int *howmany)
{
	int cnt;
	int cmp;
	int len;
 ...
 */
char  **
glob_assign_alias(char *name, int *howmany)
{
	int cnt;
	int cmp;
	int len;
 ...
 */
char  **
pmatch_cmd_alias(char *name, int *howmany)
{
	int cnt;
	char **matches = NULL;
	int matches_size = 5;
 ...
 */
char  **
pmatch_assign_alias(char *name, int *howmany)
{
	int cnt;
	char **matches = NULL;
	int matches_size = 5;
 ...
 */
char  **
get_subarray_elements(char *root, int *howmany, int type)
{
	AliasSet *as;                   /* XXXX */
...
char   *
parse_line_alias_special(char *name, char *what, char *args, int d1, int d2, void *arglist, int function)
{
...
char   *
parse_line_with_return(char *name, char *what, char *args, int d1, int d2)
{
 ...
 */
char   *
call_user_function(char *alias_name, char *args)
{
	char *result = NULL;
	char *sub_buffer;
	int cnt;
...
/* XXX Ugh. */
void
call_user_alias(char *alias_name, char *alias_stuff, char *args, void *arglist)
{
...
void
save_assigns(FILE *fp, int do_all)
{
	int cnt = 0;
...
destroy_aliases(int type)
{
	...
	/*
	 * The above function gives null pointer derefs because we don't do
	 * proper checking. XXX
	 */
	if (my_array) {
...
void
set_current_command(char *line)
{
	call_stack[wind_index].current = line;
}

void
unset_current_command(void)
{
	call_stack[wind_index].current = NULL;
}

void
BX_lock_stack_frame(void)
{
	call_stack[wind_index].locked = 1;
}

void
BX_unlock_stack_frame(void)
{
...
#if 0
char   *
lookup_member(char *varname, char *var_args, char *ptr, char *args)
{
