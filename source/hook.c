 ...
 */
static void
add_numeric_hook(int numeric, char *nick, char *stuff, Noise noisy, int not, int sernum, int flexible)
{
	NumericList *entry;
	Hook *new;
...
#ifdef WANT_DLL
static void
add_numeric_dll_hook(int numeric, Noise noise, int serial, char *nick, char *package, int (*func)(int, char *, char **))
{
	NumericList *entry;
	Hook *new;
...
void    add_dll_hook(int which, Noise noise, char *nick, char *package, int (*func1)(int, char *, char **),
 ...
 */
static void
add_hook(int which, char *nick, char *stuff, Noise noisy, int not, int sernum, int flexible)
{
	Hook *new;
...
/* * * * * * REMOVING A HOOK * * * * * * * */
static void
remove_numeric_hook(int numeric, char *nick, int sernum, int quiet)
{
	NumericList *hook;
	Hook *tmp, *next;
...
static void
remove_hook(int which, char *nick, int sernum, int quiet)
{
	Hook *tmp, *next;
...
int
BX_do_hook(int which, char *format,...)
{
	Hook *tmp = NULL, *next = NULL, **list;
...
do_stack_on(int type, char *args)
{
	...
	if (type == STACK_PUSH) {
		...
		if (which < 0) {
			/*
			 * look -- do we have any hooks already for this
			 * numeric?
			 */
			if (!(nptr = find_numeric_list(-which))) {
				if (p->list) {  /* not just a placeholder? */
					/*
					 * No. make a new list and put the
					 * stack on it
					 */
...
void
hook_add_to_list(Hook ** list, Hook * item)
{
	Hook *tmp, *last = NULL;
...
static Hook *
hook_remove_from_list(Hook ** list, char *item, int sernum)
{
	Hook *tmp, *last = NULL;
...
static void
add_numeric_list(NumericList * item)
{
...
static NumericList *
find_numeric_list(int numeric)
{
	NumericList *tmp = NULL;
...
static NumericList *
remove_numeric_list(int numeric)
{
	NumericList *tmp, *last = NULL;
...
#if 0
static NumericList *
remove_numeric_dll_list(int numeric)
{
	NumericList *tmp, *last = NULL;
...
static Hook *
remove_dll_hook_list(Hook ** entry)
{
	Hook *tmp, *last = NULL;
