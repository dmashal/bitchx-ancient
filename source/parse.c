...
#include "irc.h"
static char cvsrevision[] = "$Id: parse.c 114 2011-03-31 22:20:21Z keaston $";
...
BanList *
eban_is_on_channel(char *ban, ChannelList * chan)
{
	BanList *eban = NULL;
...
check_auto_reply(char *str)
{
	...
	if (!str || !*str || !get_int_var(AUTO_RESPONSE_VAR))
		return 0;
	size = strlen(auto_str +1);
	p = alloca(size);
	strlcpy(p, auto_str, sizeof p);
	if (p && *p) {
...
int
annoy_kicks(int list_type, char *to, char *from, char *ptr, NickList * nick)
{
	int kick_em = 0;
	ChannelList *chan;
...
char   *
BX_PasteArgs(char **Args, int StartPoint)
{
	int i;
 ...
 */
int
BX_BreakArgs(char *Input, char **Sender, char **OutPut, int ig_sender)
{
	int ArgCount = 0;
...
p_wallops(char *from, char **ArgList)
{
	...
	if (from_server || check_flooding(from, WALLOP_FLOOD, line, NULL)) {
		/*
		 * The old server check, don't use the whois stuff for
		 * servers
		 */
		int level;
		char *high;
...
{
	int level, list_type, flood_type, log_type, ar_true = 0, no_flood = 1, do_beep = 0;

	unsigned char ignore_type;

	char *ptr = NULL, *to, *high;
...
void
add_user_who(WhoEntry * w, char *from, char **ArgList)
{
	char *userhost;
	ChannelList *chan;
	int op = 0, voice = 0;
	...
	size = strlen(ArgList[1]) + strlen(ArgList[2]) + 2;
	userhost = alloca(size);
	snprintf(userhost, size, "%s@%s", ArgList[1], ArgList[2]);
...
void
add_user_end(WhoEntry * w, char *from, char **ArgList)
{
...
p_channel(char *from, char **ArgList)
{
	...
	*extra = 0;
	...
	if (tmpnick && !tmpnick->ip && get_int_var(AUTO_NSLOOKUP_VAR)) {
		...
		strlcpy(user, FromUserHost, sizeof user);
...
void
check_auto_join(int server, char *from, char *channel, char *key)
{
	ChannelList *chan = NULL;
...
static int
check_mode_change(NickList * nick, char type_mode, char *from, char *this_nick, char *channel)
{
...
static void
check_bitch_mode(char *from, char *uh, char *channel, char *line, ChannelList * chan)
{
	NickList *nick;
	char *new_mode = NULL;
	char *n = NULL;
	...
	if (target && line) {
		...
		if (is_channel(target)) {
			...
			if (chan && get_cset_int_var(chan->csets, COMPRESS_MODES_CSET)) {
				...
				if (tmpbuf)
					strlcpy(line, tmpbuf, sizeof line);
				else
					flag = IGNORED;
			}
#endif
			...
			if (flag != IGNORED && do_hook(MODE_LIST, "%s %s %s", from, target, line)) {
				/*
				 * User mode changes where from != target
				 * don't occur on standard servers, but are
				 * used by services on some networks.
				 */
...
p_kick(char *from, char **ArgList)
{
	...
	if (channel && who && chan) {
		...
		if (!my_stricmp(who, get_server_nickname(from_server))) {
			...
			/*
			 * if it's me that's doing the kick don't flood
			 * check
			 */
			if (!itsme) {
...
static void
rfc1459_odd(char *from, char *comm, char **ArgList)
{
	PasteArgs(ArgList, 0);
...
static void
p_rpong(char *from, char **ArgList)
{
	if (!ArgList[3]) {
		PasteArgs(ArgList, 0);
...
parse_server(char *orig_line)
{
	...
	if (!(comm = (*ArgList++)) || !from || !*ArgList)
		return;                 /* Serious protocol violation --
					 * ByeBye  */
#ifdef WANT_TCL
