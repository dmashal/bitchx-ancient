...
static int
safe_puts(char *str, int len, int echo)
{
	int i = 0;
 ...
 */
extern void
BX_update_input(int update)
{
	int old_zone;
	char *ptr, *ptr_free;
	int len, free_it = 0, echo = 1, max;
 ...
 */
void
BX_set_input_prompt(Window * win, char *prompt, int unused)
{
	if (prompt)
...
NickList *
BX_lookup_nickcompletion(ChannelList * chan, char *possible)
{
...
BUILT_IN_KEYBINDING(send_line)
{
	...
	if (last_input_screen->promptlist && last_input_screen->promptlist->type == WAIT_PROMPT_LINE) {
		...
		if (line && (*line != *cmdchar) && get_int_var(NICK_COMPLETION_VAR) && !current_window->query_nick) {
			...
			/*
			 * this is for people with old BitchX.sav files that
			 * set it to '\0'
			 */
...
char   *
BX_getchannick(char *oldnick, char *nick)
{
	ChannelList *chan;
	...
	if (channel) {
		...
		/*
		 * we've never been here before so return first nick user
		 * hasn't entered anything on the line.
		 */
		...
		if (nick && in_completion == STATE_NORMAL) {
			/*
			 * if oldnick was the last one in the channel cnick
			 * will be NULL;
			 */
			if (!cnick) {
...
#if 1
				/*
				 * Same as before, make sure bx gets its try
				 * w/o ours interfering...
				 */
...
void
BX_addtabkey(char *nick, char *type, int which)
{
	NickTab *tmp, *new;
...
NickTab *
BX_gettabkey(int direction, int which, char *nick)
{
	NickTab *tmp, *new;
...
NickTab *
BX_getnextnick(int which, char *input_nick, char *oldnick, char *nick)
{
	ChannelList *chan;
	NickList *cnick = NULL;
	...
	if (tmp && (in_completion == STATE_NORMAL || in_completion == STATE_TABKEY)) {
		...
		if (oldnick) {
			...
			/*
			 * nick was not in the list. oops didn't come from
			 * here
			 */
...
enum tab_types {
	MP3 = 0, DCC, EXEC, LS, LOAD, SERVER, CDCC, IMSG, KB, CHDIR
};
...
char   *
get_home_dir(char *possible, int *count)
{
#ifdef HAVE_GETPWENT
	struct passwd *pw;
	char *booya = NULL;
...
char   *
get_completions(enum completion type, char *possible, int *count, char **suggested)
{
	char *booya = NULL;
	char *path = NULL;
	char *path2, *freeme;
	...
	case TABKEY_COMPLETION:
	{
		...
		*count = 0;
		...
		if (((*count) == 0) || ((*count) == 1)) {
			/*
			 * nothing specified for a match. so grab
			 * the first one, if available. or, only one
			 * match was available.
			 */
			if (possible && count == 0)
				possible = NULL;
...
#elif defined(WINNT)                    /* Might need something for WinNT
					 * itself C:/WINNT/SYSTEM32/ *.EXE */
 ...
 */
void
set_input_best(enum completion type, char *inp, char *old, int count, char **completes)
{
	int i, c, j = 0, match = 0;
...
BUILT_IN_KEYBINDING(tab_completion)
{
	...
	/*
	 * is this the != second word, then just complete from the channel
	 * nicks. if it is the second word, grab the first word, and this is
	 * used to determine what type of nick completion will be done.
	 */
...
int
BX_add_completion_type(char *name, int len, enum completion type)
{
	Ext_Name_Type *new;
