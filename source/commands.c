...
void
handle_dcc_chat(UserhostItem * stuff, char *nick, char *args)
{
...
char   *
glob_commands(char *name, int *cnt, int pmatch)
{
	IrcCommand *var = NULL;
	char *loc_match;
	char *mylist = NULL;
 ...
 */
IrcCommand *
BX_find_command(char *com, int *cnt)
{
	IrcCommand *retval;
	int loc;
...
#ifdef WANT_DLL
IrcCommandDll *
find_dll_command(char *com, int *cnt)
{
	int len = 0;
...
BUILT_IN_COMMAND(realname_cmd)
{
	...
	if (*args) {
		...
		say("Randomly selected a new ircname: %s", realname);
	}
}
 ...
 */
int
nick_in_scan(NickList * nick, enum SCAN_TYPE scan_type, char *mask)
{
	switch (scan_type) {
	case SCAN_VOICES:
...
BUILT_IN_COMMAND(echocmd)
{
	...
	int old_und = 0, old_rev = 0, old_bold = 0, old_color = 0, old_blink = 0, old_ansi = 0;
	int xtended = 0;
	...
	if (command && *command == 'X') {
		while (more && args && (*args == '-' || *args == '/')) {
			...
			case '*':
				...
				all_windows = 1;
				break;
			case 'K':       /* X -- allow all attributes to be
					 * outputted */
			{
				next_arg(args, &args);
...
void
really_save(char *ircrc_file, int flags, int save_all, int save_append)
{
	FILE *fp;
	char *mode[] = {"w", "a"};
...
AJoinList *
add_to_ajoin_list(char *channel, char *args, int type)
{
...
BUILT_IN_COMMAND(auto_join)
{
	...
	/*
	 * If the user used the ignore ajoin switch and we aren't in
	...
	if ((chan = next_arg(args, &args)) != NULL) {
		...
		if (!my_strnicmp(chan, "-i", 2)) {
			...
			if (!is_server_connected(from_server))
				...
				/*
				 * XXXX - right here we want to check to see
				 * if the channel is bound to this window.
				 * if it is, we set it as the default
				 * channel.  If it is not, we warn the user
				 * that we cant do it
				 */
 ...
 */
void
add_address(Virtuals ** vhost_list, int norev, struct sockaddr *sa)
{
	...
	/*
	 * Ignore INADDR_ANY, loopback and link-local addresses (see RFC
	 * 2373)
	 */
...
void
add_addrs_getifaddrs(Virtuals ** vhost_list, int norev)
{
 ...
 */
void
add_addrs_proc_net(Virtuals ** vhost_list, int norev)
{
	FILE *fptr;
 ...
 */
void
add_addrs_ioctl(Virtuals ** vhost_list, int norev)
{
	int s;
	size_t len;
	char *buf = NULL;
 ...
 */
void
get_local_addrs(Virtuals ** vhost_list, int norev)
{
	*vhost_list = NULL;
...
BUILT_IN_COMMAND(send_kill)
{
	...
	if ((reason = strchr(args, ' ')) != NULL)
		*reason++ = '\0';
	else if ((reason = get_reason(args, r_file)))
		reason = empty_string;
	new_free(&r_file);
...
{
	char *kickee, *comment, *channel = NULL;
	ChannelList *chan;
	int server = from_server;
...
void
redirect_text(int to_server, const char *nick_list, const char *text, char *command, int hook, int log)
{
 ...
 */
void
BX_send_text(const char *nick_list, const char *text, char *command, int hook, int log)
{
	int i, old_server, not_done = 1, is_current = 0, old_window_display = window_display;
	char *current_nick, *next_nick, *free_nick, *line, *key = NULL;
	...
	for (i = 0; i < 4; i++) {
		...
		/* This causes clang to warn on an empty body XXX */
		if (hook)
			blah_hook = do_hook(target[i].hook_type, "%s %s", target[i].nick_list, copy);

		/*
		 * supposedly if this is called before the do_hook it can
		 * cause a problem with scripts.
		 */
...
char   *function_eleet(char *);
char   *function_annoy(char *);
char   *function_cool(char *);
char   *function_cool2(char *);
...
BUILT_IN_COMMAND(do_send_text)
{
	...
	if (command) {
		...
		if (args && *args == '-' && *(args + 1)) {
			...
			case 'e':
				...
				new_text = function_eleet(args);
				break;
			case 'c':
				comm = next_arg(args, &args);
				new_text = function_cool(args);
				break;
			case 'l':
				comm = next_arg(args, &args);
				new_text = function_cool2(args);
				break;
			case 'a':
				comm = next_arg(args, &args);
				new_text = function_annoy(args);
			default:
				break;
			}
		}
	...
	if (cmd) {
		...
		if (dll && (dllcnt == -1 || dllcnt == 1))
			dll->func(dll_commands, dll->server_func, new_text ? new_text : text, NULL, dll->help);
		else
#endif
		if (!my_stricmp(cmd, "SAY") || !my_stricmp(cmd, "PRIVMSG") || !my_stricmp(cmd, "MSG"))
			send_text(tmp, new_text ? new_text : text, NULL, 1, 1);
		else if (!my_stricmp(cmd, "NOTICE"))
			send_text(tmp, new_text ? new_text : text, "NOTICE", 1, 1);
		else if (!my_strnicmp(cmd, "WALLO", 5))
			e_wall("WALLOPS", new_text ? new_text : text, NULL, NULL);
		else if (!my_strnicmp(cmd, "SWALLO", 6))
			e_wall("SWALLOPS", new_text ? new_text : text, NULL, NULL);
		else if (!my_strnicmp(cmd, "WALLC", 5))
			ChanWallOp(NULL, new_text ? new_text : text, NULL, NULL);
#ifndef BITCHX_LITE
		else if (!my_stricmp(cmd, "CSAY"))
			csay(NULL, new_text ? new_text : text, NULL, NULL);
		else if (!my_stricmp(cmd, "CMSG"))
			cmsg(NULL, new_text ? new_text : text, NULL, NULL);
#endif
		else
			send_text(tmp, new_text ? new_text : text, NULL, 1, 1);
	} else
...
command_completion(char unused, char *not_used)
{
	...
	int cmd_cnt = 0, alias_cnt = 0, dll_cnt = 0, function_cnt = 0, i,
	    c;
	char **aliases = NULL;
	char **functions = NULL;

	char *line = NULL, *com, *cmdchars, *rest, firstcmdchar[2] = "/";
	IrcCommand *command;
 ...
 */
void
BX_parse_line(const char *name, char *org_line, const char *args, int hist_flag, int append_flag, int handle_local)
{
	char *line = NULL, *stuff, *s, *t;
 ...
 */
int
BX_parse_command(char *line, int hist_flag, char *sub_args)
{
	static unsigned int level = 0;
	int display, old_display_var;
	char *cmdchars;
	const char *com;
	char *this_cmd = NULL;
...
#define MAX_LINE_LEN 5 * BIG_BUFFER_SIZE
	...
	while (args && *args && (filename = next_arg(args, &args))) {
		 ...
		 * filename (via continue) but at the bottom of the loop we
		 * will exit the loop
		 */
		...
		if (!(fp = uzfopen(&expanded, irc_path, 0))) {
			...
			/*
			 * uzfopen emits an error if the file is not found,
			 * so we dont have to.
			 */
			window_display = 1;
#ifdef WANT_DLL
...
/* *_NOT_* attached, so dont "fix" it */
		{
			...
			for (;; load_level[load_depth].line++) {
				...
				while (start && *start) {
					...
					/*
					 * if no_semicolon is set, we will
					 * not attempt to parse this line,
					 * but will continue grabbing text
					 */
					if (no_semicolon)
						no_semicolon = 0;
		...
		new_free(&expanded);
		fclose(fp);
	}                               /* end of the while loop that allows
					 * you to load more then one file at a
					 * time.. */
...
BUILT_IN_COMMAND(e_pause)
{
	...
	/*
	 * I use comment here simply becuase its not going to mess with the
	 * arguments.
	 */
