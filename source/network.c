 ...
 */
int
socks4_connect(int s, int portnum, struct sockaddr_in *server)
{
	...
	if (write(s, &sock4_connect, 8 + strlen(sock4_connect.username) + 1) == -1) {
 ...
 */
int
socks5_connect(int s, int portnum, struct sockaddr_in *server)
{
	struct _sock_connect {
		char version;
		char type;
	...
	sock5_connect.authtype = AUTH_NONE;     /* AUTH_PASSWD, AUTH_GSSAPI,
						 * AUTH_CHAP */

	if (write(s, &sock5_connect, 4) == -1) {
		bitchsay("Cannot write to proxy: %s", strerror(errno));
		return 0;
	}
	memset(tmpbuf, 0, sizeof(tmpbuf));
	alarm(get_int_var(CONNECT_TIMEOUT_VAR));
	if (read(s, tmpbuf, sizeof(tmpbuf) - 1) == -1) {
		alarm(0);
...
int
handle_socks(int fd, struct sockaddr_in addr, char *host, int portnum)
{
 ...
 */
int
BX_connect_by_number(char *hostn, unsigned short *portnum, int service, int protocol, int nonblocking)
{
	int fd = -1;
...
#endif
		/*
		 * Even on an IPv6 client this opens up a IPv4 socket... for
		 * now. (Some OSes need two sockets to be able to accept
...
#ifdef PARANOID
		name.sin_port += (unsigned short)(bsd_arc4random() & 255);
#endif
...
int
lame_resolv(const char *hostname, struct sockaddr_foobar *buffer)
{
#ifdef IPV6
	struct addrinfo *res;
...
BX_lookup_host(const char *host)
{
...
extern char *
BX_one_to_another(const char *what)
{
 ...
 */
int
my_accept(int s, struct sockaddr *addr, socklen_t *addrlen)
{
	int retval;
