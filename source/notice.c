...
long oper_kills = 0, nick_collisions = 0, serv_fakes = 0, serv_unauth = 0,
     serv_split = 0, serv_rejoin = 0, serv_squits = 0, serv_connects = 0,
     client_connects = 0, serv_rehash = 0, client_exits = 0, serv_klines = 0,
     client_floods = 0, client_invalid = 0, stats_req = 0, client_bot = 0,
     client_bot_alarm = 0, oper_requests = 0;
...
static int
handle_oper_vision(const char *from, char *cline, int *up_status)
{
	...
	if (!strncmp(line, "Received KILL message for ", 26)) {
		...
		if (ircnet) {
			...
			if (conn)
				sscanf(p, "%s is %s from %s", for_, fr, temp);
			else
				sscanf(p, "%s was %s from %s", for_, fr, temp);
...
int
check_ignore_notice(char *from, char *to, unsigned long type, char *line, char **high)
{
	int flag;
...
void
get_nat_address(UserhostItem * stuff, char *nick, char *args)
{
	char *h;
...
void
got_initial_version_28(char **ArgList)
{
	char *server, *sversion, *channel_modes;
...
void
setup_ov_mode(int on, int hide, int log)
{
#ifdef WANT_OPERVIEW
...
unsigned long
ircop_str_to_flags(unsigned org_flags, char *str)
{
...
void
convert_swatch(Window * win, char *str, int unused)
{
	unsigned long flag;
	char *p;
...
void
set_operview_flags(int server, unsigned long flags, int neg)
{
...
BUILT_IN_COMMAND(s_watch)
{
	...
	if (flag != old_flags)
		...
		put_it("\t%s", buffer);
		/*
		 * ALL COLLIDE KILLS MISMATCH HACK IDENTD FAKES UNAUTHS
		 * CLIENTS TRAFFIC CRAP REHASH KLINE BOTS OPER SQUIT SERVER
		 * CONNECT FLOOD USER STATS NICK ACTIVEK NONE");
		 */
		return;
	}
