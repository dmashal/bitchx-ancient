...
check_server_sync(char *from, char **ArgList)
{
	...
	/*
	 * If we get a "permission denied" type numeric from a _remote_
	 * server, then it means the network is desyched.
	 */
...
handle_server_stats(char *from, char **ArgList, int comm)
{
	static int norm = 0, invisible = 0, services = 0, servers = 0, ircops = 0,
	           unknown = 0, chans = 0, local_users = 0, total_users = 0, services_flag = 0;
	int ret = 1;
	char *line;
...
void
whohas_nick(UserhostItem * stuff, char *nick, char *args)
{
...
void
change_alt_nick(int server, char *nick)
{
	char *t = NULL;
...
numbered_command(char *from, int comm, char **ArgList)
{
	...
	case 4:                 /* #define RPL_MYINFO           004 */
	{
		...
		if (do_hook(current_numeric, "%s %s", from, *ArgList))
			display_msg(from, ArgList);
		/*
		 * if the server kicks us, then we maybe should
		 * reconnect to all our previous channels.
		 */
...
#ifdef WANT_CHATNET
	case 310:                       /* RPL_WHOISUSER_OPER [or something]
					 * 310 on chatnet .. fixed. =] */
	{
		PasteArgs(ArgList, 7);
		reset_display_target();
...
#endif
			/*
			 * routine for getting ident and host by-tor
			 * really wrote this little routine,
			 * something else he did for me, thanks =]
			 */
			if ((p = (char *)strchr(ArgList[2], '@'))) {
				*p++ = '\0';
				size = strlen(ArgList[2]) + 1;
				ident = alloca(size);
				strlcpy(ident, ArgList[2], sizeof ident);
				for (; *p == ' '; )
					p++;
				host = p;
				if ((p = (char *)strchr(p, '\n')))
	...
	case 338:                       /* #define RPL_WHOISACTUALLY    338
					 * (hybrid, ratbox, bahamut) */
	case 378:                       /* #define RPL_WHOISHOST        378
					 * (unreal, freenode) */
	{
		if (ArgList[2]) {
			/*
			 * hybrid / ratbox: <nick> <ip> :actually
			 * using host
			 */
			if (do_hook(current_numeric, "%s %s %s %s", from, ArgList[0], ArgList[1], ArgList[2]))
			...
			/*
			 * Bahamut: <nick> :is actually user@host
			 * [ip]
			 */
			if (do_hook(current_numeric, "%s %s %s", from, ArgList[0], ArgList[1]))
...
/*		fudge_nickname(from_server, 1);*/
	...
	case 464:                       /* #define ERR_PASSWDMISMATCH   464 */
	{
		...
		/*
		 * If we get this numeric before registering, it
		 * means that our server password was wrong - so
		 * reprompt and reconnect.
		 *
		 * However if we're already registered, the server is
		 * trying to tell us something else (usually, OPER
		 * password wrong) so just pass it on to the user.
		 */
		if (is_server_connected(from_server)) {
			if (flag)
				display_msg(from, ArgList);
	...
	{
		/*
		 * hybrid / ratbox: <nick> :is in +g mode (server
		 * side ignore)
		 */
		if (do_hook(current_numeric, "%s %s %s", from, ArgList[0], ArgList[1]))
...
/*		case 242:		 #define RPL_STATSUPTIME      242 */
		...
		case 328:               /* #define RPL_CHANNELURL       328
					 * (atheme, bahamut, freenode) */
		{
		...
		case 329:               /* #define CREATION_TIME	329 */
		{
			...
			if (!ArgList[2])
				...
				sscanf(ArgList[2], "%lu", &ts);
				tme = ts;
				strlcpy(pts, ctime(&tme), sizeof pts);
				pts[strlen(pts) - 1] = '\0';
				ots[0] = 0;
				if (ArgList[3]) {
					sscanf(ArgList[3], "%lu", &ts);
					tme = ts;
					strlcpy(ots, ctime(&tme), sizeof ots);
				}
				ots[strlen(ots) - 1] = '\0';
				put_it("%s Channel %s was created at %s", numeric_banner(),
				       ArgList[0], this_sucks);
...
#define RPL_SERVLISTEND      235
		...
		{
			/*
			 * ratbox / unreal / freenode: <nick> :is
