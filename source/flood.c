...
void
get_flood_val(ChannelList * chan, int type, unsigned int *flood_count, unsigned int *flood_rate)
{
...
int
BX_is_other_flood(ChannelList * channel, NickList * tmpnick, int type, int *t_flood)
{
	time_t diff = 0, flood_time = 0;
	int doit = 0;
	int count = 0;
	unsigned int flood_rate = 0, flood_count = 0;
...
int
BX_check_flooding(char *nick, int type, char *line, char *channel)
{
	static int users = 0, pos = 0;
	time_t flood_time = now, diff = 0;

	Flooding *tmp;
	unsigned int flood_rate, flood_count;
...
void
check_ctcp_ban_flood(char *channel, char *nick)
{
	NickList *Nick = NULL;
	...
	if (chan && chan->have_op && get_cset_int_var(chan->csets, CTCP_FLOOD_BAN_CSET) && Nick) {
		if (!Nick->userlist || (Nick->userlist && !(Nick->userlist->flags & ADD_FLOOD))) {
			if (!nick_isop(Nick) || get_cset_int_var(chan->csets, KICK_OPS_CSET)) {
				...
				size = strlen(Nick->host) + 1;
				u = alloca(size);
				strlcpy(u, Nick->host, sizeof u);
				h = strchr(u, '@');
				*h++ = 0;
...
int
BX_flood_prot(char *nick, char *userhost, char *type, int ctcp_type, int ignoretime, char *channel)
{
	ChannelList *chan;
	NickList *Nick;
