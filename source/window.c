...
unsigned long who_level = LOG_CRAP;             /* Log level of message being
						 * displayed */

int in_window_command = 0;              /* set to true if we are in window().
					 * This is used if a put_it() is
					 * called within the window() command.
					 * We make sure all windows are fully
...
#ifdef DEFAULT_DOUBLE_STATUS_LINE
	new->double_status = DEFAULT_DOUBLE_STATUS_LINE;
#else
	if (new->refnum == 1)
		new->double_status = 1;
	else
 ...
 */
void
BX_delete_window(Window * window)
{
	...
	if (window->name)
		strlcpy(buffer, window->name, sizeof buffer - 1);
	else
 ...
 */
int
BX_traverse_all_windows(Window ** ptr)
{
	/*
...
void
BX_add_to_invisible_list(Window * window)
{
 ...
 */
Window *
BX_add_to_window_list(Screen * screen, Window * new)
{
 ...
 */
void
BX_remove_window_from_screen(Window * window)
{
 ...
 */
void
BX_recalculate_window_positions(Screen * screen)
{
	Window *tmp;
	int top;
 ...
 */
void
BX_move_window(Window * window, int offset)
{
	Window *tmp, *last;
	int win_pos, pos;
 ...
 */
void
BX_resize_window(int how, Window * window, int offset)
{
	Window *other;
 ...
 */
void
BX_rebalance_windows(Screen * screen)
{
	Window *tmp;
	int each, extra;
 ...
 */
void
BX_recalculate_windows(Screen * screen)
{
	int old_li = 1;
	int excess_li = 0;
	Window *tmp;
	...
	/*
	 * Expanding the screen takes two passes.  In the first pass, We
	 * figure out how many windows will be resized.  If none can be
	 * rebalanced, we add the whole shebang to the last one.
	 */
	for (tmp = screen->window_list; tmp; tmp = tmp->next) {
 ...
 */
void
BX_goto_window(Screen * s, int which)
{
	Window *tmp;
	int i;
...
void
BX_swap_last_window(char key, char *ptr)
{
...
void
BX_swap_next_window(char key, char *ptr)
{
...
void
BX_swap_previous_window(char key, char *ptr)
{
 ...
 */
char   *
BX_get_status_by_refnum(unsigned refnum, unsigned line)
{
	Window *the_window;
 ...
 */
Window *
BX_get_window_by_desc(const char *stuff)
{
	Window *w = NULL;
 ...
 */
Window *
BX_get_window_by_refnum(unsigned int refnum)
{
	Window *tmp = NULL;
 ...
 */
int
BX_get_visible_by_refnum(char *args)
{
	char *arg;
	Window *tmp;
 ...
 */
Window *
BX_get_window_by_name(const char *name)
{
	Window *tmp = NULL;
 ...
 */
void
BX_next_window(char key, char *ptr)
{
	Window *w;
 ...
 */
void
BX_previous_window(char key, char *ptr)
{
	Window *w;
 ...
 */
void
BX_update_window_status(Window * window, int refresh)
{
	if (!window)
		window = current_window;
 ...
 */
void
BX_update_all_status(Window * win, char *unused, int unused1)
{
	Window *window = NULL;
...
void
BX_update_window_status_all(void)
{
	Window *win = NULL;
 ...
 */
void
BX_set_prompt_by_refnum(unsigned int refnum, char *prompt)
{
	Window *tmp;
...
char   *
BX_get_prompt_by_refnum(unsigned int refnum)
{
	Window *tmp;
 ...
 */
char   *
BX_get_target_by_refnum(unsigned int refnum)
{
	Window *tmp;
...
char   *
BX_get_target_cmd_by_refnum(unsigned int refnum)
{
	Window *tmp;
...
Window *
BX_get_window_target_by_desc(char *name)
{
	Window *tmp = NULL;
 ...
 */
int
BX_is_current_channel(char *channel, int server, int delete)
{
 ...
 */
const char *
BX_set_current_channel_by_refnum(unsigned int refnum, char *channel)
{
	Window *tmp;
	char *oldc;
...
char   *
BX_get_current_channel_by_refnum(unsigned int refnum)
{
	Window *tmp;
...
char   *
BX_get_refnum_by_window(const Window * w)
{
...
int
BX_is_bound_to_window(const Window * window, const char *channel)
{
...
Window *
BX_get_window_bound_channel(const char *channel)
{
	Window *tmp = NULL;
	...
	while ((traverse_all_windows(&tmp))) {
		if (tmp->bind_channel) {
			...
			if (!channel)
				return tmp;
			/* Clang gives a warning about p being unused XXX. */
			q = p = LOCAL_COPY(tmp->bind_channel);
			/*
			 * Removing p here seems to break channel output,
			 * ie: (DjTranceFire/#cod4.wars) 3on3 server on!13
			 * (DjTranceFire/#cod4.wars) 3on3 server on!14
			 * annel #cod4.wars was created at Thu Mar 22
			 * 11:24:51 2007 (DjTranceFire/#cod4.wars) 3on3
			 * server on!15 (DjTranceFire/#cod4.wars) 3on3
			 * server on!16
			 */
			while ((p = next_in_comma_list(q, &q))) {
				if (!p || !*p)
...
int
BX_is_bound_anywhere(const char *channel)
{
	Window *tmp = NULL;
...
extern int
BX_is_bound(const char *channel, int server)
{
	Window *tmp = NULL;
...
void
BX_unbind_channel(const char *channel, int server)
{
	Window *tmp = NULL;
...
char   *
BX_get_bound_channel(Window * window)
{
 ...
 */
int
BX_get_window_server(unsigned int refnum)
{
	Window *tmp;
 ...
 */
void
BX_set_window_server(int refnum, int server, int misc)
{
...
void
BX_window_check_servers(int unused)
{
	Window *tmp;
 ...
 */
void
BX_change_window_server(int old_server, int new_server)
{
	Window *tmp = NULL;
 ...
 */
void
BX_set_level_by_refnum(unsigned int refnum, unsigned long level)
{
	Window *tmp;
 ...
 */
void
BX_message_to(unsigned long refnum)
{
 ...
 */
void
BX_save_display_target(const char **saved_from, unsigned long *saved_level)
{
	*saved_from = who_from;
...
void
BX_restore_display_target(const char *saved_from, unsigned long saved_level)
{
	who_from = saved_from;
 ...
 */
int
wordinlist(const char *word, const char *list)
{
	char *nw;
	int wl;
...
void
BX_set_display_target(const char *who, unsigned long level)
{
	Window *tmp;
...
void
BX_reset_display_target(void)
{
	set_display_target(NULL, LOG_CRAP);
}

void
BX_set_display_target_by_desc(char *desc)
{
	if (!desc)
		target_window = current_window;
...
void
set_display_target_by_winref(int refnum)
{
	Window *w;
 ...
 */
unsigned long
message_from_level(unsigned long level)
{
	unsigned long temp;
...
void
BX_clear_all_windows(int unhold, int scrollback)
{
	Window *tmp = NULL;
 ...
 */
void
BX_clear_window_by_refnum(unsigned int refnum)
{
	Window *tmp;
...
void
unclear_all_windows(int unhold, int visible, int hidden)
{
	Window *tmp = NULL;
...
void
BX_unclear_window_by_refnum(unsigned refnum)
{
	Window *tmp;
 ...
 */
void
BX_set_scroll_lines(Window * win, char *unused, int size)
{
	if (size == 0) {
		return;
 ...
 */
void
BX_set_continued_lines(Window * win, char *value, int unused)
{
...
unsigned int
BX_current_refnum(void)
{
	return current_window->refnum;
}

int
BX_number_of_windows_on_screen(Window * w)
{
 ...
 */
void
BX_set_scrollback_size(Window * w, char *unused, int size)
{
	Window *window = NULL;
...
char   *
BX_get_nicklist_by_window(Window * win)
{
 ...
 */
static Window *
window_create(Window * window, char **args, char *usage)
{
#ifdef WINDOW_CREATE
	Window *tmp;
 ...
 */
static Window *
window_delete(Window * window, char **args, char *usage)
{
#ifdef WINDOW_CREATE
	if (window->screen)
 ...
 */
static Window *
window_discon(Window * window, char **args, char *usage)
{
	if (window->server != -1)
 ...
 */
static Window *
window_split(Window * window, char **args, char *usage)
{
...
static Window *
window_triple(Window * window, char **args, char *usage)
{
	int booya = window->status_lines;
 ...
 */
static Window *
window_echo(Window * window, char **args, char *usage)
{
	const char *to_echo;
...
static Window *
window_new_hide(Window * window, char **args, char *usage)
{
	new_window(NULL);
	return window;
}
...
static Window *
window_noserv(Window * window, char **args, char *usage)
{
	window->server = -1;
	return window;
}
...
static Window *
window_refresh(Window * window, char **args, char *usage)
{
...
static Window *
window_scratch(Window * window, char **args, char *usage)
{
	int scratch = 0;
...
Window *
window_scroll(Window * window, char **args, char *usage)
{
	int scroll = 0;
...
static Window *
window_scrollback(Window * window, char **args, char *usage)
{
...
static Window *
window_status_special(Window * window, char **args, char *usage)
{
	char *arg;
...
static Window *
window_set(Window * window, char **args, char *usage)
{
...
static Window *
window_update(Window * window, char **args, char *usage)
{
...
static Window *
window_menu(Window * window, char **args, char *usage)
{
#ifdef GUI
...
#ifdef GUI
static Window *
window_setpos(Window * window, char **args, char *usage)
{
...
static Window *
window_font(Window * window, char **args, char *usage)
{
	char *tmp;
...
static Window *
window_nicklist(Window * window, char **args, char *usage)
{
	char *tmp;
	...
	if (!(arg = next_arg(*args, args))) {
		...
		for (i = 0; options[i].command; i++) {
			if (!my_stricmp(options[i].command, arg)) {
				snprintf(buffer, sizeof buffer, "WINDOW %s", arg);
...
void
BX_scrollback_backwards_lines(int lines)
{
...
void
BX_scrollback_forwards_lines(int lines)
{
	Window *window = current_window;
	Display *new_top = window->top_of_display;
	int new_lines = 0;
...
void
BX_scrollback_forwards(char dumb, char *dumber)
{
...
void
BX_scrollback_backwards(char dumb, char *dumber)
{
	int ratio = get_int_var(SCROLLBACK_RATIO_VAR);
	int lines;
...
void
BX_scrollback_end(char dumb, char *dumber)
{
	Window *window = current_window;
...
void
BX_scrollback_start(char dumb, char *dumber)
{
	Window *window = current_window;
 ...
 */
void
BX_set_hold_mode(Window * window, int flag, int update)
{
	if (window == NULL)
		window = current_window;
...
void
BX_unstop_all_windows(char dumb, char *dumber)
{
	Window *tmp = NULL;
 ...
 */
void
BX_reset_line_cnt(Window * win, char *unused, int value)
{
	win->hold_mode = value;
...
void
BX_toggle_stop_screen(char unused, char *not_used)
{
 ...
 */
void
BX_flush_everything_being_held(Window * window)
{
	Display *ptr, *save;
	int count;
 ...
 */
int
BX_unhold_a_window(Window * window)
{
...
void
BX_recalculate_window_cursor(Window * window)
{
	Display *tmp;
...
void
BX_set_screens_current_window(Screen * screen, Window * window)
{
	if (!window) {
...
void
BX_make_window_current(Window * window)
{
...
void
BX_clear_scrollback(Window * window)
{
...
void
make_to_window_by_desc(char *desc)
{
...
int
get_current_winref(void)
{
	return current_window->refnum;
}

int
get_winref_by_desc(const char *desc)
{
	Window *w;
...
void
make_window_current_by_desc(char *desc)
{
	Window *new_win = get_window_by_desc(desc);
...
void
make_window_current_by_winref(int refnum)
{
	Window *new_win;
