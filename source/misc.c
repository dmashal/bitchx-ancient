...
int
timer_unban(void *args, char *sub)
{
	char *p = (char *)args;
	char *channel;
...
int
timer_idlekick(void *args, char *sub)
{
...
BUILT_IN_COMMAND(channel_stats)
{
	...
	NickList *l;
	unsigned long nick_mem = 0, ban_mem = 0;
	BanList *b;
	...
	if (args && *args) {
		...
		if (my_strnicmp(channel, "-ALL", strlen(channel))) {
			...
			int stats_ops = 0, stats_dops = 0, stats_bans = 0,
			    stats_unbans = 0;
			int stats_topics = 0, stats_kicks = 0, stats_pubs = 0,
			    stats_parts = 0;
			int stats_signoffs = 0, stats_joins = 0;
			int total_nicks = 0, max_nicks = 0, total_bans = 0,
			    max_bans = 0;
			int stats_sops = 0, stats_sdops = 0, stats_sbans = 0,
			    stats_sunbans = 0;
...
void
update_stats(int what, NickList * nick, ChannelList * chan, int splitter)
{
			...
			if (chan->have_op && is_other_flood(chan, nick, JOIN_FLOOD, &t)) {
				if (get_cset_int_var(chan->csets, JOINFLOOD_CSET) && get_cset_int_var(chan->csets, KICK_ON_JOINFLOOD_CSET) && !nick->kickcount++) {
					...
					*host++ = 0;
					...
					send_to_server("KICK %s %s :\002Join flood\002 (%d joins in %dsecs of %dsecs)", chan->channel, nick->nick, get_cset_int_var(chan->csets, KICK_ON_JOINFLOOD_CSET)                /* chan->set_kick_on_join
																											   flood */, t, get_cset_int_var(chan->csets, JOINFLOOD_TIME_CSET));
...
/* modified Colten Edwards                                      */
	...
	unsigned char *sptr = line, *dptr = newline1;
	short code;
	...
	while (*sptr) {
		if (*sptr == '\x03') {
			...
			if (isdigit((unsigned char)*sptr)) {
				...
				/*
				 * ^C not followed by digit - assume end of
				 * color
				 */
				strcpy(dptr, "\x1b[0m");
				while (*dptr)
...
stripansicodes(const char *line)
{
	...
	while (*tstr) {
		/*
		 * Note that we use '\x9b' here, rather than 0x9b, because
		 * the former will have the correct value whether or not
		 * char is signed.
		 */
...
int
check_split(char *nick, char *reason)
{
...
void
clear_array(NickTab ** tmp, char *nick)
{
	NickTab *t, *q;
...
void
BX_userage(char *command, char *use)
{
	if (command) {
...
putlog(int type,...)
{
	...
	va_list va;
	time_t t;
	char *format, *chname, *logfilen = NULL, s[BIG_BUFFER_SIZE + 1], s1[40],
...
enum REDIR_TYPES {
	PRIVMSG = 0, KICK, TOPIC, WALL, WALLOP, NOTICE, KBOOT, KILL, DCC, LIST
};
void    userhost_ban(UserhostItem * stuff, char *nick1, char *args);

int
redirect_msg(char *to, enum REDIR_TYPES what, char *str, int showansi)
{
	char *new_str;
...
{
	if ((get_string_var(MSGLOGFILE_VAR) == NULL) || !get_string_var(CTOOLZ_DIR_VAR)) {
...
void
error_not_opped(const char *channel)
{
...
char   *
get_reason(char *nick, char *file)
{
	char *temp;
	char *filename = NULL;
...
void
auto_nslookup(struct reslist *rptr)
{
	NickList *nick;
	ChannelList *chan;
	struct in_addr ip;
...
void
print_ns_succede(struct reslist *rptr)
{
	char *u, *n, *h;
...
void
print_ns_fail(struct reslist *rptr)
{
	if (rptr->command) {
		char buffer[BIG_BUFFER_SIZE];
 ...
 */
void
cdns_generic_callback(DNS_QUEUE * dns)
{
...
void
ar_rename_nick(char *old_nick, char *new_nick, int server)
{
}
...
static int do_query_name(struct resinfo *, char *, struct reslist *, char *, char *, char *, char *, int, void (*func)(), char *);
static int do_query_number(struct resinfo *, char *, struct reslist *, char *, char *, char *, char *, int, void (*func)(), char *);
...
#ifdef HAVE_LIBIPHLPAPI
		/*
		 * The Cygwin resolver library doesn't fill out
		 * _res.nsaddr_list and sets _res.nscount to -1 if there's
		 * no /etc/resolv.conf file, so we try fetching the first
		 * DNS server address ourselves.
		 */
		if (_res.nscount < 1)
			ar_get_windows_dns();
#endif
 ...
 */
static int
ar_add_request(struct reslist *new)
{
	if (!new)
		return -1;
	if (!ar_first)
 ...
 */
static int
ar_remrequest(struct reslist *old)
{
 ...
 */
static struct reslist *ar_make_request(struct resinfo *resi, char *nick, char *user, char *h, char *chan, int server, void (*func)(), char *command){
	struct reslist *rptr;
	struct resinfo *rp;
 ...
 */
long
ar_timeout(time_t now, char *info, int size, void (*func)(struct reslist *))
{
	struct reslist *rptr, *r2ptr;
	long next = 0;
 ...
 */
static int
ar_send_res_msg(char *msg, int len, int rcount)
{
	int i;
	int sent = 0;
 ...
 */
int
ar_delete(char *ptr, int size)
{
	struct reslist *rptr;
	struct reslist *r2ptr;
	int removed = 0;
 ...
 */
static int
ar_query_name(char *name, int class, int type, struct reslist *rptr)
{
...
static int do_query_name(struct resinfo *resi, char *name, struct reslist *rptr, char *nick, char *user, char *h, char *chan, int server, void (*func)(), char *command){
	char hname[HOSTLEN];
	int len;
 ...
 */
static int do_query_number(struct resinfo *resi, char *numb, struct reslist *rptr, char *nick, char *user, char *h, char *chan, int server, void (*func)(), char *command){
	unsigned char *cp;
 ...
 */
static int
ar_resend_query(struct reslist *rptr)
{
	if (!rptr->re_resend)
		return -1;
 ...
 */
static int
ar_procanswer(struct reslist *rptr, HEADER * hptr, char *buf, char *eob)
{
	char *cp, **alias;
...
#ifndef __QNX__
	/*
	 * QNX doesn't seem to have this function, not sure what it does at
	 * the moment but I may change this in the future. - Brian
	 */
 ...
 */
struct hostent *
ar_answer(char *reip, int size, void (*func)(struct reslist *))
{
...
#ifdef WANT_NSLOOKUP
void
userhost_nsl(UserhostItem * stuff, char *nick, char *args)
{
	char *nsl = NULL;
...
cluster(char *hostname)
{
	...
	if (*host && isdigit((unsigned char)*(host + strlen(host) - 1))) {
		/*
		 * Thanks icebreak for this small patch which fixes this
		 * function
		 */
		int i;
		char *tmp;
		...
		*tmp = '\0';
		...
		/*
		 * We don't need strlcat for these first two, because at
		 * this point the maximum length of the string in result is
		 * 11
		 */
		strcat(result, "*");
...
#ifdef GUI
char   *
checkmenu(MenuList * check, int menuid)
{
	MenuList *tmpml;
	MenuStruct *tmpms;
	char *tmpalias;
...
unsigned long
BX_set_socketflags(int s, unsigned long flags)
{
	if (check_socket(s)) {
...
void
BX_set_socketinfo(int s, void *info)
{
	if (check_socket(s))
...
int     BX_add_socketread(int s, int port, unsigned long flags, char *server, void (*func_read)(int),
                          void (*func_write)(int))
{
...
int
BX_set_socketwrite(int s)
{
	if ((unsigned int) s > FD_SETSIZE)
		return -1;
	if (s > sock_manager.max_fd)
		sock_manager.max_fd = s;
...
void
BX_add_sockettimeout(int s, time_t timeout, void *func)
{
	if (timeout < 0) {
...
int
BX_write_sockets(int s, unsigned char *str, int len, int nl)
{
	size_t size;
...
int
BX_read_sockets(int s, unsigned char *str, int len)
{
	if (s < 1)
		return -1;
...
void
read_netfinger(int s)
{
	char tmpstr[BIG_BUFFER_SIZE + 1];
	unsigned char *p = tmpstr;
...
void
start_finger(UserhostItem * stuff, char *nick, char *args)
{
...
static int
scan(char *remote_host, int low_port, int high_port, struct sockaddr_foobar *host)
{
...
void
userhost_scanport(UserhostItem * stuff, char *nick, char *args)
{
	char *t;
...
void
userhost_ignore(UserhostItem * stuff, char *nick1, char *args)
{
	char *p, *arg;
...
int
do_newuser(char *command, char *args, char *subargs)
{
...
void
put_user(const NickList * nick, const char *channel)
{
	char *users_format;
...
BUILT_IN_COMMAND(users)
{
	...
	int count = 0, ops = 0, msg = 0, num_kicks = 0, hook = 0, not = 0, server = from_server,
...
int
caps_fucknut(unsigned char *crap)
{
...
int
char_fucknut(unsigned char *crap, char looking, int max)
{
...
static char *
make_timestamp(int do_timestamp, char *timestr)
{
...
char   *
convert_output_format_raw(const char *format, const char *str, va_list args)
{
		...
		if (*tmpc == '%') {
			...
			if (*tmpc == '%') {
				*s++ = *tmpc++;
				...
				this_color = (int)(((float)UNDERLINE_COLOR * bsd_arc4random()) / RAND_MAX);
			else if (*tmpc == 'P')
				this_color = MAGENTAB;
			else if (*tmpc == 'p')
				this_color = MAGENTA;
...
char   *
BX_convert_output_format(const char *format, const char *str,...)
{
	char *s;
...
char   *
convert_output_format2(const char *str)
{
...
void
add_last_type(LastMsg * array, int size, char *from, char *uh, char *to, char *str)
{
	int i;
...
int
check_last_type(LastMsg * array, int size, char *from, char *uh)
{
	int i;
...
ChannelList *
BX_prepare_command(int *active_server, char *channel, int flags)
{
	int server = 0;
