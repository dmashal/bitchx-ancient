...
void
log_denied(char *type, char *from, char *uh, char *to, char *cmd)
{
...
CTCP_HANDLER(do_sed)
{
	...
	if (!key || !ret)
		...
		/*
		 * There might be a CTCP message in there, so we see if we
		 * can find it.
		 */
 ...
 */
	 ...
	 * if they can track via IRCNAME from the /etc/passwd file...  We
	 * therefore need to either disable CTCP_FINGER or also make it
	 * customisable.  Let's do the latter because it invokes less
	 * suspicion in the long run -- Jake [WinterHawk] Khuon
	 */
	...
	if ((ctcpfinger = getenv("IRCFINGER")))
		...
		if (!pwd->pw_gecos)
			pwd->pw_gecos = "Prestigious BitchX user";
...
if ((ctcpuser = getenv("IRCUSER")))
	strlcpy(userbuff, ctcpuser, sizeof userbuff);
else if ((ctcpfinger = getenv("IRCFINGER")))
	strlcpy(pwd->pw_gecos, ctcpfinger, NAME_LEN);
...
CTCP_HANDLER(do_ping_reply)
{
	...
	snprintf(cmd, IRCD_BUFFER_SIZE / 2, "%5.3f seconds",
...
do_notice_ctcp(char *from, char *to, char *str)
{
	...
	tbuf = stripansi(str);
	strlcpy(local_ctcp_buffer, tbuf, IRCD_BUFFER_SIZE - 2);
	new_free(&tbuf);
	...
	for (;; strlcat(local_ctcp_buffer, last, sizeof local_ctcp_buffer)) {
		...
		/*
		 * If we've already parsed one, and there is a limit on this
		 * CTCP, then just punt it.
		 */
#ifdef WANT_DLL
	...
	/* Reset the window level/logging */
	reset_display_target();
	strlcpy(str, local_ctcp_buffer, sizeof str);
	return str;
}
...
#if 0 /* This doesn't appear in epic, and seems useless. heh */
 ...
 */
void
BX_split_CTCP(char *raw_message, char *ctcp_dest, char *after_ctcp)
{
