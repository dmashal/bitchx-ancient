...
#ifndef BITCHX_LITE
void
tandout_but(int idx, char *format,...)
{
	va_list args;
	char putbuf[BIG_BUFFER_SIZE + 1];
	SocketList *s;
...
void
chanout_but(int idx, char *format,...)
{
	va_list args;
	char putbuf[BIG_BUFFER_SIZE + 1];
	SocketList *s;
...
void
userhost_clink(UserhostItem * stuff, char *nick, char *args)
{
...
int
handle_tcl_chan(int idx, char *user, char *host, char *text)
{
...
int
tand_zapfbroad(int idx, char *par)
{
	char *from, *opcode;
...
int
handle_dcc_bot(int idx, char *param)
{
	char *code;
	int i = 0;
	int found = 0;
...
int
send_who_to(int idx, char *from, int arg)
{
...
void
do_command(int idx, char *from, char *par)
{
...
int
tand_command(int idx, char *args)
{
	char *from, *to, *p;
	char buffer[IRCD_BUFFER_SIZE + 1];
...
tell_whom(int idx, char *arg)
{
	...
	for (i = 0; i < get_max_fd() + 1; i++) {
		...
		if ((s->flags & DCC_TYPES) == DCC_CHAT && (s->flags & DCC_BOTCHAT)) {
			if (!found++)
				dcc_printf(idx, "^B%-10s^B  ^B%-10s^B  ^B%-25s^B\n", "Who", "Bot", "Host");
...
cmd_ops(int idx, char *par)
{
	...
	if (n && n->ul) {
		...
		if (!nick || !ch || !ch->have_op) {
			dcc_printf(idx, "No Nick specified, not on that channel or not channel op\n");
			return TCL_ERROR;
		}
...
void
invite_dcc_chat(UserhostItem * stuff, char *nick, char *args)
{
	char *id;
	int idx = -1;
