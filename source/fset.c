...
void
add_new_fset(char *name, char *args)
{
	if (args && *args) {
...
char   *
BX_fget_string_var(enum FSET_TYPES var)
{
	IrcVariable *tmp = NULL;
...
void
BX_fset_string_var(enum FSET_TYPES var, char *value)
{
	if (value && *value)
...
static int
find_fset_variable(IrcVariable * array, char *org_name, int *cnt)
{
	IrcVariable *v, *first;
	int var_index;
	char *name = NULL;
...
static void
set_fset_var_value(int var_index, char *name, char *value)
{
...
static inline void
fset_variable_case1(char *name, int var_index, char *args)
{
...
void
create_fsets(Window * win, int ansi)
{
#ifdef VAR_DEBUG
	int i;
...
IrcVariable *
fget_var_address(char *var_name)
{
	IrcVariable *var = NULL;
	char *tmp_var;
	int cnt, msv_index;
	...
	size = strlen(var_name) + 1;

	if ((tmp_var = alloca(size)) == NULL)
		ircpanic("Failed to allocate memory for fget_var_address!");  /* alloc failed, die quickly! */

	strlcpy(tmp_var, var_name, sizeof tmp_var);
	upper(tmp_var);
...
char   *
make_fstring_var(const char *var_name)
{
	IrcVariable *var = NULL;
	int cnt, msv_index;
	char *ret = NULL;
...
#ifdef WANT_TCL
char   *
fset_rem_str(ClientData * cd, Tcl_Interp * intp, char *name1, char *name2, int flags)
{
	char *s;
	IrcVariable *n;
 ...
 */
int
compare_number(List * item1, List * item2)
{
...
void
add_numeric_fset(char *name, int remove, char *args, int verbose)
{
