...
int
delay_flush_all(void *arg, char *sub)
{
...
static void
shitlist_erase(ShitList ** clientlist)
{
	ShitList *Client, *tmp;
...
char   *
ban_it(char *nick, char *user, char *host, char *ip)
{
...
void
userhost_unban(UserhostItem * stuff, char *nick1, char *args)
{
	char *tmp;
	ChannelList *chan;
	char *channel = NULL;
...
void
userhost_ban(UserhostItem * stuff, char *nick1, char *args)
{
	char *str = NULL;
	char *channel;
	ChannelList *c = NULL;
...
BUILT_IN_COMMAND(massop)
{
	...
	int maxmodes = get_int_var(NUM_OPMODES_VAR), count,
	    massvoice = 0;
	int server = 0;
...
BUILT_IN_COMMAND(masskick)
{
	...
	if (masskick_list) {
		...
		*reason = 0;
		...
		snprintf(buf, sizeof buf, "KICK %%s %%s :\002%s\002", reason);
		len = strlen(buf);
...
BUILT_IN_COMMAND(masskickban)
{
	...
	count = 0;
	if (!strchr(spec, '!')) {
		strlcpy(tempbuf, "*!", sizeof tempbuf);
		if (!strchr(spec, '@'))
			strlcat(tempbuf, "*@", sizeof tempbuf);
		strlcat(tempbuf, spec, sizeof tempbuf);
	} else
		strlcpy(tempbuf, spec, sizeof tempbuf);
...
{
	char *to = NULL, *tspec = NULL, *spec = NULL, *tnick = NULL, *rest = NULL;
	...
	strlcpy(tspec, spec, sizeof tspec);
