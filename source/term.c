...
#endif
	/*
	 * This mangles all the unprintable control characters except for
	 * the escape character.  This will only support escape sequences
	 * that use ESC + any normal, printable characters (this is an
	 * assumption throughout the program). While you can use a non-vt100
	 * emulation, the output will probably be mangled in several places.
	 */
	...
	*termcap2 = 0;
	...
	for (i = 0; i < 16; i++) {
		...
		if (current_term->TI_setaf)
			strlcat(cbuf, (char *)tparm(current_term->TI_setaf, i & 0x07, 0), sizeof cbuf);
		else if (current_term->TI_setf)
			strlcat(cbuf, (char *)tparm(current_term->TI_setf, i & 0x07, 0), sizeof cbuf);
		else if (i >= 8)
 ...
 */
void
term_repeat(unsigned char c, int rep)
{
	if (current_term->TI_rep)
 ...
 */
void
term_scroll(int top, int bot, int n)
{
#ifdef GUI
	...
	pair[1] = 7;
	if (n > 0)
		VioScrollUp(top, 0, bot, current_term->TI_cols, n, (PBYTE) &pair, (HVIO) vio_screen);
	else if (n < 0) {
		n = -n;
		VioScrollDn(top, 0, bot, current_term->TI_cols, n, (PBYTE) &pair, (HVIO) vio_screen);
	}
#ifndef __EMXX__
	return;
#endif
	...
	/*
	 * It seems that Windows 95/98 has a problem with scrolling when you
	 * set the scroll area to something other than the default, using
	 * current_term->TI_csr, so this API call will scroll the text
	 * instead.
	 */
	...
	if (current_term->TI_csr && (current_term->TI_ri || current_term->TI_rin) && (current_term->TI_ind || current_term->TI_indn)) {
		 ...
		 */
		strlcpy(start, (char *)tparm(current_term->TI_csr, top, bot), sizeof start);
		strlcpy(final, (char *)tparm(current_term->TI_csr, 0, current_term->TI_lines - 1), sizeof final);
		...
		if (n > 0) {
			...
			if (current_term->TI_indn) {
				oneshot = 1;
				strlcpy(thing, (char *)tparm(current_term->TI_indn, rn, rn), sizeof thing);
			} else
			...
			if (current_term->TI_rin) {
				oneshot = 1;
				strlcpy(thing, (char *)tparm(current_term->TI_rin, rn, rn), sizeof thing);
			} else
		...
		strlcpy(start, (char *)tparm(current_term->TI_wind, top, bot, 0, current_term->TI_cols - 1), sizeof start);
		strlcpy(final, (char *)tparm(current_term->TI_wind, 0, current_term->TI_lines - 1, 0, current_term->TI_cols - 1), sizeof final);
		...
		if (n > 0) {
			...
			if (current_term->TI_dl) {
				oneshot = 1;
				strlcpy(thing, (char *)tparm(current_term->TI_dl, rn, rn), sizeof thing);
			} else
				strlcpy(thing, current_term->TI_dl1, sizeof thing);

			if (current_term->TI_il) {
				oneshot = 1;
				strlcpy(final, (char *)tparm(current_term->TI_il, rn, rn), sizeof final);
			} else
			...
			if (current_term->TI_il) {
				oneshot = 1;
				strlcpy(thing, (char *)tparm(current_term->TI_il, rn, rn), sizeof thing);
			} else
				strlcpy(thing, current_term->TI_il1, sizeof thing);

			if (current_term->TI_dl) {
				oneshot = 1;
				strlcpy(final, (char *)tparm(current_term->TI_dl, rn, rn), sizeof final);
			} else
				strlcpy(final, current_term->TI_dl1, sizeof final);
		}
	}
	if (!thing[0])
		return;
...
void
set_meta_8bit(Window * w, char *u, int value)
{
	if (dumb_mode)
		return;
...
static char *
control_mangle(unsigned char *text)
{
...
char   *
get_term_capability(char *name, int querytype, int mangle)
{
 ...
 */
int (*term_scroll)(int, int, int);      /* this is set to the best scroll
					 * available */
int (*term_insert)(char);               /* this is set to the best insert
					 * available */
int (*term_delete)(void);               /* this is set to the best delete
					 * available */
int (*term_cursor_left)(void);          /* this is set to the best left
					 * available */
int (*term_cursor_right)(void);         /* this is set to the best right
					 * available */
...
void
ScrollBuf(HANDLE hOut, CONSOLE_SCREEN_BUFFER_INFO * scrbuf, int where)
{
	SMALL_RECT src;
	COORD dest;
	CHAR_INFO chr;
...
void
NT_MoveToLineOrChar(void *fd, int where, int line)
{
	CONSOLE_SCREEN_BUFFER_INFO gscrbuf;
...
void
NT_MoveToLineOrCharAbs(void *fd, int where, int line)
{
...
void
term_move_cursor(int cursor, int line)
{
	HANDLE hStdout = ghstdout;
	COORD dest;
...
void
reset_attrib(void *fd, int *fg, int *bg)
{
	intensity = 0;
	underline = 0;
	reverse_ch = 0;
...
int
nt_write(void *fd, unsigned char *buf, int howmany)
{
	DWORD bytes_rtn;
	int num = 0;
	int len = -1;
