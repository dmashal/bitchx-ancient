...
void
BX_add_name_to_genericlist(char *name, HashEntry * list, unsigned int size)
{
	List *nptr;
...
List   *
BX_find_name_in_genericlist(char *name, HashEntry * list, unsigned int size, int remove)
{
	HashEntry *location;
	List *tmp, *prev = NULL;
	unsigned long hvalue = hash_nickname(name, size);
	...
	/*
	 * at this point, we found the array spot, now search as regular
	 * linked list, or as ircd likes to say... "We found the bucket, now
	 * search the chain"
	 */
 ...
 */
void
BX_add_nicklist_to_channellist(NickList * nptr, ChannelList * cptr)
{
	unsigned long hvalue = hash_nickname(nptr->nick, NICKLIST_HASHSIZE);

	/*
	 * take this nicklist, and attach it as the HEAD pointer in our
	 * chain at the hashed location in our array... Note, by doing this,
	 * this ensures that the "most active" users always remain at the
	 * top of the chain... ie, faster lookups for active users, (and as
...
NickList *
BX_find_nicklist_in_channellist(char *nick, ChannelList * cptr, int remove)
{
	HashEntry *location;
 ...
 */
NickList *
BX_next_nicklist(ChannelList * cptr, NickList * nptr)
{
...
List   *
BX_next_namelist(HashEntry * cptr, List * nptr, unsigned int size)
{
 ...
 */
void
BX_add_whowas_userhost_channel(WhowasList * wptr, WhowasWrapList * list)
{
...
WhowasList *
BX_find_userhost_channel(char *host, char *channel, int remove, WhowasWrapList * wptr)
{
	HashEntry *location;
	WhowasList *tmp, *prev = NULL;
 ...
 */
WhowasList *
BX_next_userhost(WhowasWrapList * cptr, WhowasList * nptr)
{
	unsigned long hvalue = 0;
...
int
BX_remove_oldest_whowas_hashlist(WhowasWrapList * list, time_t timet, int count)
{
	WhowasList *ptr;
	int total = 0;
	unsigned long x;
...
int
nick_match(NickList * nick, char *mask)
{
	int match = 0;
...
NickList *
BX_sorted_nicklist(ChannelList * chan, int sort)
{
...
void
BX_clear_sorted_nicklist(NickList ** list)
{
	NickList *t;
...
Flooding *
BX_add_name_to_floodlist(char *name, char *host, char *channel, HashEntry * list, unsigned int size)
{
	Flooding *nptr;
	unsigned long hvalue = hash_nickname(name, size);
...
Flooding *
BX_find_name_in_floodlist(char *name, char *host, HashEntry * list, unsigned int size, int remove)
{
	HashEntry *location;
	Flooding *tmp, *prev = NULL;
	unsigned long hvalue = hash_nickname(name, size);
