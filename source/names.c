 ...
 */
char *
check_channel_type(char *channel)
{
...
extern ChannelList *
BX_lookup_channel(char *channel, int server, int unlink)
{
	ChannelList *chan = NULL, *last = NULL;
...
extern int
BX_im_on_channel(char *channel, int server)
{
 ...
 */
ChannelList *
BX_add_channel(char *channel, int server, int refnum)
{
 ...
 */
ChannelList *
BX_add_to_channel(char *channel, char *nick, int server, int oper, int voice, char *userhost, char *server1, char *away, int serv_split, int server_hops)
{
	NickList *new = NULL;
...
char   *
BX_get_channel_key(char *channel, int server)
{
	ChannelList *tmp;
 ...
 */
char   *
BX_recreate_mode(ChannelList * chan)
{
	int mode_pos = 0, mode;
	char *s;
 ...
 */
char   *
BX_do_compress_modes(ChannelList * chan, int server, char *channel, char *modes)
{
	...
	/*
	 * now, modes contains the actual modes, and rest contains the
	 * arguments to those modes
	 */
	...
	*nargs = 0;
	...
	for (tucm = ucm; tucm && (strlen(nmodes) + 2) < sizeof nmodes;
		...
		if (tucm->o_ed) {
			if (mod == 1)
				strlcat(nmodes, "o", sizeof nmodes);
			else {
				strlcat(nmodes, "+o", sizeof nmodes);
				mod = 1;
			}
		...
		if (tucm->v_ed) {
			if (mod == 1)
				strlcat(nmodes, "v", sizeof nmodes);
			else {
				strlcat(nmodes, "+v", sizeof nmodes);
				mod = 1;
			}
		...
		if (tucm->b_ed) {
			if (mod == 1)
				strlcat(nmodes, "b", sizeof nmodes);
			else {
				strlcat(nmodes, "+b", sizeof nmodes);
				mod = 1;
			}
...
int
BX_got_ops(int add, ChannelList * channel)
{
	int have_op = 0;
	NickList *tmp;
	int in_join = 0;
 ...
 */
static void
apply_channel_modes(char *from, char *mode_str,
                    ChannelList * channel)
{
		...
		case 'l':
			...
			if (add) {
				...
				if (limit) {
					...
					/*
					 * +l with no argument - broken
					 * server, just ignore it.
					 */
					value = 0;
				}
			} else {
 ...
 */
char   *
BX_get_channel_mode(char *channel, int server)
{
	ChannelList *tmp;
...
char   *
BX_get_channel_bans(char *channel, int server, int type_mode)
{
	ChannelList *tmp;
...
void
BX_clear_bans(ChannelList * channel)
{
	BanList *bans, *next;
...
void
BX_remove_channel(char *channel, int server)
{
	ChannelList *tmp;
 ...
 */
void
BX_remove_from_channel(char *channel, char *nick, int server, int netsplit, char *reason)
{
	ChannelList *chan;
	NickList *tmp = NULL;
...
void
handle_nickflood(char *old_nick, char *new_nick, NickList * nick, ChannelList * chan, int flood_time)
{
 ...
 */
void
BX_rename_nick(char *old_nick, char *new_nick, int server)
{
	ChannelList *chan;
	NickList *tmp;
	int t = 0;
 ...
 */
int
BX_is_on_channel(char *channel, int server, char *nick)
{
	ChannelList *chan;
...
int
BX_is_chanop(char *channel, char *nick)
{
	ChannelList *chan;
	NickList *Nick;
...
int
BX_is_halfop(char *channel, char *nick)
{
	ChannelList *chan;
	NickList *Nick;
...
char   *
is_chanoper(char *channel, char *nick)
{
	ChannelList *chan;
	NickList *Nick;
	char *ret = NULL;
...
int
BX_get_channel_oper(char *channel, int server)
{
	ChannelList *chan;
...
int
BX_get_channel_halfop(char *channel, int server)
{
	ChannelList *chan;
...
char   *
BX_fetch_userhost(int server, char *nick)
{
	ChannelList *tmp = NULL;
	NickList *user = NULL;
...
int
BX_get_channel_voice(char *channel, int server)
{
	ChannelList *chan;
...
extern char *
BX_create_channel_list(Window * window)
{
	ChannelList *tmp;
	char buffer[BIG_BUFFER_SIZE + 1];
	...
	*buffer = 0;
	for (tmp = get_server_channels(window->server); tmp; tmp = tmp->next) {
		if (tmp->server == from_server) {
			strlcat(buffer, tmp->channel, sizeof buffer);
...
remove_from_join_list(char *chan, int server)
{
	...
	for (tmp = join_list; tmp; tmp = tmp->next) {
		...
		if (!my_stricmp(chan, tmp->chan) && tmp->server == server) {
			/* XXX Null deref, prev might be null */
			if (tmp == join_list)
				join_list = next;
			else
				prev->next = next;
 ...
 */
void
unset_window_current_channel(Window * window)
{
	ChannelList *tmp;
 ...
 */
void
move_window_channels(Window * window)
{
	ChannelList *tmp;
