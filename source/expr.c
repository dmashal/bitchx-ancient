...
canon_number(char *input)
{
	...
	if (get_int_var(FLOATING_POINT_MATH_VAR)) {
		...
		/*
		 * If we removed all the zeros and all that is left is the
		 * decimal point, remove it too
		 */
		if (input[end] == '.')
			end--;
 ...
 */
char   *
BX_next_unit(char *str, const char *args, int *arg_flag, int stage)
{
...
#define SETUP_INTEGER_OPERATION(STAGE)                                  \
	 ...
	 * operator is at "ptr".  So if you do ptr = lastop(ptr), youll end
	 * up at the beginning of the rhs, which is the rest of the
	...
	for (ptr = str; *ptr; ptr++) {
		...
		case '(':
		{
			...
			if (stage != NU_UNIT || ptr == str) {
				/*
				 * If there is no matching ), gobble
				 * up the entire expression.
				 */
		...
		case '[':
		{
...
#if 0
				<hop > if little
				endian is ABCD and big endian is DCBA
...
#endif
			{
				...
				*ptr++ = '\0';
				...
				/*
				 * Here we expand what is inside the
				 * [] set, as literal text.
				 */
#ifndef NO_CHEATING
				/*
		...
		case '+':
		{
			if (ptr[1] == ptr[0]) {
				...
				if (stage != NU_UNIT) {
					/*
					 * only one 'ptr++' because
					 * a 2nd one is done at the
					 * top of the loop after the
					 * 'break'.
					 */
					ptr++;
					break;
		...
		/*
		 * Reworked - Jeremy Nelson, Feb 1994 Reworked
		 * again, Feb 1996 (jfn)
		 *
		 * X, XX, and X= are all supported, where X is one of
		 * "&" (and), "|" (or) and "^" (xor).  The XX forms
		 * short-circuit, as they do in C and perl.  X and
		 * X= forms are bitwise, XX is logical.
		 */
		case '&':
		{
 ...
 */
char   *
BX_parse_inline(char *str, const char *args, int *args_flag)
{
#ifndef WINNT
 ...
 */
char   *
BX_expand_alias(const char *string, const char *args, int *args_flag, char **more_text)
{
	...
	if (*string == '@' && more_text) {
		unescape = 0;
		*args_flag = 1;         /* Stop the @ command from auto
					 * appending */
	}
	quote_temp[1] = 0;
...
char   *
call_structure(char *name, const char *args, int *args_flag, char *rest, char *rest1)
{
 ...
 */
char   *
BX_alias_special_char(char **buffer, char *ptr, const char *args, char *quote_em, int *args_flag)
{
	...
	case LEFT_PAREN:
	{
		char *sub_buffer = NULL, *tmp2 = NULL, *tmpsav = NULL,
		*ph = ptr + 1;
	...
	case LEFT_BRACE:
	{
		...
		/*
		 * BLAH. This didnt allow for nesting before.  How
		 * lame.
		 */
		if ((ptr = MatchingBracket(ph, '{', '}')) ||
		    (ptr = strchr(ph, '}')))
 ...
 */
static void
TruncateAndQuote(char **buff, const char *add, int length, const char *quote_em, char pad_char)
{
	/*
	 * Semantics: If length is nonzero, then "add" will be truncated to
	 * "length" characters If length is zero, nothing is done to "add"
	 * If quote_em is not NULL, then the resulting string will be quoted
	 * and appended to "buff" If quote_em is NULL, then the value of
	 * "add" is appended to "buff"
	 */
	if (length) {
		char *buffer = NULL;
...
static void
do_alias_string(char *unused, char *input)
{
