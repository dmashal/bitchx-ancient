 ...
 */
static int
show_help(Window * window, char *name)
{
...
help_topic(char *path, char *name)
{
	...
	if ((help_fp = uzfopen(&filename, path, 0))) {
		/*
		 * Isnt this a heck of a lot better then the kludge you were
		 * using?
		 */
 ...
 */
static void
help_show_paused_topic(char *name, char *line)
{
	static int i = 0;
	int j = 0;
	int rows;
	...
	/*
	 * This cant be an else of the previous if because 'i' can change in
	 * the previous if and we need to test it again
	 */
	if (i >= help_paused_lines) {
		if (get_int_var(HELP_PROMPT_VAR)) {
			char buf[BIG_BUFFER_SIZE];
...
static void
help_put_it(const char *topic, const char *format,...)
{
