...
typedef struct _DCC_List {
	...
	char   *nick;                   /* do NOT free this. it's a shared
					 * pointer. */
	SocketList sock;
}       DCC_List;
...
static int doing_multi = 0;             /* set this if doing multiple requests
					 * helps prevent console spam by
					 * reducing the amount of output. */

#if 0
int
add_dcc_command(char *name, char *help, void (*func)(char *, char *))
{
	int count = 0;

	if (!dcc_dllcommands)           /* ooh first time. need to init. make
					 * it double */
...
void
send_ctcp_booster(char *nick, char *type, char *file, unsigned long address, unsigned short portnum, unsigned long filesize)
{
	if (filesize)
...
void
send_reject_ctcp(char *nick, char *type, char *name)
{
	/*
...
void
BX_erase_dcc_info(int s, int reject, char *format,...)
{
	DCC_int *n = NULL;
	SocketList *s1;
...
SocketList *
BX_find_dcc(char *nick, char *desc, char *other, int type, int create, int active, int num)
{
	int i;
	SocketList *s;
	DCC_int *n;
 ...
 */
DCC_List *
find_dcc_pending(char *nick, char *desc, char *othername, int type, int remove, int num)
{
	int dcc_type = 0;
	SocketList *s;
	DCC_int *n;
...
void
add_userhost_to_chat(UserhostItem * stuff, char *nick, char *args)
{
...
static void
refileread_send_start(int s, DCC_int * n)
{
	struct stat buf;
...
#ifdef HAVE_SSL
int
SSL_dcc_create(SocketList * s, int sock, int doconnect)
{
	set_blocking(sock);
	if (doconnect)
...
DCC_int *
BX_dcc_create(char *nick, char *filename, char *passwd, unsigned long filesize, int port, int type, unsigned long flags, void (*func)(int))
{
	...
	if (((new_i = find_dcc_pending(nick, filename, NULL, type, 1, -1)) && new_i->sock.flags & DCC_OFFER) || (flags & DCC_OFFER)) {
		...
		if ((s = connect_by_number(NULL, &portnum, SERVICE_SERVER, PROTOCOL_TCP, 1)) < 0) {
			/*
			 * If a port was specified and we failed, and random
			 * local ports is on, try a random port.
			 */
			if (port && get_int_var(RANDOM_LOCAL_PORTS_VAR))
				portnum = random_number(65535 - 1024) + 1024;
			else
			...
			if ((s = connect_by_number(NULL, &portnum, SERVICE_SERVER, PROTOCOL_TCP, 1)) < 0) {
				/*
				 * Finally try to get a system port number
				 * by using 0
				 */
				portnum = 0;
	...
	default:
	{
		...
		*uhost = 0;
		if (n->userhost)
			...
			strmopencat(uhost, 99, "Unknown@", "unknown"            /* inet_ntoa(remaddr.sin_
										   addr) */, NULL);
		FromUserHost = uhost;
...
#define CTCP_REPLY_LEN strlen(CTCP_REPLY)
		...
		*equal_nickname = 0;
		...
		if (!strncmp(tmp, CTCP_MESSAGE, CTCP_MESSAGE_LEN) ||
			...
			tmp2 = LOCAL_COPY(tmp);
			strlcpy(tmp, do_ctcp(equal_nickname, get_server_nickname(from_server), stripansicodes((tmp2 + ((*tmp2 == CTCP_DELIM_CHAR) ? 0 : CTCP_MESSAGE_LEN)))), sizeof tmp);
			if (!*tmp)
				break;
			...
			tmp2 = LOCAL_COPY(tmp);
			strlcpy(tmp, do_notice_ctcp(equal_nickname, get_server_nickname(from_server), stripansicodes(tmp2 + (((*tmp2 == CTCP_DELIM_CHAR) ? 0 : CTCP_REPLY_LEN)))), sizeof tmp);
			if (!*tmp)
				break;
		}
...
static void
new_dcc_message_transmit(char *user, char *text, char *text_display, int type, int flag, char *cmd, int check_host)
{
	SocketList *s = NULL;
	DCC_int *n = NULL;
...
extern void
dcc_chat_transmit(char *user, char *text, char *orig, char *type, int noisy)
{
	int fd;
...
extern void
dcc_bot_transmit(char *user, char *text, char *type)
{
...
void
register_dcc_type(char *nick, char *type, char *description, char *address, char *port, char *size, char *extra, char *uhost, void (*func1)(int))
{
	int Ctype;
	...
	if (!my_stricmp(type, "CHAT"))
		...
		/*
		 * Dont be deceieved by the arguments we're passing it. The
		 * arguments are "out of order" because MIRC doesnt send
		 * them in the traditional order.  Ugh. Comments borrowed
		 * from epic.
		 */
	...
	if (do_hook(DCC_REQUEST_LIST, "%s %s %s %lu", nick, dcc_types[Ctype]->name, description, filesize)) {
		...
		if ((Ctype == DCC_BOTMODE) && (p = get_string_var(BOT_PASSWORD_VAR))) {
			if (encrypt && !strcmp(p, encrypt)) ;   /* do it */
...
char   *
get_dcc_args(char **args, char **passwd, char **port, int *blocksize)
{
	char *user = NULL;
...
close_dcc_file(int snum)
{
	...
	*lame_type = 0;
	...
	/*
	 * Cant pass %g to put_it (lame ultrix/dgux), fix suggested by
	 * sheik.
	 */
...
start_dcc_send(int s)
{
	...
	if (type == DCC_REFILEOFFER) {
		...
		if (byte_order_test() != received.byteorder) {
			/*
			 * the packet sender orders bytes differently than
			 * us, reverse what they sent to get the right value
			 */
...
void
real_file_send(char *nick, char *filename, char *passwd, char *port, int tdcc, unsigned long type, int blocksize)
{
	DCC_int *new = NULL;
	SocketList *s = NULL;
	unsigned long filesize = 0;
...
void
BX_dcc_filesend(char *command, char *args)
{
	char *user;
	char *nick;
...
void
BX_dcc_resend(char *command, char *args)
{
	char *user;
	char *nick;
...
void
real_get_file(char *user, char *filename, char *port, char *passwd, int tdcc, int blocksize)
{
	DCC_int *new = NULL;
	SocketList *s = NULL;
	DCC_List *s1 = NULL;
...
void
real_reget_file(char *user, char *filename, char *port, char *passwd, int tdcc, int blocksize)
{
	DCC_int *new = NULL;
	SocketList *s = NULL;
	char *nick;
...
dcc_glist(char *command, char *args)
{
	...
	else do_hook(DCC_POST_LIST, "%s %s %s %s %s %s %s", "DCCnum", "Type", "Nick", "Status", "K/s", "File", "Encrypt");
...
void
dcc_tog_resume(char *command, char *args)
{
...
void
dcc_tog_auto(char *command, char *args)
{
...
void
dcc_stats(char *command, char *unused)
{
	char max_rate_in[20];
	char min_rate_in[20];
...
void
dcc_exempt(char *command, char *args)
{
	int remove;
	List *nptr = NULL;
	char *nick;
 ...
 */
static void
output_reject_ctcp(UserhostItem * stuff, char *nick, char *args)
{
...
void
cancel_dcc_auto(int i, unsigned long flags, SocketList * s)
{
...
void
dcc_closeall(char *command, char *args)
{
	close_all_dcc();
}
...
void
dcc_getfile_resume_demanded(char *nick, char *filename, char *port, char *offset)
{
	SocketList *s;
	DCC_int *n;
...
void
dcc_getfile_resume_start(char *nick, char *description, char *address, char *port)
{
	SocketList *s;
	DCC_int *n;
	char *tmp = NULL;
...
void
dcc_resume(char *command, char *args)
{
	char *user, *nick;
	char *filename = NULL;
	char *fullname = NULL;
...
int
handle_ftp(int comm, int snum, SocketList * s, char *buf)
{
	int err = 0;
	DCC_int *n;
...
void
open_ftpget(SocketList * s, char *args)
{
	DCC_int *new;
	struct sockaddr_in data_addr = {0};
	socklen_t len;
...
int
dcc_ftpcommand(char *host, char *args)
{
	SocketList *s;
...
char   *
parse_ncftp(char *buffer, char **hostname)
{
	char *dir = NULL, *p;
...
char   *
read_ncftp_config(char **hostname)
{
	char *tmp, *file = NULL;
	char *buffer;
	char *dir = NULL;
...
void
dcc_ftpopen(char *command, char *args)
{
	char u[] = "anonymous";
	char p[] = "- bxuser@";
...
int
check_dcc_init(char *type, char *nick, char *uhost, char *description, char *size, char *extra, unsigned long TempLong, unsigned int TempInt)
{
	int i;
...
int
BX_add_dcc_bind(char *name, char *module, void *init_func, void *open_func, void *input, void *output, void *close_func)
{
	int i;
...
int
BX_remove_dcc_bind(char *name, int type)
{
...
int
BX_remove_all_dcc_binds(char *name)
{
	int ret = 0;
	int i, j;
...
char   *
get_dcc_info(SocketList * s, DCC_int * n, int i)
{
	char local_type[80];
 ...
 */
char   *
dcc_raw_listen(unsigned short port)
{
	DCC_list *Client;
	char *PortName;
