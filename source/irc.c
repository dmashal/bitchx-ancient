...
#ifndef VERSION
	const char irc_version[] = "BitchX-1.2.2";
...
int irc_port = IRC_PORT,                /* port of ircd */
    strip_ansi_in_echo, current_on_hook = -1,           /* used in the
							 * send_text() routine */
    use_flow_control = USE_FLOW_CONTROL,                /* true: ^Q/^S used for
							 * flow cntl */
    current_numeric,                    /* this is negative of the current
					 * numeric! */
    dumb_mode = 0,                      /* if true, IRCII is put in "dumb"
					 * mode */
    bflag = 1, use_input = 1,           /* if 0, stdin is never checked */
    waiting_out = 0,                    /* used by /WAIT command */
    waiting_in = 0,                     /* used by /WAIT command */
    who_mask = 0,                       /* keeps track of which /who switchs
					 * are set */
    dead = 0, inhibit_logging = 0,
#ifndef ONLY_STD_CHARS
startup_ansi = 1,
/* display startup ansi */
#else
...
do_use_ssl = 0,
#endif
run_level = 0, foreground = 1, reconnect = 0,           /* reconnecting to old
							 * process */
    use_nat_address = 0,                /* use NAT address */
    term_initialized = 0;

char zero[] = "0", one[] = "1", space[] = " ", space_plus[] = " +", space_minus[] = " -",
     dot[] = ".", star[] = "*", comma[] = ",", empty_string[] = "",
     on[] = "ON", off[] = "OFF";

const char *unknown_userhost = "<UNKNOWN>@<UNKNOWN>";

char oper_command = 0;                  /* true just after an oper() command
					 * is given.  Used to tell the
					 * difference between an incorrect
					 * password generated by an oper()
					 * command and one generated when
...
int inbound_line_mangler = 0, logfile_line_mangler = 0, operlog_line_mangler = 0, outbound_line_mangler = 0;
...
*new_script = NULL,                     /* rephacement for .bitchxrc and
					 * .ircrc */
...
/* irc_exit: cleans up and leaves */
void
BX_irc_exit(int really_quit, char *reason, char *format,...)
{
...
void
set_detach_on_hup(Window * dummy, char *unused, int value)
{
	if (value)
...
display_bitchx(int j)
{
	...
	if (j == -1)
#ifdef ASCII_LOGO
		i = (int)(5.0 * bsd_arc4random() / RAND_MAX);
#else
		i = (int)(17.0 * bsd_arc4random() / RAND_MAX);
#endif
	else
		i = j;
...
static char *
parse_args(char *argv[], int argc, char **envp)
{
	int ac;
	int add_servers = 0;
...
/*				use_input = 0;*/
...
#else
	{
		...
		if (!(ptr = getenv("IRCUSER")))
			...
			strncpy(username, ptr, size, sizeof username);
	}
#endif
	...
	if (!check_nickname(nickname)) {
		...
		fprintf(stderr, "Please restart BitchX with a valid nickname\n");
		exit(1);
	}
	if (ircrc_file == NULL) {
		ircrc_file = (char *)new_malloc(strlen(my_path) + strlen(IRCRC_NAME) + 1);
		strlcpy(ircrc_file, my_path, sizeof ircrc_file);
		strlcat(ircrc_file, IRCRC_NAME, sizeof ircrc_file);
	}
	if (bircrc_file == NULL)
#if defined(WINNT) || defined(__EMX__)
...
void
BX_io(const char *what)
{
	static int first_time = 1, level = 0;
	long clock_timeout = 0, timer_timeout = 0, server_timeout = 0, real_timeout = 0;
	static struct timeval my_now, my_timer, *time_ptr = &my_timer;
...
main(int argc, char *argv[], char *envp[])
{
	...
	/*
	 * First thing we need to do is initialize the global function table
	 * with the default function pointers.
	 */
...
/*			printf(" connected to tty [%s]", s);*/
	...
	/*
	 * We move from run level 0 to run level 1 signifying that we have
	 * completed the initial startup and are now ready to load scripts.
	 */
	run_level = 1;
	...
	/*
	 * We now move from run level 1 to run level 2 signifying that we
	 * are in normal operation mode.
	 */
	run_level = 2;
