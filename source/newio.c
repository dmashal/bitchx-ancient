 ...
 */
const char *
dgets_strerror(int dgets_errno)
{
	switch (dgets_errno) {
	case -1:
 ...
 */
int
BX_dgets(char *str, int des, int buffer, int buffersize, void *ssl_fd)
{
	int cnt = 0, c;
	MyIO *ioe;
	int nbytes;
			...
			*str = 0;
			...
			if (ssl_fd) {
				...
				if (c == -1 && (rc = SSL_get_error((SSL *) ssl_fd, c)) == SSL_ERROR_WANT_READ) {
					/*
					 * If SSL needs more data, then we
					 * need to call SSL_read again, so
					 * we'll return with 0 bytes read,
					 * and hope we get called again. :)
					 */
					return 0;
				}
			} else
