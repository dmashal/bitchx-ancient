...
static void
who_queue_flush(void)
{
	while (who_queue_top(from_server))
		who_queue_pop();
...
whoreply(char *from, char **ArgList)
{
	...
	do {
		/*
		 * this can happen in a very likely situation. The
		 * timing is critical. a user joins a channel. We
		 * send a userhost, but before the server responds,
		 * we are kicked and rejoin the channel. A reply is
		 * still coming from the server but the channel sync
		 * hasn't finished, which means we are in limbo.
		 * This should also fix the core when /quote who is
		 * done.
		 */
		if (!new_w)
			break;
		...
		*buf_data = 0;
		...
		if (*stat == 'S') {     /* this only true for the header
					 * WHOREPLY */
			char buffer[1024];
...
void
who_end(char *from, char **ArgList)
{
	WhoEntry *new_w = who_queue_top(from_server);
	char buffer[1025];
...
void
BX_isonbase(char *args, void (*line)(char *, char *))
{
	IsonEntry *new_i;
	char *next = args;
...
static void
userhost_queue_add(UserhostEntry * item)
{
...
static UserhostEntry *
get_new_userhost_entry(void)
{
...
void    BX_userhostbase(char *args, void (*line)(UserhostItem *, char *, char *), int userhost, char *format,...){
	int total = 0, userhost_cmd = 0;
	int server_query_reqd = 0;
	char *nick;
	char buffer[BIG_BUFFER_SIZE + 1];
	char buf_data[BIG_BUFFER_SIZE + 1];
	char *ptr, *next_ptr, *body = NULL;
...
void
userhost_cmd_returned(UserhostItem * stuff, char *nick, char *text)
{
