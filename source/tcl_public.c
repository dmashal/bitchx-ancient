 ...
 */
static int
SplitAndFindField(tcl_interp, fieldName, keyedList, fieldInfoPtr)
Tcl_Interp *tcl_interp;
char *fieldName;
char *keyedList;
 ...
 */
int
Tcl_GetKeyedListField(tcl_interp, fieldName, keyedList, fieldValuePtr)
Tcl_Interp *tcl_interp;
char *fieldName;
char *keyedList;
char **fieldValuePtr;
 ...
 */
char   *
Tcl_SetKeyedListField(tcl_interp, fieldName, fieldValue, keyedList)
Tcl_Interp *tcl_interp;
char *fieldName;
char *fieldValue;
char *keyedList;
 ...
 */
char   *
Tcl_DeleteKeyedListField(Tcl_Interp * tcl_interp, char *fieldName, char *keyedList)
{
	char *nameSeparPtr;
	char *newList;
	int idx;
 ...
 */
int
Tcl_KeyldelCmd(ClientData clientData, Tcl_Interp * tcl_interp, int argc, char **argv)
{
 ...
 */
int
Tcl_KeylgetCmd(ClientData clientData, Tcl_Interp * tcl_interp, int argc, char **argv)
{
	char *keyedList;
	char *fieldValue;
	char **fieldValuePtr;
 ...
 */
int
Tcl_KeylkeysCmd(ClientData clientData, Tcl_Interp * tcl_interp, int argc, char **argv)
{
 ...
 */
int
Tcl_KeylsetCmd(ClientData clientData, Tcl_Interp * tcl_interp, int argc, char **argv)
{
...
int
Tcl_LmatchCmd(ClientData notUsed, Tcl_Interp * tcl_interp, int argc, char **argv)
{
#define EXACT   0
#define GLOB    1
#define REGEXP  2
...
int
Tcl_RelativeExpr(Tcl_Interp * tcl_interp, char *cstringExpr, long stringLen, long *exprResultPtr)
{
 ...
 */
int
Tcl_LvarpopCmd(ClientData clientData, Tcl_Interp * tcl_interp, int argc, char **argv)
{
	int listArgc, idx;
	long listIdx;
	char **listArgv;
 ...
 */
int
Tcl_LemptyCmd(ClientData clientData, Tcl_Interp * tcl_interp, int argc, char **argv)
{
	char *scanPtr;
...
#if 0
void
init_ircii_vars(Tcl_Interp * intp)
{
	for (
		}
...
void
init_public_tcl(Tcl_Interp * intp)
{
...
void
init_public_var(Tcl_Interp * intp)
{
	int i;
...
/* add a timer */
char   *
tcl_add_timer(TimerList ** stack, long elapse, char *cmd, unsigned long prev_id)
{
	TimerList *old = (*stack);
...
int
tcl_remove_timer(TimerList ** stack, unsigned long id)
{
...
void
do_check_timers(TimerList ** stack)
{
	TimerList *mark = *stack, *old;
	Tcl_DString ds;
	int argc, i;
...
void
tcl_list_timer(Tcl_Interp * irp, TimerList * stack)
{
...
time_t
tclTimerTimeout(time_t timeout)
{
...
int
check_tcl_dcc(char *cmd, char *nick, char *host, int idx)
{
	int x, atr = 0;
