...
int
check_mode_lock(char *channel, char *mode_list, int server)
{
	ChannelList *chan;
...
void
who_user_kill(WhoEntry * w, char *from, char **ArgList)
{
	char *match_arg;
	char *arg = NULL;
	int server = 0;
...
void
who_user_killend(WhoEntry * w, char *unused, char **unused1)
{
	...
	if (w->who_buff) {
		...
		*reason++ = 0;
		...
		if (match && *match && server != -1) {
			...
			strlcpy(m_buff, match, sizeof m_buff);
...
void
trace_handlekill(int comm, char *nick)
{
	static int count = 0;
...
void
handle_tracekill(int comm, char *nick, char *user, char *host)
{
	char *q, *n;
		...
		if (!is_window_name_unique(name + 1)) {
			...
			if ((tmp = get_window_by_name(name + 1))) {
				...
				if ((logic = logical_to_index(name + 1)) > -1)
					...
					bitchsay("No such process [%s]", name + 1);
			}
...
/*		start_process(p, name+1, NULL, NULL, refnum, direct);*/
		...
		bitchsay("Unable to create a new window");
}
...
static void
gain_nick(UserhostItem * stuff, char *nick, char *args)
{
	int gotit = 0;
	int server = -1;
	int old_serv;
...
static int
delay_gain_nick(void *arg, char *sub)
{
...
int
grab_http(char *from, char *to, char *text)
{
#ifdef PUBLIC_ACCESS
	return 0;
#else
	if ((get_int_var(HTTP_GRAB_VAR) && stristr(text, "HTTP:")) || (get_int_var(FTP_GRAB_VAR) && (stristr(text, "FTP:") || stristr(text, "FTP.")))) {
		...
		for (cur_url = url_list, prev_url = NULL; cur_url; prev_url = cur_url, cur_url = cur_url->next) {
			url_count++;
			/*
			 * If we find that the URL is already in the list,
			 * bail out.
			 */
...
void
get_range(char *line, int *start, int *end)
{
	char *q = line, *p = line;
...
#if defined(GUI)
void
parsemenuitem(char *pargs, char **itemtext, char **options, char **itemalias, int *refnum)
{
	char *text;
...
void
parsesubmenu(char *pargs, char **submenutext, char **options)
{
	if (*pargs == '-') {
...
static int
null_cmp_func(List * s, List * s1)
{
	return 0;
}

MenuStruct *
addmenu(char *menuname)
{
	MenuStruct *new = NULL;
...
BUILT_IN_COMMAND(filedialog)
{
	...
	*code = NULL, szButton[10] = "OK";
	...
	if (*args == '\"') {
		...
		strlcpy(szButton, ok, sizeof szButton);
	}
	if (*args == '\"')
...
void
kill_attached_if_needed(int type)
{
...
make_cookie(void)
{
	...
	for (i = 0; i < (int)(20.0 * bsd_arc4random() / RAND_MAX) + 5; i++) {
		j = (int)(95.0 * bsd_arc4random() / RAND_MAX);
...
BUILT_IN_COMMAND(detachcmd)
{
	 ...
	 * detach and then later a screen type program can be used to
	 * re-attach to us.
	 */
	if (already_detached)
		return;
...
BUILT_IN_COMMAND(show_revisions)
{
	...
	while (cvsfuncs[z]) {
		...
		/*
		 * If there are args... we look for a matching filename or
		 * start string
		 */
		if (args && *args) {
