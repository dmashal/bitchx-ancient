...
void start_dns(void)
{
	...
	init_dns_mutexes();
	/*
	 * init our pending queue condition, and set to default attributes
	 * (NULL)
	 */
...
static void
kill_dns_thread(int notused)
{
	/* Empty */
}

static void
kill_dns_thread2(int notused)
{
	int ecode = 0;
...
start_dns_thread(void *args)
{
	...
	while (1) {
		/*
		 * Try the quit mutex, if we get it, that means main() wants
		 * us to die.
		 */
		...
		if (!dns) {
			...
			/*
			 * We have been woken up. Thus, 2 conditions are
			 * present. 1) We have been signalled that data is
			 * waiting. 2) The mutex is locked.
			 */
			pthread_mutex_unlock(&pending_queue_mutex);
		} else {
			char c = ' ';
			...
			pthread_mutex_unlock(&finished_queue_mutex);
			/*
			 * Write to our pipe, this will wake up mains select
			 * loop
			 */
...
my_hostent *
duphostent(struct hostent *orig)
{
...
void
freemyhostent(my_hostent * freeme)
{
	int z;
...
static void
destroy_dns_queue(DNS_QUEUE ** QueueHead, DNS_QUEUE ** QueueTail)
{
	DNS_QUEUE *dns;
