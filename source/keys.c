...
static void
snew_key(int meta, unsigned chr, char *what)
{
	int i;
	int j;
...
static void
snew_key_from_str(uc * string, char *what)
{
	int i;
	int meta;
	int old_display;
...
static void
new_key(int meta, unsigned chr, int type, int change, char *stuff)
{
	/*
 ...
 */
int
get_binding(int meta, uc c, KeyBinding * func, char **name)
{
	*func = NULL;
	*name = NULL;
...
void
unload_bindings(const char *filename)
{
	int i, j;
...
/* * * * * * * * * * * * * * BIND  * * * * * * * * * * * * * */
static int
grok_meta(const uc * ptr, const uc ** end)
{
	int meta = -1;
	const uc *str;
 ...
 */
void
copy_redux(const uc * orig, uc * result)
{
	const uc *ptr;
 ...
 */
static int
parse_key(const uc * sequence, uc * term)
{
	uc *copy;
	uc *end;
	int return_meta = 0;
 ...
 */
static int
lookup_function(const uc * orig_name, int *lf_index)
{
	int len, cnt;
	uc *name;
	size_t i;
...
init_keys(void)
{
	...
	/*
	 * All the "default" bindings are self_insert unless we bind them
	 * differently
	 */
	for (i = 1; i <= 255; i++)
	...
	/*
	 * european keyboards (and probably others) use the eigth bit for
	 * extended characters.  Having these keys bound by default causes
	 * them lots of grief, so unless you really want to use these, they
	 * are commented out.
	 */
