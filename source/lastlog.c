...
unsigned long
BX_set_lastlog_msg_level(unsigned long level)
{
	unsigned long old;
...
unsigned long
parse_lastlog_level(char *str, int display)
{
	char *ptr, *rest;
	int len, i;
...
void
set_new_server_lastlog_level(Window * win, char *str, int unused)
{
...
Lastlog *
get_lastlog_current_head(Window * win)
{
...
int
logmsg(unsigned long log_type, char *from, int flag, char *format,...)
{
#ifdef PUBLIC_ACCESS
	return 0;
#else
