 ...
 */
void   *
n_malloc(size_t size, const char *module, const char *file, const int line)
{
	char *ptr;
 ...
 */
void   *
n_free(void *ptr, const char *module, const char *file, const int line)
{
	if (ptr) {
#ifdef FREE_DEBUG
...
void   *
n_realloc(void *ptr, size_t size, const char *module, const char *file, const int line)
{
	char *ptr2 = NULL;
 ...
 */
char   *
n_malloc_strcpy(char **ptr, const char *src, const char *module, const char *file, const int line)
{
	if (!src)
...
char   *
n_malloc_strcat(char **ptr, const char *src, const char *module, const char *file, const int line)
{
	size_t msize;
...
char   *
BX_malloc_str2cpy(char **ptr, const char *src1, const char *src2)
{
	if (!src1 && !src2)
		return new_free(ptr);
...
char   *
BX_m_3dup(const char *str1, const char *str2, const char *str3)
{
...
char   *
n_m_strdup(const char *str, const char *module, const char *file, const int line)
{
	char *ptr;
...
char   *
BX_m_s3cat(char **one, const char *maybe, const char *definitely)
{
	if (*one && **one)
...
char   *
BX_m_s3cat_s(char **one, const char *maybe, const char *ifthere)
{
	if (ifthere && *ifthere)
...
char   *
BX_m_3cat(char **one, const char *two, const char *three)
{
	int len = 0;
	char *str;
...
char   *
BX_malloc_sprintf(char **to, const char *pattern,...)
{
...
char   *
BX_m_sprintf(const char *pattern,...)
{
	char booya[BIG_BUFFER_SIZE * 3 + 1];
...
char   *
BX_stristr(const char *source, const char *search)
{
	int x = 0;
...
char   *
BX_rstristr(char *source, char *search)
{
	char *ptr;
	int x = 0;
...
char   *
BX_next_arg(char *str, char **new_ptr)
{
	char *ptr;
...
extern char *
BX_remove_trailing_spaces(char *foo)
{
	char *end;
...
#define risspace(c) (c == ' ')
char   *
BX_new_next_arg(char *str, char **new_ptr)
{
	char *ptr, *start;
...
char   *
BX_new_new_next_arg(char *str, char **new_ptr, char *type)
{
	char *ptr, *start;
...
int
BX_my_stricmp(const char *str1, const char *str2)
{
...
int
BX_my_strnicmp(const char *str1, const char *str2, size_t n)
{
...
int
BX_my_strnstr(const unsigned char *str1, const unsigned char *str2, size_t n)
{
	char *p = (char *)str1;
...
char *
BX_chop (char *stuff, size_t nchar)
{
 ...
 */
char   *
BX_strmcpy(char *dest, const char *src, int maxlen)
{
 ...
 */
char   *
BX_strmcat(char *dest, const char *src, int maxlen)
{
 ...
 */
char   *
n_m_strcat_ues(char **dest, char *src, int unescape, const char *module, const char *file, const int line)
{
	int total_length;
	char *ptr, *ptr2;
	int z;
 ...
 */
extern int
BX_scanstr(char *str, char *source)
{
	int i, max, len;
 ...
 */
char   *
BX_sindex(char *string, char *group)
{
	char *ptr;
 ...
 */
char   *
BX_rsindex(char *string, char *start, char *group, int howmany)
{
	char *ptr;
...
char   *
BX_rfgets(char *buffer, int size, FILE *file)
{
	char *ptr;
	off_t pos;
 ...
 */
char   *
BX_path_search(char *name, char *path)
{
	...
	/*
	 * A "relative" path is searched in the path if the filename doesnt
	 * really exist from where we are
	 */
	if (strchr(name, '/'))
		if (!access(name, F_OK))
			return name;
 ...
 */
char   *
BX_double_quote(const char *str, const char *stuff, char *buffer)
{
	char c;
	int pos;
...
char   *
quote_it(const char *str, const char *stuff, char *buffer)
{
	char c;
	int pos;
...
int
BX_end_strcmp(const char *one, const char *two, int bytes)
{
	if (bytes < strlen(one))
...
FILE   *
open_compression(char *executable, char *filename, int hook)
{
 ...
 */
FILE   *
BX_uzfopen(char **filename, char *path, int hook)
{
	static int setup = 0;
	 ...
	 * filename_trying, and it should exist.  If ok_to_decompress is
	 * one, then we can gunzip the file if guzip is available, else we
	 * uncompress the file
	 */
	if (ok_to_decompress) {
 ...
 */
extern int
opento(char *filename, int flags, off_t position)
{
	int file;
...
struct timeval
BX_get_time(struct timeval *timer)
{
 ...
 */
double
BX_time_diff(struct timeval one, struct timeval two)
{
	struct timeval td;
...
char   *
BX_plural(int number)
{
	return (number != 1) ? "s" : empty_string;
}

char   *
BX_my_ctime(time_t when)
{
 ...
 */
char   *
BX_strformat(char *dest, const char *src, ssize_t length, char pad_char)
{
...
char   *
BX_MatchingBracket(char *string, char left, char right)
{
	int bracket_count = 1;
...
char   *
BX_unsplitw(char ***container, int howmany)
{
	char *retval = NULL;
	char **str = *container;
...
char   *
BX_m_2dup(const char *str1, const char *str2)
{
...
char   *
BX_m_e3cat(char **one, const char *yes1, const char *yes2)
{
	if (*one && **one)
...
BX_check_val(char *sub)
{
	...
	/*
	 * Its OK if: 1) the f-val is not zero. 2) the first illegal
	 * character was not a null. 3) there were no valid f-chars.
	 */
 ...
 */
extern char *
BX_strextend(char *str, char app, int num)
{
 ...
 */
extern char *
BX_pullstr(char *source_string, char *dest_string)
{
...
char   *
BX_remove_brackets(const char *name, const char *args, int *arg_flag)
{
...
is_root(char *root, char *var, int descend)
{
	...
	/*
	 * ``var'' must not contain any additional dots if we are checking
...
size_t
BX_streq(const char *one, const char *two)
{
	size_t cnt = 0;
...
/* Returns the number of characters they are equal at. */
size_t
BX_strieq(const char *one, const char *two)
{
	size_t cnt = 0;
...
char   *
n_m_strndup(const char *str, size_t len, const char *module, const char *file, const int line)
{
...
char   *
BX_strmopencat(char *dest, int maxlen,...)
{
	va_list args;
	int size;
	char *this_arg = NULL;
 ...
 */
char   *
BX_ov_strcpy(char *one, const char *two)
{
	if (two > one) {
		while (two && *two)
			*one++ = *two++;
...
char   *
BX_next_in_comma_list(char *str, char **after)
{
	*after = str;
...
void
BX_strip_control(const char *src, char *dest)
{
	for (; *src; src++) {
 ...
 */
int
BX_figure_out_address(char *nuh, char **nick, char **user, char **host, char **domain, int *ip)
{
...
#ifdef INCLUDE_DEADCODE
int
count_char(const unsigned char *src, const unsigned char look)
{
...
char   *
BX_strnrchr(char *start, char which, int howmany)
{
...
char   *
strlpcat(char *source, size_t size, const char *format,...)
{
	va_list args;
 ...
 */
size_t
BX_ccspan(const char *string, int s)
{
	size_t count = 0;
	char c = (char)s;
...
int
BX_charcount(const char *string, char what)
{
	int x = 0;
...
char   *
BX_strpcat(char *source, const char *format,...)
{
	va_list args;
	char buffer[BIG_BUFFER_SIZE + 1];
...
char   *
BX_strmpcat(char *source, size_t siz, const char *format,...)
{
	va_list args;
	char buffer[BIG_BUFFER_SIZE + 1];
...
u_char *
BX_strcpy_nocolorcodes(u_char *dest, const u_char *source)
{
	u_char *save = dest;
...
int
checkpass(const char *password, const char *old)
{
	char seed[3], *p;
 ...
 */
size_t
BX_mangle_line(char *incoming, int how, size_t how_much)
{
	int stuff;
	char *buffer;
	int i;
	...
	if (stuff & MANGLE_ANSI_CODES) {
		...
		output = normalize_string(incoming);
...
void
strip_chars(char *buffer, char *strip, char replace)
{
	char *p;
...
unsigned long
BX_random_number(unsigned long l)
{
