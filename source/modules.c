...
/* screen.c */
	...
	global_table[NORMALIZE_STRING] = (Function_ptr) BX_normalize_string;
...
/* names.c */
	...
	global_table[MAKE_CHANNEL] = (Function_ptr) BX_make_channel;    /* this is really in
									 * misc.c */
...
int
BX_check_module_version(unsigned long number)
{
...
#ifdef WANT_DLL
char   *
BX_get_dllstring_var(char *typestr)
{
...
int
BX_get_dllint_var(char *typestr)
{
	IrcVariableDll *dll = NULL;
...
void
BX_set_dllstring_var(char *typestr, char *string)
{
	if (typestr) {
		IrcVariableDll *dll = NULL;
...
void
BX_set_dllint_var(char *typestr, unsigned int value)
{
	if (typestr) {
		IrcVariableDll *dll = NULL;
...
RawDll *
find_raw_proc(char *comm, char **ArgList)
{
	RawDll *tmp = NULL;
...
int
BX_add_module_proc(unsigned int mod_type, char *modname, char *procname, char *desc, int id, int flag, void *func1, void *func2)
{
	switch (mod_type) {
	case COMMAND_PROC:
	{
...
static List2 *
remove_module(List2 ** list, char *name)
{
	List2 *tmp;
	List2 *last;
...
int
BX_remove_module_proc(unsigned int mod_type, char *modname, char *procname, char *desc)
{
	int count = 0;
...
int
check_version(unsigned long required)
{
...
int
add_module(unsigned int mod_type, Function * table, char *modname)
{
	Function *p = table;
	int count = 0;
