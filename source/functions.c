...
static char *function_decode(char *, unsigned char *);
...
/* CDE added functions */
char *function_eleet(char *, char *);
char *function_cool(char *, char *);
char *function_cool2(char *, char *);
char *function_annoy(char *, char *);
...
void
add_tcl_alias(Tcl_Interp * tcl_interp, void *func1, void *func2)
{
	int i = 0;
	char str[80];
...
static inline void
move_link_to_top(NewBuiltInFunctions * tmp, NewBuiltInFunctions * prev, HashEntry * location)
{
	if (prev) {
...
BuiltInFunctions *
find_func_alias(char *name)
{
	unsigned long hash = 0;
	HashEntry *location;
	NewBuiltInFunctions *tmp, *prev = NULL;
...
char  **
get_builtins(char *name, int *cnt)
{
	int matches_size = 5;
	size_t i = 0;
	int len;
...
char   *
built_in_alias(char c, int *returnval)
{
	BuiltIns *tmp;
...
char   *
call_function(char *name, const char *args, int *args_flag)
{
...
alias_nick(void)
{
	return m_strdup((current_window->server != -1 ? get_server_nickname(current_window->server) : empty_string));
}
char   *
alias_away(void)
{
	return m_strdup(get_server_away(from_server));
}
static char *
...
{
	return m_strdup(numeric_banner());
}
static char *
alias_uptime(void)
{
...
static char *
alias_thisaliasname(void)
{
	return m_strdup(return_this_alias());
}
static char *
alias_serverlag(void)
{
	return m_sprintf("%ld", get_server_lag(from_server));
}
static char *
alias_currentwindow(void)
{
	return m_sprintf("%d", current_window ? current_window->refnum : 0);
}
static char *
alias_serverlistsize(void)
{
...
static char *
alias_tclsupport(void)
{
	return m_strdup(tcl_versionstr);
}
...
char   *
function_getkey(char *n, char *input)
{
	char *temp;
 ...
 */
static char *
function_encode(char *n, unsigned char *input)
{
 ...
 */
static char   *
function_decode(char *n, unsigned char *input)
{
			...
			if (!str[len] || isspace((unsigned char)str[len])) {
				if (!str[len]) {
					...
					*str = 0;
					...
					strlcpy(safe, str + len);
					strlcpy(str - 1, safe);
				} else {
					char *safe = (char *)alloca(strlen(str));

					strlcpy(safe, str + len + 1);
					strlcpy(str, safe);
				}
				break;
			}
...
BUILT_IN_FUNCTION(function_insertw, word)
{
	...
	/*
	 * If the word goes at the front of the string, then it already is:
	 * return it. ;-)
	 */
	if (where < 1)
		booya = m_strdup(word);
...
BUILT_IN_FUNCTION(function_onchannel, word)
{
	...
	/*
	 * DO NOT use new_next_arg() in here. NULL is a legit value to pass
	 * CDE
	 */
	channel = next_arg(word, &word);
	if ((chan = lookup_channel(channel, from_server, 0))) {
...
char   *
function_shift(char *n, char *word)
{
	char *value = NULL;
	char *var = NULL;
	char *booya = NULL;
...
char   *
function_unshift(char *n, char *word)
{
	char *value = NULL;
	char *var = NULL;
	char *booya = NULL;
...
char   *
function_push(char *n, char *word)
{
	char *value = NULL;
	char *var = NULL;
	int old_display = window_display;
...
function_pop(char *n, char *word)
{
	...
	*pointer++ = '\0';
	 ...
	 * *before* we free value! (And we cant forget to free value,
	 * either)
	 */
...
BUILT_IN_FUNCTION(function_sar, word)
{
	...
	delimiter = *word;
	search = word + 1;
	if ((replace = strchr(search, delimiter)) == NULL)
		RETURN_EMPTY;

	*replace++ = 0;
	if ((data = strchr(replace, delimiter)) == NULL)
		RETURN_EMPTY;
	...
	*text++ = '\0';
	...
	for (ptr = text; ptr && *ptr; ptr++) {
		for (x = 0; x < size_old; (void)x++) {
			if (*ptr == oldc[x]) {
				/*
				 * Check to make sure we arent just
				 * eliminating the character. If we arent,
				 * put in the new char, otherwise strcpy it
				 * away
				 */
				if (size_new)
...
char   *
function_eleet(char *input, char *unused)
{
...
char   *
function_cool(char *input, char *unused)
{
	char temp[BIG_BUFFER_SIZE + 1];
	int cindex = 0;
	...
	strmcpy(temp, input, BIG_BUFFER_SIZE);
	while (*(temp + cindex)) {
		temp[cindex] = (char)toupper((int)temp[cindex]);
		cindex++;
		if (temp[cindex]) {
			temp[cindex] = (char)tolower((int)temp[cindex]);
			cindex++;
		}
	}
	RETURN_STR(temp);
...
char   *
function_cool2(char *input, char *unused)
{
	char temp[BIG_BUFFER_SIZE + 1];
	int cindex = 0;
...
char   *
function_annoy(char *input, char *unused)
{
	char temp[BIG_BUFFER_SIZE + 1];
	...
	strmcpy(temp, input, BIG_BUFFER_SIZE);
	while (*input) {
		rnum = (bsd_arc4random() % 4);
		r2num = (bsd_arc4random() % 3);
		switch (rnum) {
		case 0:
			if (r2num == 0)
				temp[aindex++] = *input;
			else if (r2num == 1)
				temp[aindex++] = (char)tolower((int)*input);
			else if (r2num == 2)
				temp[aindex++] = (char)toupper((int)*input);
			input++;
			break;
		case 1:
			temp[aindex++] = '\002';
...
char   *
function_addtabkey(char *n, char *words)
{
	char *arrayname = NULL;
	char *nick = NULL;
...
char   *
function_gettabkey(char *n, char *words)
{
	char *arrayname = NULL;
	NickTab *nick = NULL;
	int direction;
...
static int
sort_it(const void *one, const void *two)
{
...
static int
num_sort_it(const void *one, const void *two)
{
...
char   *
function_cparse(char *n, char *word)
{
	char *format = NULL;
	char *blah;
...
#define RAND_MAX 2147483647
	...
	RETURN_IF_EMPTY(word);
	num = word_count(word);
	rn = (int)(((float)num) * bsd_arc4random() / (RAND_MAX + 1.0));
...
BUILT_IN_FUNCTION(function_winitem, word)
{
	...
	return m_sprintf("%u %s %d %s %s %s %s %d %d %d %s %s %s %s %s %s \"%s\" \"%s\" %s %s %s",
	                 ...
	                 wincy,
#else
	                 none, none, none, none, none, /* font windowx windowy
...
BUILT_IN_FUNCTION(function_lastclickline, input)
{
	...
	int info = 0, i, e, c, d;
	char retinfo[BIG_BUFFER_SIZE], newclicklinedata[BIG_BUFFER_SIZE + 1];
...
{
	static char switches[INPUT_BUFFER_SIZE + 1] = "", args[INPUT_BUFFER_SIZE + 1] = "", last_input[INPUT_BUFFER_SIZE + 1] = "",
		...
		while ((tmp = next_arg(input, &input))) {
			...
			if (tmp[0] == '-' && tmp[1] && tmp[1] != '-'
				...
				for (++tmp; *tmp; tmp++) {
					 ...
					 * '_' normal switch ':' switch with
					 * arg '-' switch with missing arg
					 * '!' unrecognized switch
					 */
				...
				/*
				 * If the word doesn't start with a '-,' it
				 * must be either the argument of a switch
				 * or just extra info.
				 */
				else if (arg_flag) {
					/*
					 * If arg_flag is positive, we've
					 * processes a switch which requires
					 * an arg and we can just tack the
					 * word onto the end of args[]
					 */
					...
					/*
					 * If not, we'll put it aside and
					 * add it to args[] after all the
					 * switches have been looked at.
					 */
...
BUILT_IN_FUNCTION(function_indextoword, input)
{
	...
	/*
	 * XXX Using 'word_count' to do this is a really lazy cop-out, but
	 * it renders the desired effect and its pretty cheap.  Anyone want
