...
GtkWidget *mainwindow, *mainviewport, *mainmenubar, *mainscroller, *mainscrolledwindow,
*mainclist, *mainbox;
GdkFont *mainfont;
char   *mainfontname;
...
int lastclickcol, lastclickrow, contextx, contexty;
...
void
gtk_paste(GtkWidget * widget, GtkSelectionData * selection_data,
          gpointer data);
void
...
#ifdef __sun__
int
gtkputs(const char *str, int n, int (*blah)(int))
#else
int gtkputs
__P((const char *str, int n, int (*blah)(int)))
#endif
{
	if (str && *str) {
		/*
		 * write((output_screen && output_screen->pipe[1]) ?
		 * output_screen->pipe[1] : mainpipe[1], str, strlen(str));
		 */
...
int
notebook_page_by_refnum(int refnum)
{
...
void
sendevent(unsigned int event, unsigned int window)
{
...
int
new_menuref(MenuRef ** root, int refnum, char *menutext, int checked)
{
...
int
remove_menuref(MenuRef ** root, int refnum)
{
	MenuRef *prev = NULL, *tmp = *root;
...
MenuRef *
find_menuref(MenuRef * root, int refnum)
{
	MenuRef *tmp = root;
...
gint
window_destroy(GtkWidget * widget, GdkEvent * event, gpointer data)
{
#ifdef WINDOW_CREATE
	Screen *closescreen;
...
int
gtkputc(unsigned char c)
{
	unsigned char tmpbuf[2];

	tmpbuf[0] = (unsigned char)c;
	tmpbuf[1] = 0;
	if (output_screen && output_screen->pipe[1])
...
void
drawnicklist(Screen * this_screen)
{
...
gint
gtk_keypress(GtkWidget * widget, GdkEventKey * event, gpointer data)
{
	char extkeycode[4];
...
void
output_info(gpointer data, int pipefd)
{
...
void
gtk_sizewindow(Screen * thisscreen)
{
...
void
gtk_label_set_color(GtkWidget * label, int color)
{
	GtkStyle *style;
	GtkStyle *defstyle;
...
void
gtk_windowsize(GtkWidget * widget, gpointer data)
{
	Screen *sizescreen;
	...
	while (sizescreen) {
		...
		/*
		 * In MDI mode we check that viewport exists... otherwise
		 * the tab is being removed
		 */
		if (sizescreen->alive && sizescreen->maxfontwidth > 0 && sizescreen->maxfontwidth > 0 && sizescreen->viewport) {
			/*
			 * Not sure why it is x3 and x2.... klass shows as
			 * 2x2 and the variation is 6x4
			 */
			...
			/*
			 * size_allocate((GtkWidget *)sizescreen->viewport,
			 * (GtkWindow *)sizescreen->window);
			 */
...
void
gtk_windowfocus(GtkWidget * widget, gpointer * data)
{
	Screen *focusscreen;
...
void
gtk_notebookpage(GtkWidget * widget, GtkNotebookPage * page, gint page_num, gpointer * data)
{
...
void
gtk_scrollerchanged(GtkWidget * widget, Screen * thisscreen)
{
...
int
gtk_contextmenu(GtkWidget * widget, GdkEventButton * event, Screen * this_screen)
{
	...
	/*
	 * This will only give decent results with fixed width fonts at the
	 * moment
	 */
...
void
gtk_file_ok(GtkWidget * widget, GtkParam * param)
{
	char *tmp;
...
void
gtk_file_cancel(GtkWidget * widget, GtkParam * param)
{
	gtk_widget_destroy(GTK_WIDGET(param->window));
	codeptr = (char *)param->data[0];
...
void
delete(GtkWidget * widget, GtkWidget * event, GtkParam * param)
{
	gtk_widget_destroy(GTK_WIDGET(param->window));
	free(param);
}
...
void
delete2(GtkWidget * widget, GtkParam * param)
{
	gtk_widget_destroy(GTK_WIDGET(param->window));
	free(param);
}
...
void
zvt_load_font(char *fontname, Screen * screen)
{
	GdkFont *font = NULL;
...
void
gtk_windowicon(GtkWidget * window)
{
#ifndef USE_IMLIB
	GtkStyle *iconstyle;
...
void
font_ok_handler(GtkWidget * widget, GtkParam * param)
{
	gchar *font;
...
void
font_ok_handler2(GtkWidget * widget, GtkParam * param)
{
	gchar *font;
...
typedef struct _propstruct {
	GtkWidget *nbwindow, *clist, *entryfield, *check;
	int which, last;
	Window *nb_window;
}       GtkProp;
...
void
nb_delete(GtkWidget * widget, GtkWidget * event, GtkProp * gtkprop)
{
	int z;
...
int
gtk_click_nicklist(GtkWidget * widget, GdkEventButton * event, GtkWidget * window)
{
	gchar *nicktext = NULL;
...
void
select_row(GtkWidget * widget, gint row, gint column, GdkEventButton * event, GtkProp * gtkprop)
{
...
void
font_browse(GtkWidget * widget, GtkParam * firstparam)
{
...
void
gtk_setfocus(Screen * thisscreen)
{
	if (get_int_var(MDI_VAR)) {
...
gtk_command_handler(void)
{
	...
	case GTKPROP:
	{
		GtkWidget *nbwindow, *frame, *label, *tmphbox, *tmpvbox,
		*scrollwindow;
		GtkProp *gtkprop;
	...
	case GTKFONT:
	{
		GtkWidget *fontwindow, *fontentry, *browsebutton,
		*defaultcheck, *allwindowscheck, *okbutton,
		*cancelbutton, *hbox, *vbox;
	...
	case GTKWINKILL:
	{
		...
		if (get_int_var(MDI_VAR)) {
			...
			/*
			 * Make sure gtk_windowsize() doesn't try to
			 * use this screen
			 */
	...
	case GTKMDI:
		if (gtkcommand.data[0]) {
			...
			/*
			 * gtk_signal_connect(GTK_OBJECT(nbwindow),
			 * "delete_event", GTK_SIGNAL_FUNC (nb_delete),
			 * gtkprop);
			 */
...
/* Used to tell the main thread to continue after the window has
   been created. */
void
viewport_map(GtkWidget * widget, gpointer data)
{
...
void
window_map(GtkWidget * widget, gpointer data)
{
	Screen *mapscreen;
...
gtkbx_init(void)
{
	...
	/*
	 * glib 1.2.10 seems to poll on fd 0, so we can't leave it in a
	 * closed state.
	 */
...
void
removetilde(char *dest, char *src)
{
	int z, cur = 0;
...
GtkWidget *
newsubmenu(MenuStruct * menutoadd)
{
	GtkWidget *tmphandle, *tmpmenu;
	MenuList *tmp;
	char expbuf[500];
	...
	/*
	 * When you readd shared menu support remove the #ifndef GTK from
	 * commands2.c
	 */
	...
	while (tmp != NULL) {
		if (tmp->menutype & GUISEPARATOR)
			...
			if (tmp->refnum > 0) {
				/*
				 * If we already have an entry defined, use
				 * it's info
				 */
		...
		/*
		 * if((tmp->menutype & GUIBRKSUBMENU) || (tmp->menutype &
		 * GUIBRKMENUITEM))
...
void
newmenubar(Screen * menuscreen, MenuStruct * menutoadd)
{
	GtkWidget *tmphandle;
	MenuList *tmp;
	char expbuf[500];
...
void
gui_scroll(int top, int bot, int n)
{
	/* Stolen from term.c */
...
void
gui_new_window(Screen * gtknew, Window * win)
{
	predicate = 0;
...
void
gtk_new_window(Screen * gtknew, Window * win)
{
...
void
gui_kill_window(Screen * killscreen)
{
...
int
gui_read(Screen * screen, char *buffer, int maxbufsize)
{
	return read(gtkipcin[0], buffer, maxbufsize);
}

void
gui_settitle(char *titletext, Screen * gtkwin)
{
	if (gtkwin->alive == 0)
		return;
...
void
gui_file_dialog(char *type, char *path, char *title, char *ok, char *apply, char *code, char *szButton)
{
...
void
gtk_paste(GtkWidget * widget, GtkSelectionData * selection_data, gpointer data)
{
	char *sdata;
...
gtk_main_paste(int refnum)
{
	...
	if (!smartpaste) {
		...
		/*
		 * Rosmo's Incredibly Dull SmartPaste: Fits as much as
		 * possible into a 80-nick_length buffer, stripping spaces
		 * from beginning and end and then pasting
		 */
		...
		clip = oclip;
		while (*clip && *clip == ' ')
			clip++;         /* Strip spaces from the start */
		...
		*smart = '\0';
		while (*clip) {
			...
			*smart = '\0';
			...
			if (strcmp(smart, "") != 0) {   /* If our smart buffer
							 * has stuff, send it! */
				if (input_line) {
					if (*smart) {
						ADD_TO_INPUT(smart);
...
void
gui_scrollerchanged(Screen * screen, int position)
{
	int *pos;
...
void
gui_query_window_info(Screen * screen, char *fontinfo, int *x, int *y, int *cx, int *cy)
{
...
int
gui_send_mci_string(char *mcistring, char *retstring)
{
	return 0;
}

void
gui_get_sound_error(int errnum, char *errstring)
{
}

void
gui_menu(Screen * screen, char *addmenu)
{
...
int
gui_isset(Screen * screen, fd_set *rd, int what)
{
...
void
gui_font_set(char *font, Screen * screen)
{
...
int
gui_screen_width(void)
{
	return gdk_screen_width();
}

int
gui_screen_height(void)
{
	return gdk_screen_height();
}

void
gui_setwindowpos(Screen * screen, int x, int y, int cx, int cy, int top, int bottom, int min, int max, int restore, int activate, int size, int position)
{
	if (screen && screen->window) {
...
void
BX_gui_mutex_lock(void)
{
	gdk_threads_enter();
}

void
BX_gui_mutex_unlock(void)
{
	gdk_threads_leave();
}

int
gui_setmenuitem(char *menuname, int refnum, char *what, char *param)
{
	MenuRef *tmp = NULL;
...
void
gui_mdi(Window * window, char *text, int value)
{
...
void
gui_update_nicklist(char *channel)
{
...
void
gui_nicklist_width(int width, Screen * this_screen)
{
...
void
gui_startup(int argc, char *argv[])
{
	pipe(guiipc);
	new_open(guiipc[0]);
...
void
gui_setfileinfo(char *filename, char *nick, int server)
{
}
